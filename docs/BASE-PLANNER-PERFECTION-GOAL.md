...
- One connected road network touching every structure. No roads ON ramparts —
  ...
  **231 wall crossings on the cut + 30 container seats + 13 controller
  stand-denial RING tiles + 0 personal cover + 0 unclassified = 274.**
  **ROUND 26 TOOK THE WORD "BUBBLE" OUT OF THAT SECOND CLASS, AND THE RENAME IS
  A FINDING RATHER THAN A TIDY-UP.** `shell.bubble` membership is a
  DISJUNCTION — a tile is bubbled when it is OUTSIDE the shell **or** inside it
  below safe depth — so a class named after that list reads as a claim about
  which side of the wall a container stands on, and it is not one. `plan.mjs`
  prints `30 container seats` today and `declprose-notes.mjs` tags the paved-run
  roster `[container seat]`; the film facet that DID make the side-of-the-wall
  claim was false on **210 of 436** tiles until this round re-derived it from the
  board — criticism 129. **No count moves**: it is the same 30 tiles it has been
  since round 13. Paragraphs further down keep the old word where they are
  narrating the round that used it.
  ...
  **AND THEN THE D8 ROSTER WAS STILL SCOPED TO THE CLASS THAT WAS EASY TO SEE.**
  ...
  cut, so a paved rampart that is a container seat or a stand-denial ring tile — the
  ...
  fleet-wide** broke down 16 `breaks-network` · 7 `no-parallel` · 4 `seat` —
  **a round-21 reading, and today's is 23 = 9 `breaks-network` + 10 `no-parallel`
  + 4 `seat`, derived from each room's own board and PRINTED by `validate.mjs` on
  its own fleet line** rather than carried by this paragraph; the same three
  digits were still sitting inside a validator failure MESSAGE three rounds after
  they stopped being true, which is criticism 126 — that
  last one being the class the old scope had no word for (a container seat's road IS
  ...
  **AND THAT FIX WAS APPLIED TO THE INSTANCE AND NOT TO THE CLASS — THE SAME
  STALE FLOOD WAS STILL THE SOLE EXPANSION PREDICATE ONE STAGE OVER, IN THE SAME
  FILE, WRITTEN BY THE SAME ROUND.** Round 23 re-pointed stage 5b at a live
  flood and left `paveable()` reading `plan.exterior` — assigned exactly once, in
  `pipeline.mjs`'s shell hand-off, from LAYER 2, and never refreshed after any
  pass that moves a rampart — while `paveable()` is the only expansion
  predicate `fieldFromNetwork` has and feeds three board-writing stages: the
  stitch, the rampart spurs and the ext-face net. Instrumented at the predicate
  itself over all 172 rooms, the frozen answer and the live one disagree in the
  dangerous direction on **35 tiles across 21 rooms** (stale says INSIDE, the
  wall the room is standing on says OUTSIDE — so the paver may pave outside the
  wall) and in the conservative direction on **392 tiles across 144 rooms**
  (legal interior silently refused); the owner-voice reviewer's independent
  re-compose, counting whole-room verdict flips rather than reachable ones, read
  **46 across 30** and **314 across 146** — a definitional difference between two
  instruments that agree about the defect, and both rosters include the exact two
  tiles criticism 106 called BLOCKING. **The structural finding is the one worth
  keeping**: nothing REMOVES a rampart before layer 7, so the four placement
  layers' drift is one-directional — withhold-only, **0 exposed at every one of
  them** — and `paveable()` is the only consumer in the pipeline that runs after
  a rampart-REMOVING pass and therefore the only one with the direction that
  bites. The board effect today is **zero tiles in 172 of 172 rooms**, and that
  is terrain luck rather than a defence: 46 of the 46 mis-accepted tiles are D8
  of a shipped rampart, 36 of 46 are D8 of a cut tile — exactly the class the
  spur search picks from — and the single exposed tile that carries a road at all
  (E19S8 `34,36`) is `roadLayer` 1, an eco road `paveable()` never gated.
  The fix is the class: one `exteriorFlood` in `shared.mjs`, a memoised
  `liveExterior(terrain, plan)` re-pointed at every late consumer, an explicit
  `invalidateExterior(plan)` at **all eight rampart-mutation sites** (five in
  `pipeline.mjs`, `pruneInertRamparts` and the 7b reflow in `layer-walls.mjs`,
  the retirement in `layer-ext.mjs` — two of which had been riding on the memo
  key's length term without anyone knowing), and the four consumers that keep the
  FROZEN flood on purpose — they are asking about layer 2's min-cut ENCLOSURE and
  not about today's wall — now say so by publishing
  `meta.exteriorContract`: **688 records, 172 rooms × the four consumers in run
  order (towers L3, labs L4, misc L5, ext L6), exposed 0 fleet-wide, withheld
  2,786**, with a new `gate:"exterior"` shortfall class behind it and the
  obligation gated BOTH ways, so a layer that exposes tiles owes a declaration
  and a declaration filed over a contract that exposed nothing fails.
  **BOTH HALVES OF THAT LAST SENTENCE WERE FALSE FOR A ROUND AND ROUND 25 FIXED
  THEM RATHER THAN RE-WORDING THEM.** `withheld` was PRESENCE-AND-TYPE gated only
  (`Number.isInteger && >= 0`): zeroing all 688 records, inflating one by 500 and
  setting the whole fleet to 999999 each passed `172/172` with byte-identical
  summary lines, there was no `withheldTiles` roster, and `plan.exterior` is
  stripped from the artifact, so **the fleet's 2,786 was a number no reader could
  check and no producer was held to** — criticism 123. And the `exterior` class
  had no PASSING state at all: exposure pushed a failure unconditionally, so
  declaring it correctly bought nothing and "gated BOTH ways" described one way —
  criticism 127. What the four records carry now: `withheldTiles` per record
  (`withheld` is its length in 688/688), `meta.exteriorContractBasis` stating in
  the room's own text that the four counts are **one tile set read at four
  moments** and must not be summed (`2,786` summed against **797 distinct tiles**
  fleet-wide — E11S1 alone publishes `[10,10,10,11] = 41` for 11 tiles), and
  `meta.shell.cutAtFreeze`, the layer-2 cut the frozen flood was actually taken
  against, which makes the first consumer's roster re-derivable from terrain in
  **172/172** rooms. The contract's own justification — nothing removes a rampart
  before layer 7 — is a gate now and a strictly stronger one than it was written
  as: **monotone by SET CONTAINMENT** in run order (L3 ⊆ L4 ⊆ L5 ⊆ L6), naming the
  tile that dropped out rather than comparing two counts.
  (`layer-ext.mjs` keeps its own `floodExterior` and that is not a third copy —
  roads tunnel natural wall in that one, and the file says so.) The observation
  this bullet has been repeating — "the layer-7 roads outside the wall are
  exactly the 3 legitimate ones" — is an INVARIANT now rather than a reading:
  every layer-7 road of every kind must be inside the shipped wall unless its
  kind is in the declared-legit set, and `validate.mjs` derives and prints the
  count on its own line. Filed as **criticism 114**, and the sentence it earns is
  that the round's whole board diff is a measurement: terrain luck became one.

  **AND THE ARITHMETIC GATE BESIDE IT WAS HANDING OUT AN EXEMPTION THAT BOUNDED
  NOTHING, FOR A DERIVATION THAT HAD BEEN AVAILABLE THE ENTIRE TIME.** Criticism
  108 closed the refusal magnitudes by re-deriving them on the shipped board and
  carved out one honest-looking hole: where the offered parallel is already paved
  by this room's own `alongCutMoved` take, the flood cannot see the board the
  offer was priced on, so the checker returned before any arithmetic ran. Two
  reviewers took that door from opposite sides in the same round. The offer-time
  board is **reconstructible** — `shipped ∪ {from2} \ {t}`, where `from2` is the
  take's own origin and is UNIQUE per room (the one D8 rampart-neighbour of the
  target that carries no road today and yields a board on which this very offer
  is measurably worse; zero candidates or more than one is a FAILURE, not a
  licence) — so
  the exemption is dead rather than tightened, and the class it was excusing is
  re-derived like everything else. What it had been costing is the other half:
  because the exemption was granted on BOARD STATE, an editor could point a
  fabricated refusal at the room's own real take and claim anything, and a
  refusal reading **"999 more road tile(s) fall off the network (5 → 3; newly
  off: 99,99)"** — a magnitude that is not the subtraction beside it, readings
  that IMPROVE the axis it claims to worsen, a baseline the room does not
  publish, and a tile off the 50×50 board — shipped `pass 172/172 · fail 0`.
  Well-formedness now runs on EVERY priced offer of EVERY refusal in EVERY loop,
  in-roster or not, and the container/extension-axis prices that loop 1 used to
  reject and loop 2 used to ignore are accepted and fully re-derived.
  Filed as **criticism 113**.
...
- No structure on source/controller/mineral tiles (extractor on mineral exempt),
  ...
  **NUMERAL 2/2**. **The theme is that the round's own gates were attacked one
  ...
  written; **122 each way today**, and the 30 they gained are the eight claims
  round 22's waiver scope was silently shielding plus the ones the tree has added
  since — round 25's segment-joining scan (criticism 126) is the largest single
  jump, because a claim the scanner could not see was a claim these two cases
  could not flag either — see criticism 109), which is the
  ...
  **The theme is that a gate is only as honest as the board it is asked about**:
  ...
  Round 24 adds **44 cases tagged `r24`** for a net add of 44, again with nothing
  retired (1124 → **1168**). The grouping: **OB1 the out-of-roster loop, where
  nothing ran at all, 12/12** · **OB1 the taken-parallel witness class, until
  this round UNREACHABLE because the gate bare-returned on it, 11/11** ·
  **OB2 the four layer-7 road kinds shipped outside the wall plus the enclosure
  contract 10/10** · ME the declaration order on a moved and an unmoved board
  5/5 · OB1 the container axis, a price shape one loop refused and the other
  ignored, 4/4 · OM3 the pass verdict on an unjudged lap 2/2. **The theme is a
  gate that had written itself an exception**: 27 of the 44 attack one — the
  witness class and the second loop are the two doors round 23's own fix left
  in the arithmetic it had just closed, and neither could be reached by a
  mutation before this round because the checker returned before it looked. The
  one class NOT reachable from here is stated rather than counted in: the OM4
  channel-agreement gate is a SOURCE guard over `plan.mjs`, and this harness
  mutates artifacts, so it is exercised live (it bites the unfixed tree in E6S3
  and E7S5) and carries no case. **That sentence was the round's own excuse and
  round 25 deleted the thing it excused** — the source guard is an OUTPUT
  comparison now and carries three cases, at criticism 121.
  Round 25 adds **32 cases tagged `r25`** for a net add of 32, again with nothing
  retired (1168 → **1200**). The grouping, taken off the harness's own case names
  rather than off a cluster's summary: **MM2 the withheld witness — roster,
  containment, frozen-cut snapshot and basis, 10/10** · **OB1 the film-vs-note
  rampart taxonomy tile by tile 5/5** · ML7 the `exterior` declaration class,
  which until this round had no passing state to mutate, 4/4 · MM3 the eco skip's
  index (with the two-real-key control kept beside it) 4/4 · MM1 the unjudged
  reason across three reader channels 3/3 · OM1 the kinded mobility verdict 3/3 ·
  OM2 the pre-take tense 2/2 · MM5 the refusal census inside a failure message
  1/1. **The theme is a witness that was decorative**: 14 of the 32 attack a
  number a producer could have written freely — `withheld` survived being zeroed
  fleet-wide and being inflated by 500, and a `gate:"exterior"` class that pushes
  bad unconditionally is an obligation no mutation can satisfy or break, so
  neither had a case before this round for the same reason: there was nothing to
  disagree with. **And the cluster's own count was 33 against a harness that
  prints 32** — the roster it wrote sums to 32 on its own lines, `1168 + 32 =
  1200`, and the figure written here is the one `mutate.mjs` prints, because a
  ledger that takes a total from a summary rather than from the tool is the class
  criticism 105 exists for.
  Round 26 adds **37 cases tagged `r26`** for a net add of 37, again with nothing
  retired (1200 → **1237**). The grouping, taken off the harness's own case names:
  **MM1 the frozen anchor and the four withheld rosters 15/15** · **MM3 the
  pre-take basis census parsed against the record it is printed on 6/6** ·
  **OM1 the film's rampart census against the shipped board 6/6** · ML6 the three
  unjudged channels held to whole-value equality 5/5 · OB1 the seat disjunct
  2/2 · MM5 the cover caption's occupant 2/2 · OL2 an empty facet's reason
  rewritten as a world-fact the room refutes 1/1. **The theme is a gate that
  compared two readers instead of a reader and the board**, and the structural
  half of it is that **11 of the 37 forge a FILE this suite writes rather than a
  plan** — a new `runFile` harness patches the shipped
  film, page or chip, runs the gate against the room's UNMUTATED plan, restores
  the bytes in a `finally` and reports a FAILURE rather than a pass if the
  restore is not byte-identical or if the forgery changed nothing. That closes
  criticism 128(a)'s split in the direction it asked for: the four round-25
  channel forgeries lived outside the count, and their round-26 successors are
  inside it. **And the cluster's own roster said 16 for MM1 against a harness
  that prints 15** — the figure written here is `mutate.mjs`'s, for the reason
  the round-25 paragraph above gives.
...
- Deterministic output — identical plans across runs. Verified by hashing
  ...
  on the 172-room world: the round-26 artifact was rebuilt **twice by the
  producer cluster**, both runs byte-identical at md5
  **`d7e82af74326da3a7aad7caabca17be4`**, which is the number
  `md5sum tools/plan-suite/out-v2/plans-hub.json` prints today, and the doc pass
  re-derived that digest off the shipped file rather than building a third time —
  which is one reading of the artifact and is said here rather than counted as
  three. Round 25's `1c07287158226514fc50974430489d15` is retired into the chain
  below with the rest. Round 24's own paragraph is kept below for the trend line: that artifact
  was rebuilt **four times by the
  producer cluster**, and the pair the determinism claim rests on is the LAST
  two — P3 and P4, compared byte for byte with `cmp` rather than by digest, on a
  tree whose `md5sum tools/plan-suite/v2/*.mjs` was captured before P3 and
  re-diffed after P4 with no difference — both printing md5
  **`2dfcb104e032ced89ff7940aaace4b39`**, which is retired into the chain below.
  All four builds
  print it, and the two the pair does NOT rest on are the interesting half:
  between P1 and P3 the tree took comment-only edits plus five
  `invalidateExterior` calls that force a re-flood of a pure function, so three
  builds across that produced the identical artifact — which is the evidence
  those edits were output-neutral rather than the claim that they were.
  ...
  **One determinism pair was DISCARDED in round 24 and is recorded rather than
  quietly dropped**: the first pair disagreed
  (`725409109168606d02f6c9ba425d898a` vs `35320a64f7ff19bc9dced76c4330352e`),
  which is not nondeterminism — a validator-cluster finding landed while the pair
  was running and `declprose-mobility.mjs` was edited between the two builds. The
  pair was DIFFED rather than assumed: the entire delta is
  `meta.shortfalls[].detail` in E17S3 and E7S9, exactly the mass-share fix and
  nothing else. A disagreeing pair whose diff is explained is worth more than a
  matching pair nobody looked inside.
  **The doc pass did NOT rebuild in rounds 24, 25 or 26 and this bullet says so
  rather than counting the runs it did not make**: it re-derived the digest off
  the shipped file and re-ran
  `validate`, `mutate` and `numeral-audit` against it, which is a reading of the
  artifact and not a second reading of the build. **Round 25 proved its zero-board
  claim rather than asserting it**, and the mechanism is worth the clause because
  it is the one this bullet has always wanted: the `anim/` films were still the
  ROUND-24 build's when the first round-25 run finished, and `plan.mjs`'s own
  check still read `animations: 172/172 carry this plan's structure digest` — a
  digest taken over the previous round's boards agreeing with this round's plans
  is a stronger statement than a diff, because it is made by an instrument that
  had no idea it was being used as one. Round 22's was rebuilt three
  ...
  **Round 26 moved NOTHING either — 0 of 172 boards, THREE zero-board rounds
  running — and it proved the zero the same way round 25 did rather than by
  diffing.** The producer cluster's two `--all-claimable` runs both printed
  `d7e82af74326da3a7aad7caabca17be4`, and `plan.mjs`'s own
  `animations: 172/172 carry this plan's structure digest` still read clean while
  the films in `out-v2/anim/` were STILL THE ROUND-25 BUILD'S — a digest taken
  over last round's boards agreeing with this round's plans, made by an
  instrument that did not know it was being used as one. Every fleet total is
  byte-still: **ramparts 8,208 · roads 14,100 · extensions 10,320 (60/60 × 172) ·
  declarations 300 · planner notes 236 · road+rampart `274 = 231 + 30 + 13 + 0 +
  0` across 153 rooms**, `validate.mjs` still ends on `pass 172/172 · fail 0 ·
  declared-shortfall 122` with every physical total 0, and all four of its
  derived fleet lines (layer-7 roads outside the wall 3 · along-cut offers 14 ·
  refusals `23 = 9 + 10 + 4` · negotiated detour 16 of 57, floor −9) are
  character-identical to round 25's. The artifact delta, derived key by key
  against the round-25 file: **`meta.shell.cutDrift` added in all 172 rooms
  (`[]` in the 143 that never move their cut, 80 entries across the other 29) ·
  `meta.exteriorContractBasis` re-rendered in all 172 (1 distinct string → **166**)
  · every `anim/<room>.json` re-exported with the seat facet split on the BOARD
  instead of on `shell.bubble` — `seat.bubble` is gone, `seat.outside`/`seat.inside`
  read 347/89 where the old pair read 295/141 — and `emptyBecause` derived per
  room (494 rows over 7 constant sentences → **555 rows over 41 room-specific
  ones**). No board tile, no count, no structure, no room, no declaration.**
  Single-link round: `1c07287158226514fc50974430489d15` (round 25, the artifact
  both round-26 reviewers attacked, and both of them re-derived that md5 before
  they started) → **`d7e82af74326da3a7aad7caabca17be4`**, and round 25's is
  retired into the chain below.
  **Round 25 moved NOTHING either — 0 of 172 boards, two zero-board rounds
  running — and this one is not a headline, it is what a reader-channel round
  looks like.** The BLOCKING finding was in the FILM, four MEDIUMs and a MAJOR
  were in sentences, and the only producer edits were witnesses being published
  where a number used to stand alone. Every fleet total is byte-still: **ramparts
  8,208 · roads 14,100 · extensions 10,320 (60/60 × 172) · declarations 300 ·
  planner notes 236 · road+rampart `274 = 231 + 30 + 13 + 0 + 0`**, and
  `validate.mjs` still ends on `pass 172/172 · fail 0 · declared-shortfall 122`
  with every physical total 0. The artifact delta, derived key by key against the
  round-24 file: **`meta.exteriorContract[].withheldTiles` and
  `meta.exteriorContractBasis` added in all 172 rooms · `meta.shell.cutAtFreeze`
  added in all 172 rooms · `preTakeShortfallBasis` re-rendered on all 134 records
  carrying it (1 distinct string → **24**) · the note and declaration texts
  re-rendered where OM1/OM2/OM4/OL6 and the `sharedTarget` clause landed · every
  `anim/<room>.json` re-exported with a top-level `rampartCensus`. No board tile,
  no count, no structure, no room.** Single-link round:
  `2dfcb104e032ced89ff7940aaace4b39` (round 24, the artifact both round-25
  reviewers attacked, and both of them re-derived that md5 before they started) →
  **`1c07287158226514fc50974430489d15`**, and round 24's is retired into the
  chain below. **The one figure this round could NOT leave alone is not a board
  figure and is written here rather than in an entry**: `meta.shell.cutAtFreeze`
  shows the layer-2 cut differing from the shipped cut in **29 rooms**, where the
  mechanical reviewer's derivation had predicted **7**. The 7 was a count of rooms
  where the difference MOVED the withheld derivation; the 29 is the count of rooms
  where the two cuts differ at all, and the second number is the one the snapshot
  had to be sized for. Both are true, they are answers to different questions, and
  the round found that out by publishing the snapshot instead of by arguing —
  which is the only reason this paragraph can state it as a correction rather than
  as a disagreement.
  **Round 24 moved NOTHING — 0 of 172 rooms differ by a single structure tile —
  and that zero is the round's headline rather than its excuse.** Two BLOCKING
  board-class findings were fixed (the stale flood re-pointed at every consumer
  that runs after a rampart moves, criticism 114; the along-cut arithmetic's
  exemption killed and every offer re-derived on a reconstructed offer-time
  board, criticism 113) and the boards did not move, because the 35 exposed and
  392 withheld tiles the fixed predicate now answers differently carry no road
  the search would have taken. **That is the whole point of measuring it**: last
  round the same class cost two tiles and one primary haul lane, this round it
  costs none, and the difference between those two sentences is terrain and not
  the program — which is a fact the fleet now publishes (`meta.exteriorContract`)
  instead of a fact nobody could have known. The entire artifact delta, derived
  key by key against the round-23 file rather than described: **`meta.exteriorContract`
  added in all 172 rooms · `metric.maxDetour` and `metric.coveredPairs` added to
  the 57 mobility declarations · `meta.shortfalls[].detail` re-rendered in
  exactly 2 rooms (E17S3 and E7S9, the only two whose mobility gate judged no
  pair). Nothing else — no board tile, no count, no other room, no other key.**
  Single-link round: `5505af67c1c2587a97e2753979d459d5` (round 23, the artifact
  both round-24 reviewers attacked, and both of them re-derived that md5 before
  they started) → **`2dfcb104e032ced89ff7940aaace4b39`**, and round 23's is
  retired into the chain below.
  ...
  **byte-identical to its predecessor plus `"source": "towers"` inserted after
  ...
  (`5505af67c1c2587a97e2753979d459d5` is retired into that chain as the round-23
  artifact — kept on disk, and it earned its keep the same way its predecessor
  did: round 24's own gates were run against it before the rebuild, and the
  reviewers' six exploits were landed on copies of it, which is what makes
  "ESCAPED then CAUGHT" a before-and-after rather than two assertions — as
  `87ecaf3a0abfe1a29102d8520c30bf2a` is retired as the round-22
...
- Defender mobility: internal wall-to-wall paths must not exceed attacker external
  ...
  **AND THE SENTENCE THAT ANNOUNCED THAT CONTRACT WAS ITSELF GATED ON ONE HURDLE
  INSTEAD OF TWO, IN THREE PLACES, AND SHIPPED A PASS VERDICT ON A LAP NOTHING
  JUDGED.** `declprose-mobility.mjs` branched on `lap > target` alone, so a room
  whose gate judged ZERO pairs has `maxGated 0`, fails that test, and falls into
  the INSIDE-the-target branch. Two rooms shipped it — E17S3 (`gatedPairs 0`,
  ungated 1.27) and E7S9 (`gatedPairs 0`, ungated 1.67) — reading *"the defender
  lap is 0 over pairs costing more than 4 tiles of detour, INSIDE the 1.2 target
  (ungated over every pair it is 1.27/1.67)"*: a pass verdict with an
  ABOVE-target number as its supporting parenthetical, which is the exact
  inversion of the paragraph above. **The comment four lines up quotes this
  headline as the defect and names E7S9, and then the fix was applied to the
  paragraph underneath it** — the same one-copy-of-two shape as the frame banner,
  in a file whose whole subject is saying what was measured. The headline takes
  the chips' three-way branch now (`judged === false` is a non-verdict carrying
  the ungated reading as context), and the round found two further copies of the
  identical false verdict by sweeping the file rather than by being told: the
  MASS-SHARE clause (*"THERE IS NO MISS TO ATTRIBUTE: the room's gated lap is 0,
  inside the 1.2 target"*) and the CAUSE clause, both one sentence down, both now
  on the same branch. The remaining "inside the target" strings in that file are
  per-PAIR readings and not room verdicts, which is stated because the sweep is
  the evidence and the count is not. `metric.maxDetour` and
  `metric.coveredPairs` are published on all **57** mobility declarations to make
  the branch derivable from the record, and the gate requires a pass verdict to
  have `gatedPairs > 0`. Filed as **criticism 116**.
...
- The plan gallery page + animation must make the reasoning legible ("watch the
  ...
  **AND THEN THIS PARAGRAPH DID THE THING IT WAS WRITTEN TO COMPLAIN ABOUT, IN
  ...
  reading, re-derived off the round-24 artifact rather than carried from the
  round-23 one it happens to equal: laid **spur 375 · swampPave 82 · reflow
  ...
  **370 · 82 · 21 · 11 · 4 · 3 · 0**, lost **5 · 0 · 5 · 0 · 2 · 0 · 0** — the
  ...
  the identity below is for — and `restoredByKind` swampPave 2 · reflow 6.
  **Round 24 moved no board, so every digit above is unchanged — and it was
  re-run anyway, because "the boards did not move" is a reason to EXPECT the same
  digits and not a substitute for reading them.** The
...
Where the fleet stands after round 26 (172 rooms, the world this doc is now
...
**Round 26 moves NOTHING either, and its BLOCKING finding was this document's
own previous BLOCKING recurring inside the fix that closed it.** One BLOCKING
(the film's seat caption, wrong on 210 of 436 tiles, gated by a comparison that
could not see it), one MAJOR (the withheld anchor, a free parameter with a new
name), three MEDIUM, two MINOR and three LOW. **Unchanged in round 26: every
physical figure in this block** — ramparts 8208 · roads 14,100 and every room's
own road COUNT · extensions 60/60 in 172/172 (10,320) · declarations 300 in 157
rooms · `declared-shortfall 122` · shallow extensions 25 in the same three rooms
· planner notes 236 == `noteRecords` 236 in 119 rooms · the along-cut roster 7
rooms / 16 tiles, `alongCutMoved` 11, the refusals 23 at 9/10/4 · `spur` 370 /
`swampPave` 82 / `reflow` 21 / `alongCutMoved` 11 / `stitch` 4 /
`conductBridge` 3 · the prune census 2014/2013/2001/12/1 · road+rampart
`274 = 231 + 30 + 13` across 153 rooms · roads median 81 · the 117 unjudged
mobility chips, re-derived as 115 + 2 · the enclosure contract's 688 records, `exposed 0`
fleet-wide, withheld 2,786 over **797 distinct tiles** · `validate.mjs` at
`pass 172/172 · fail 0 · declared-shortfall 122` with every physical total 0.
**Moved in round 26: nothing on any board, and five things in `meta`, the films
and the harness** — `meta.shell.cutDrift` in all 172 rooms, one row per tile
either layer-7 pass moved on or off the cut (`{x, y, op, pass, why}`, 34 adds
all `layer7-reconcileSeal` + 46 removes all `layer7-inertPrune` across 29 rooms,
`[]` in the other 143), which makes `cutAtFreeze + adds − removes == the shipped
cut` an arithmetic identity in **172/172** · `meta.exteriorContractBasis` from
**1 distinct string across 172 records to 166**, carrying each consumer's own
withheld roster · the film's seat facet split on the BOARD rather than on
`shell.bubble`, so the fleet's 8,208 now read **7,234 crossing · 347 seat
outside the shell · 89 seat inside · 213 stand-denial ring · 325 personal cover
· 0 unclassified · 0 none** (was 295/141 on the two seat facets, and 210 of the
436 were on the wrong side) with `emptyBecause` derived per room — **555 empty
rows, 555 reasons, 41 distinct strings** where round 25 shipped 494 rows over 7
constants · the numeral audit 183/122/61 → **192 claims over 5 recognised shapes
...
**1300**. r29p6: reserved tiles are this room's walkable interior (cheb≤2 of
the reserved set or the cut). r29p7: criticism 88 — a last discarded rung
...
over it reaches no owned core structure (0 of the first sixty rooms' 2,524
frozen cut tiles can be dropped with the core still sealed, while the SHIPPED
cut seals only 170/172, so it is not a free property), every freeze-only tile is
`inertPruned` and unramparted **46/46**, and the drift log REPLAYS with every
move required to be a real move at the moment it is applied.
**Round 25 moves NOTHING either, and it is the first round whose entire finding
list lives in what the artifact SAYS rather than in what it contains.** One
BLOCKING (a film taxonomy with a class that could never be assigned), one MAJOR
(a source guard with three ways past it), five MEDIUM and the rest sentences and
witnesses. **Unchanged in round 25: every physical figure in this block** —
ramparts 8208 · roads 14,100 and every room's own road COUNT · extensions 60/60
in 172/172 (10,320) · declarations 300 in 157 rooms · `declared-shortfall 122` ·
shallow extensions 25 in the same three rooms · planner notes 236 ==
`noteRecords` 236 in 119 rooms · the along-cut roster 7 rooms / 16 tiles,
`alongCutMoved` 11, the refusals 23 at 9/10/4 · `spur` 370 / `swampPave` 82 /
`reflow` 21 / `alongCutMoved` 11 / `stitch` 4 / `conductBridge` 3 · the prune
census 2014/2013/2001/12/1 · road+rampart `274 = 231 + 30 + 13` across 153 rooms
· roads median 81 · 117 unjudged rooms and 55 over the gated target ·
`validate.mjs` at `pass 172/172 · fail 0 · declared-shortfall 122` with every
physical total 0. **Moved in round 25: nothing on any board, and five things in
`meta`, the films and the harness** — `withheldTiles` on all 688
`exteriorContract` records with `meta.exteriorContractBasis` beside them and
`meta.shell.cutAtFreeze` in all 172 rooms, which together turn a hand-carried
`2,786` into a roster whose union is **797 distinct tiles** and whose first
consumer re-derives from terrain in 172/172 · `preTakeShortfallBasis` from **1
distinct string across 134 records to 24** · every film gaining a top-level
`rampartCensus` over a fixed seven-facet order, empty facets included WITH an
`emptyBecause`, so that the fleet's 8,208 read, ON THAT BUILD, **7,234 crossing ·
295 bubble seat · 141 inside seat · 213 stand-denial ring · 325 personal cover ·
0 unclassified · 0 none** and the film's class equalled the note's class tile by
tile in 172/172. **THE TWO SEAT FIGURES IN THAT LINE WERE THE DEFECT AND ARE
RETIRED**: the split was `shell.bubble` membership, which is a disjunction and
not a side of the wall, so 210 of the 436 seats were captioned wrong and the
class-level gate beside them could not see it — the live census is
`seat.outside 347 / seat.inside 89` in the round-26 block above, criticism 129
· the numeral audit 173/114/59 → **183 claims over 5 recognised shapes
/ 122 re-derived / 61 waived at 37 distinct sites / 0 unowned / 0 WRONG**, exit 0
· and the mutation suite 1168 → **1200**. **And two derived fleet lines the
validator did not have**, both printed rather than transcribed, and both of them
retiring a figure that had been typed into `validate.mjs` and left there — the
first for three rounds, inside a message a reader SEES:
*"along-cut refusals: 23 across the fleet · 9 breaks-network + 10 no-parallel + 4
seat — re-derived from each room's own board (a container on the run tile makes
it a seat refusal; the sentence's own network clause makes it breaks-network),
never read off the `kind` field being checked"*, and
*"negotiated noWalls detour: 16 of 57 record(s) negative · floor -9 (E9S9) —
lifting the interior's natural walls as well as our own mass can make the
defender's route shorter than the attacker's, and this is how often and how far
it does on the artifact in front of you"*.
**Round 24 moves NOTHING — the first zero-board round since round 15, and the
first one this campaign has bought rather than been given.** Two BLOCKING
findings, both in the board channel, both fixed, and 0 of 172 rooms differ by a
tile: the stale exterior flood re-pointed at every consumer that runs after a
rampart can move (criticism 114) and the along-cut arithmetic's witness
exemption killed outright (criticism 113). **Unchanged in round 24: every
physical figure in this block** — ramparts 8208 · roads 14,100 and every room's
own road COUNT · extensions 60/60 in 172/172 · declarations 300 in 157 rooms ·
`declared-shortfall 122` · shallow extensions 25 in the same three rooms ·
planner notes 236 == `noteRecords` 236 in 119 rooms · the along-cut run roster
7 rooms / 16 tiles, `alongCutMoved` 11 with 11/11 inside the shipped wall, the
refusals 23 at 9/10/4 · `spur` 370 / `swampPave` 82 / `reflow` 21 / `stitch` 4 /
`conductBridge` 3 · the prune census 2014/2013/2001/12/1 · road+rampart
`274 = 231 + 30 + 13` across 153 rooms, median 2 / max 5 · roads median 81 ·
`validate.mjs` at `pass 172/172 · fail 0 · declared-shortfall 122` with every
physical total 0. **Moved in round 24: nothing on any board, and four things in
`meta` and the harness** — `meta.exteriorContract` published in all 172 rooms
(688 records, four consumers in run order, **exposed 0 fleet-wide**, withheld
2,786) with a new `gate:"exterior"` declaration class that fires in 0 rooms and
is gated in BOTH directions · `metric.maxDetour` and `metric.coveredPairs` added
to the 57 mobility declarations, which is what makes the unjudged branch
derivable from the record · `meta.shortfalls[].detail` re-rendered in exactly 2
rooms (E17S3, E7S9 — the only two whose mobility gate judged no pair, and the
two that had been shipping a pass verdict over it) · the numeral audit
148/103/45 → **173 claims over 5 recognised shapes / 114 re-derived / 59 waived
at 35 distinct sites / 0 unowned / 0 WRONG**, over a scope that now reads
**1,482 numeral+noun occurrences scanned, 119 parsed, 1,363 NOT PARSED** and a
registry that self-checks at **21 of 22 extractors twinned, 0 disagreeing**
(`roadContainerTile` is the one single-sourced, at 62 tiles across 55 rooms) ·
and the mutation suite 1124 → **1168**. **And two derived fleet lines the
validator did not have**, both printed rather than transcribed: *"layer-7 roads
outside the shipped wall: 3 across the fleet — every one of them a kind with a
stated reason to cross the wall (swampPave, conductBridge); any other layer-7
kind out there fails its room"*, and *"along-cut swap offers: 14 priced across
the fleet · 13 on the road axis (`offNetwork`), 1 on the container/extension
axes · 4 offered a parallel this room's own swap later took (re-derived on the
reconstructed offer-time board) · 14/14 reproduce exactly"*. The second one
retires a figure six code sites had been typing by hand.
A from-layer-1
...
**274 = 231 crossings + 30 container seats + 13 stand-denial ring + 0 personal
cover + 0 unclassified** (median 2, max 5; `unclassified` printed whether or not it is
...
6. **The RCL BUILD STAGING is a separate contract from the plan, and it had gaps.**
   ...
   **AND ONE OWNER PREFERENCE ABOUT THIS STAGING HAD NO HOME IN THIS DOCUMENT AT
   ALL — SO IT IS SCOPED OUT HERE, IN WORDS, RATHER THAN LEFT TO LOOK
   ADDRESSED.** "Walls timed to safe-mode" is a standing owner preference and
   `safe.?mode` matches **nowhere** in this document, nowhere in
   `tools/plan-suite/v2/*.mjs` and nowhere in `tools/server/push-plan.mjs`; a
   round-24 reviewer went looking and found neither an implementation nor a
   refusal, which is the worse of the two states a preference can be in. **It is
   OUT OF SCOPE for this goal, and the reason is a boundary this document already
   draws everywhere else**: the planner ships the FULL wall in the plan — every
   rampart the min-cut needs, priced and declared — and WHEN a rampart gets built
   is the bot's build order, which lives in `src/` and is already RCL-staged by
   the rungs above (`CONTAINER_RCL`, `EXTRACTOR_RCL`, `ARTERIAL_RCL`). A planner
   that withheld ramparts from the plan to time them against safe-mode would be
   the silent-capping anti-pattern with a schedule attached, and a planner that
   published a rampart RCL rung would be publishing a runtime policy it cannot
   measure. The preference is real and it belongs to the runtime campaign; it is
   named here so the next reviewer finds a ruling instead of a silence, and so
   that nobody reads the absence of a safe-mode rung as an oversight. Filed as the
   LOW half of round 24's ledger and closed by scoping, not by building.
...
12. **STILL OPEN: THREE ROOMS SHIP 25 SHALLOW EXTENSIONS BETWEEN THEM** —
   ...
   **THE SENTENCE IS FIXED IN ROUND 25 AND IT TOOK THIRTEEN ROUNDS, WHICH IS THE
   PART WORTH RECORDING.** `sharedTarget` was published as a FIELD and the prose a
   reader meets never said the six targets were one tile: the declaration
   presented six independent lap-bound refusals, `22,7` appeared seven times in
   its own text, and a reader who counted them had to notice by themselves. The
   renderer now states it in words — and states it only where it is DERIVABLY
   true, from the record's own `slots` (every sharing slot offered exactly one
   target), which today is E12S6 and no other room. A field nobody reads is not a
   disclosure, and this entry had been quoting its own field as if it were one.
...
85. **MEDIUM (READER CHANNEL): 117 OF 172 GALLERY CARDS PAINTED "NOT MEASURED" IN
   ...
   **AND THE THIRD CHANNEL WENT ON STATING A FALSE REASON FOR THAT SAME ZERO FOR
   FOUR ROUNDS, IN THE TWO ROOMS THIS ENTRY ALREADY NAMES.** The label branches
   in the chip and on the room page because `mobilityUnjudgedWhy` exists; the
   FILM's caption did not call it. `animNotes` hardcoded *"as-built gated lap 0 —
   no pair of wall tiles detours more than 4 tiles, so no pair was judged"*, which
   is false in exactly E7S5 (worst pair 35 in / 2 out — a **33-tile** detour, the
   worst in the fleet, `over: 3`) and E6S3 (**5** tiles, `over: 26`) — the two
   rooms two paragraphs up, whose whole reason for existing here is that the
   obvious label is false in them. So one room shipped three reader channels and
   two different answers, with the film asserting the one the other two were
   written to avoid. `animNotes` calls the helper now, and the round went one step
   further than the finding asked: the wording lived in two places (the helper and
   a new declaration branch), so it is ONE exported `unjudgedReason` with three
   call sites in `plan.mjs` — film, page, chip — and the declaration reading the
   same function. The gate is a SOURCE guard rather than an artifact one and is
   named as such at criticism 119. Filed as **criticism 117**.
   **AND THE GUARD THAT WAS SUPPOSED TO HOLD THIS SHUT HAD THREE WAYS PAST IT AND
   ASKED ABOUT 2 ROOMS OF 117.** A token grep over `plan.mjs` could be defeated by
   rewording past its one literal regex, by a column-0 `}` inside a template
   literal truncating its read window, or by a comment mentioning the helper's
   name to keep its call COUNT up while the real call was deleted — all three
   re-shipped a false caption to E6S3 and E7S5 at `pass 172/172`. The three
   channels are compared as OUTPUT now, on every unjudged room rather than on the
   two the old guard could see. Criticism 121.
...
94. **STILL OPEN, AND THE FINDING IS THE SCOPE AND NOT THE NUMERALS:
   ...
   **`tools/plan-suite/v2/numeral-audit.mjs`** reads every `.mjs` under
   ...
   every fleet-level total the artifact has. It read **148 claims over 5
   recognised claim shapes · 103 re-derived and correct · 45 waived at 27 distinct
   sites · 0 unowned · 0 WRONG · 0 in files not yet swept** on the round it was
   audited (**173 · 114 · 59 at 35 sites · 0 · 0** today — every one of those five
   numbers moved when round 24 taught the scanner to read a wrapped line, and the
   census is at the foot of this entry rather than restated here), exit 0; it runs as the last
   ...
   **1,375 numeral+noun occurrences in the audited prose, 96 read by the library
   and 1,279 NOT PARSED — unchecked rather than clean** (all three moved in round
   24, to 1,482 / 119 / 1,363, when the scanner learned to join a wrapped claim),
   with the frequent
   ...
   apart).
   **AND THE SECOND ROUND OF ITS LIFE THE HARNESS WAS BLIND TO WHERE THIS
   SUITE ACTUALLY WRITES PROSE — THE LINE BREAK.** Four more defects, all found
   by round 24's mechanical reviewer, all of them the gate's own scanner rather
   than anybody's numeral; the write-up is at criticism 115. The scanner treated
   each `//` line as its own prose range and could not cross a `' * '` inside a
   block comment, so **any claim that wraps was invisible** — and this suite wraps
   at 80 columns, which means the blind spot was not an edge case but the house
   style. De-wrapping alone, with no digit touched, turned up **two LIVE wrong
   figures**: `validate.mjs`'s *"one in 60 tiles across 53 shipped rooms"* (the
   fleet ships **62 across 55**) with a rider claiming *"push-plan.mjs says the
   same thing"* — doubly false, because push-plan documents 60/53 AS the rot —
   and `layer-walls.mjs`'s *"the fleet's 14288 road tiles"* against a fleet of
   **14,100**. Both are retired: the first is de-numeralised and pointed at
   `push-plan --census`, the second is quoted as the rotted figure it is, with a
   waiver saying so. The registry's self-test was the second hole and it was a
   one-directional one — it caught an extractor reading 0 or undefined and
   nothing else, so re-pointing `cut tiles` at `meta.shell.battlements` (5,823
   against 7,234) or the road TOTAL at ramparts (8,208 against 14,100) passed
   BYTE-IDENTICALLY, exit 0. Every extractor with an independent second
   derivation is now compared against its twin and a disagreement exits 1:
   **21 of 22 twinned, 0 disagreeing**, with the untwinned one printed as
   SINGLE-SOURCED rather than left to look checked. Third, the gate was not
   CRLF-safe — a fresh `git checkout` on this repo's `autocrlf` setting flipped it
   to exit 1 with 1 WRONG, a dead waiver and 2 unowned claims and **not one
   numeral changed**, i.e. a clean clone would have failed the build; line endings
   are normalised before the scan and there is a test for it. Fourth, a waiver
   still bound too widely in the other direction: a tag written about an UNPARSED
   numeral waived the nearest PARSED claim, and `// ----` divider rules were not
   paragraph breaks, so 9 of the 27 tags could reach 11–20 lines. A waiver now
   binds only to a claim its own text substring-references, and dividers break
   paragraphs. The census after all four: **173 claims · 114 re-derived · 59
   waived at 35 sites · 0 unowned · 0 WRONG · 0 dead waivers**, over
   **1,482 occurrences scanned, 119 parsed, 1,363 NOT PARSED**. The waived line
   also prints `h.measured` beside every waiver an extractor can resolve, which it
   had been computing and discarding — the two fully-resolvable waived sites were
   unfalsifiable on sight precisely where they were the most falsifiable.
...
- **MEDIUM (READER CHANNEL): every rampart in every film was captioned
  ...
  **THAT SPLIT IS RETIRED, AND THE `0 stand-denial` IN IT WAS THE FINDING NOBODY
  READ FOR EIGHT ROUNDS.** This bullet published a census with a dead class in it
  and treated the zero as a reading; it was a construction — `bubble` was tested
  before `denial` and 223 of the fleet's 237 stand-denial tiles are also bubbles,
  so the class could never have been assigned once. The 807 was three occupant
  classes wearing one caption that names a container (**295 container · 299 link ·
  213 carrying nothing at all**). Under the note channel's own order the same
  8,208 read **7,234 crossing + 295 bubble seat + 141 inside seat + 213
  stand-denial ring + 325 personal cover + 0 unclassified**. Criticism 120.
  **AND THE SECOND AND THIRD TERMS OF THAT REPLACEMENT WERE THEMSELVES WRONG FOR
  A ROUND, WHICH IS WHY THIS PARAGRAPH NOW HAS THREE LAYERS OF RETIRED SPLIT IN
  IT.** The `bubble seat` / `inside seat` boundary was `shell.bubble`
  membership — a DISJUNCTION (outside the shell **or** inside it below safe
  depth), so the caption's "outside the shell" was false on 79 of them and its
  "inside the wall" was false on 131 more. The census the films ship today
  derives the disjunct from the frozen-cut flood per tile and reads **7,234
  crossing + 347 seat outside the shell + 89 seat inside + 213 stand-denial ring
  + 325 personal cover + 0 unclassified + 0 none = 8,208**. Criticism 129.
...
105. **STILL OPEN, AND IT IS THE NEW HARNESS'S OWN BLIND SPOT: `numeral-audit.mjs`
   ...
   148 claims, 103 re-derived, 45 waived at 27 sites, 0 unowned, 0 wrong when this
   entry was written, **183 / 122 / 61 at 37 sites / 0 / 0 today** — and the
   standalone
   ...
   **STILL OPEN AFTER ROUND 24, AND THE ROUND PAID FOR IT IN A NEW CURRENCY.**
   The harness was widened again — wrapped claims joined before matching, twinned
   extractors, CRLF normalisation, waivers bound to text they quote (criticism
   115) — and it still reads `.mjs` and only `.mjs`. What round 24 found in THIS
   file was not a stale digit: it was **two false statements of impossibility**,
   one in criticism 108 (*"the offer-time board cannot be reached"* — it is
   `shipped ∪ {from2} \ {t}`, and `from2` is unique in every room that has one)
   and one in residue 112(b) (*"the re-measurement is a committed one-liner"* —
   there was no such instrument, and the audit listed all six sites carrying the
   figure as UNPARSED). A numeral harness would have caught neither, because
   neither has a numeral wrong in it. That is the entry's own scope finding
   sharpened rather than repeated: the class this document rots into is not only
   figures, it is CLAIMS ABOUT WHAT CANNOT BE DONE, and those are checkable only
   by somebody trying. Criticism 104(a) was the first, 108 is the second, and both
   were written into the paragraph that had just retired the previous one.
   **STILL OPEN AFTER ROUND 25 — AND FOR THE FIRST TIME THE LIVE-ROT HALF OF THE
   CLAIM HAS BEEN MEASURED, AND IT CAME BACK CLEAN.** Round 25's mechanical
   reviewer did by hand what this entry says nothing does automatically: ran
   `numeral-audit.mjs`'s own `PATTERNS` over THIS FILE with a markdown-aware,
   paragraph-preserving wrap-join, and re-derived every claim they matched against
   `plans-hub.json`. **144 claims re-derive CORRECT; not one live wrong figure on
   the readable shapes.** Every apparent refutation triaged into one of three
   deliberate kinds — a figure the sentence itself names as retired (*"241 of the
   fleet's 286 road+rampart tiles … the 159-room world this document retired six
   rounds ago"*), a dated round-N accounting block, or an N/N harness count that
   is not a fleet denominator — and a hand spot-check of fifteen of the load
   bearing ones (117 unjudged · 55 over target · 976 candidates · 63 records ·
   `allRefused` 4 · `otherMoved` 26 across 15 · the prune census 2014/2013/2001/12/1
   · `274 = 231 + 30 + 13` across 153, median 2 max 5 · `reflow` 81 ·
   `sealCritical` 7191 · sealed floor 111 in 58 · `roadContainerTile` 62 across 55
   · the roadKind census · roads median 81 · notes 236 == `noteRecords` 236 in 119
   · declarations 300 in 157) was EXACT. **This does not close the entry and the
   distinction is the whole point of it.** What was measured is that the file is
   not rotten TODAY; what stays open is that nothing in the build says so
   tomorrow, and a one-off sweep by a reviewer who happened to be thorough is
   precisely the instrument this entry was filed to stop relying on. The tagging
   problem for prose a human reads is still unsolved, and the two round-24 finds
   in this file were false statements of IMPOSSIBILITY that no numeral harness
   would catch in any case — round 25 added a third of that species and it is
   recorded at criticism 123, where a residue named an instrument's coverage in
   rooms and the artifact turned out to disagree by a factor of four. **The
   honest summary is that this entry's headline claim is now half-measured and
   half-open, and it is filed that way rather than downgraded on the strength of
   one clean sweep.**
   **STILL OPEN AFTER ROUND 26, AND THE ROUND FOUND A WHOLE SCRIPT THE GATE HAD
   NEVER READ.** Every one of the five claim shapes required DIGITS, so a fleet
   figure spelled out in words was invisible — and three live wrong ones were
   sitting in `.mjs` files the harness DOES sweep, each saying *"seven"* where
   the fleet says 29. The vocabulary is read now and the residue is printed every
   run (95 spelled-out occurrences at the ten-and-over threshold, 0 of them
   currently owned by a shape), which is criticism 133. What does NOT change is
   this entry's own scope: the harness still reads `.mjs` and only `.mjs`, so the
   file you are reading is swept by whoever is reading it.
...
106. **BLOCKING (BOARD), AND IT SHIPPED INSIDE THE FIX THAT WAS SUPPOSED TO
   ...
   **E9S8 is the one that costs**: an attacker walks 30 tiles from the room edge to
   ...
   `conductBridge` E5S1 `28,30` — an OBSERVATION when this entry was written, and
   nothing enforced it for the other five layer-7 kinds; criticism 114 makes it an
   invariant the validator derives and prints). Two refusal-priced candidates carrying the same
...
108. **MAJOR: THE ALONG-CUT REFUSAL'S ARITHMETIC WAS FREE TEXT GATED ON ITS SIGN,
   ...
   **ROUND-24 ADDENDUM, AND IT IS THE WORST SHAPE ON THIS LIST: THE PARAGRAPH
   THAT RETIRED ONE FALSE IMPOSSIBILITY WROTE A NEW ONE INTO ITSELF.** The
   struck sentences above are that new one. The exception was granted on BOARD
   STATE — target paved and tagged `alongCutMoved` — and the checker then
   returned before any arithmetic ran, so "any other occupant fails" is true and
   irrelevant: an editor who points a fabricated refusal at the room's own real
   take satisfies the exception and is thereafter unbounded. The proof is a
   round-24 mutant that shipped `pass 172/172 · fail 0`: *"999 more road tile(s)
   fall off the network (5 → 3; newly off: 99,99)"* — a magnitude that is not the
   subtraction printed beside it, readings that IMPROVE the axis the sentence
   claims to worsen, a baseline the room does not publish, and a tile off the
   50×50 board. **And the exception was never necessary.** The offer-time board is
   `shipped ∪ {from2} \ {t}`, where `from2` is the take's origin and is UNIQUE per
   room, so all four witness offers re-derive EXACTLY — magnitude, both readings,
   roster string, elision — and the class is gone rather than tightened. So this
   entry did to itself, in one round, precisely what it had criticised 104(a) for:
   an impossibility written from the outside of a derivation nobody had attempted.
   It is recorded here rather than deleted for the same reason 104(a) was.
   **AND THE CENSUS IN THE STRUCK CLAUSE WAS STALE ON ITS OWN ARTIFACT BEFORE
   EITHER REVIEWER TOUCHED IT** — criticism 106's fix created a fourth
   taken-parallel case (E9S8 `19,24 → 20,25`) in the round that wrote "3", and six
   code sites plus this sentence carried the old denominator, one of them inside a
   FAILURE MESSAGE. Two reviewers reported two different rosters, 13/10/3 and
   14/10/4, and **both were right**: the disagreement was a definition, not a
   measurement. It is settled by defining the census — an OFFER is one *"moving it
   to X,Y — <price>"* segment of one `alongCutRefused` entry, counted in BOTH
   loops — deriving it, and printing ONE figure, which `validate.mjs` now does:
   **14 offers priced across the fleet · 13 on the road axis (`offNetwork`) · 1 on
   the container/extension axes · 4 offered a parallel this room's own swap later
   took · 14/14 reproduce exactly**. The six hand-typed sites are derived or
   stripped, the shape is in the numeral-audit registry, and the residue that used
   to sit at 112(b) is gone with them.
...
112. **STILL OPEN, AND ALL OF IT WAS FILED BY THE CLUSTERS AGAINST THEIR OWN WORK
   ...
   **CLOSED IN ROUND 24, AND NOT BY THE HALF THAT LOOKED CLOSEABLE.** (a) is
   still the honest kind and round 24 added one more of the same species — the OM4
   channel-agreement gate is a source guard, named at criticism 119 — so the
   population grew rather than shrank. What closed is (b), and it closed by being
   WRONG on all three of its clauses.
   ...
   dated figure without one is criticism 94.~~ **RETIRED — the figure was already
   stale when this residue was written, the site count undercounted it, and the
   named instrument did not exist.** The swap set HAD moved: criticism 106's own
   fix created a fourth taken-parallel case in the round that typed "3". The
   figure appears at **six** sites, not four — five in `validate.mjs`, one of them
   inside a `bad.push` FAILURE MESSAGE, and one in `mutate.mjs` — and
   `numeral-audit --list` showed every one of them as UNPARSED, i.e. unchecked and
   unwaived. And *"the re-measurement is a committed one-liner"* names nothing,
   because there was nothing to name: **a dated figure with no instrument is
   criticism 94, which is exactly what this clause was written to deny.** The
   figure is derived and printed by `validate.mjs` now (14 · 13 · 1 · 4 · 14/14),
   the six sites are derived or stripped, and the shape is in the numeral-audit
   registry so it cannot go back to being unparsed. A residue that describes its
   own instrument inaccurately is worse than an open one, which is the same
   sentence 104(c) earned a round earlier — the second time in two rounds that
   this list's own bookkeeping was the defect.
   ...
   carries the count — **and round 24 added two more of a kind no numeral harness
   could ever catch, because neither has a wrong number in it**: criticism 108's
   claim that a board could not be reconstructed, and the clause struck above,
   which named an instrument that did not exist.

Round-24 findings. **Two fresh reviewers, 13 confirmed findings between them** —
the owner-voice reviewer **2 BLOCKING**, 3 MEDIUM and 2 LOW with **two exploits
landed**; the mechanical reviewer 0 CRITICAL, **3 MAJOR**, 3 MINOR and 1 LOW with
**four more** — a severity split its own report states two ways over the same six
lettered findings (its header counts 2 MAJOR where its body labels 3, and files
the waiver-scoping item at MINOR in one write-up and LOW in the other), recorded
here rather than reconciled by picking, because a ledger that quietly rounds its
own inputs is the class this document opens with. One of the MAJORs is the first
BLOCKING found from the other side and strengthened on the way, so the thirteen
findings are **twelve distinct defects**. **Both reviewers' whole-fleet re-derivations were clean before either
found anything, and that list is again what makes the findings small**: a
from-mongo re-composition of all 172 rooms with **0 structure-set differences**
against the shipped artifact; an independent hard-gate sweep at 0 failures on
every class this document gates (0 seal leaks, 0 shallow eco without a personal
rampart, 0 towers under depth 4, 0 extensions without a D4 road face, 0 orphan
roads, 0 off-network structures, 0 illegal stacks, 0 structures on a room
object, 0 minerals entombed, controllers 172/172 and sources 344/344 inside);
`sealCritical` **7191** re-derived exactly, with the cut's extra tiles **43
across 19 rooms** matching `redundantCut.reasons` in SET-EQUALITY per room;
the sealed interior floor re-flooded to **111 tiles across 58 rooms, 0 per-room
differences**; the road+rampart taxonomy re-classified from scratch to a
**byte-identical** `{274, 231, 30, 13, 0, 0}`; the owner rule that the
first-built tower must be the easiest to refill re-walked with engine costs at
**0 violations in 172 rooms** (and `refillDists` is in TOWER order, ascending in
only 60 of 172, so it is a real test); **all 162 refusal rejection clauses**
re-derived TRUE on the shipped board; and the film channel replayed at planHash
172/172 with the roadKind badges matching the laid set exactly. **The mutation
harness went 1124 → 1168, all biting, +44 cases**; `validate.mjs` ends on
`pass 172/172 · fail 0 · declared-shortfall 122` with every physical total 0, and
`numeral-audit.mjs` on `173 claims · 114 re-derived · 59 waived at 35 sites · 0
unowned · 0 WRONG`, exit 0. **And the strengthening is measured rather than
asserted in the sharpest way this campaign has managed yet**: the six exploits
the reviewers landed were landed on copies of the ROUND-23 artifact, they are
kept, and every one of them now fails — `m5b` and `m5e`, which escaped at
`172/172`, are caught at `171/172` naming the sign, the subtraction, the
baseline, the roster and the off-board tile.
**Three themes, and the first two are the round.**
First: **an exception that bounds nothing is worse than no gate, and this one was
not even necessary.** Criticism 108 closed the along-cut arithmetic and wrote
itself a witness-class door for the offers it believed unreachable; the door was
granted on board state and the checker returned before looking, so a refusal
could claim anything behind it. Both reviewers walked through it, from opposite
sides, in the same round — and the mechanical reviewer then demonstrated that the
offer-time board was reconstructible all along. **A paragraph that retires a
false impossibility and writes a new one into itself is the shape this round
exists to record**, and it is recorded at 108 rather than tidied.
Second: **round 23 fixed the instance and this document called it the class.**
The stale exterior flood was re-pointed at one pass; the predicate that decides
where every layer-7 road may go was still reading layer 2's answer, one stage
over, in the same file, written by the same round. It cost nothing this time —
0 of 172 boards move — and the reason it cost nothing is terrain, which is why
the fix is a published contract rather than a second one-line repair.
Third: **the harness's own blind spots, and they were where it writes rather than
where it reads.** A numeral gate that could not see a claim spanning two lines was
blind to the house style of the suite it audits; a self-test that only caught an
extractor returning zero certified extractors reading the wrong field entirely; a
gate that was not CRLF-safe would have failed a fresh clone with no numeral
changed; and a waiver could still bind to a claim it never mentions.
**Six entries carry the round — 113–118 — because the numeral harness's four
defects and the waiver-printing LOW are one entry and one program**, and three
further findings are written where the thing they correct lives rather than given
a number of their own. **Entry 119 is not a finding**: it is round 24's own
residue list, filed by the clusters against their own work, and both of its live
items are gates that cannot be tripped from the artifact side.
- **MEDIUM: the along-cut offers roster was stale on the artifact it was dated
  to, and no instrument existed to notice.** Truth 14/10/4 against a shipped
  "13 offers, 3 of them witness", the fourth created by criticism 106's OWN fix;
  six code sites carried the old denominator, one inside a failure message, and
  `numeral-audit --list` had all six as UNPARSED. Written into criticism 108's
  addendum with the definition that settles it and the fleet line that now prints
  it, because that is where the figure it corrects lives.
- **LOW: an owner preference with no home.** "Walls timed to safe-mode" matched
  nowhere in this document, the planner or push-plan. It is SCOPED OUT in the
  staging bullet, in words, with the boundary stated: the planner ships the full
  wall, the build order is the bot's, and a planner that withheld ramparts to
  time them would be silent capping with a schedule attached.
- **LOW: the numeral audit's waived lines never printed what the gate had already
  measured.** Two of the resolvable sites claimed "28 shallow extensions across 5
  rooms" over a fleet shipping 25 across 3 — legitimately waived, both sentences
  self-date — but rendered unfalsifiable on sight precisely where they were most
  falsifiable. `h.measured` prints beside every waiver an extractor resolves;
  folded into criticism 115 with the rest of the harness work.
113. **BLOCKING + MAJOR, FOUND FROM BOTH SIDES IN ONE ROUND: THE TAKEN-PARALLEL
   WITNESS CLASS BOUGHT UNLIMITED FREEDOM ON THE PRICE, AND THE EXEMPTION WAS
   UNNECESSARY.** `netPriceCheck9` bare-`return`ed on a witness target: the
   exception was granted on BOARD STATE (the offered tile is paved and its
   `roadKind` is this room's own `alongCutMoved` take) and nothing then bounded
   what the refusal could CLAIM. Worse, it was two gaps stacked — the round-22
   well-formedness checks (sign, `n == b − a`, `a == baseline[axis]`, roster
   length against the difference, elision, on-board tiles) lived INSIDE the
   shipped-run-roster loop, and the second loop, "the refusals whose run the take
   broke", called only the price check. **A room with an empty run roster files
   every refusal into that second loop**, so E17S5 got an appended
   `breaks-network` refusal at `41,35` — a D8 neighbour of its own take, so every
   structural gate is satisfied — offering `42,35`, the room's own take, priced
   *"999 more road tile(s) fall off the network (5 → 3; newly off: 99,99)"*: not a
   subtraction, sign-improving, a baseline the room does not publish, a tile off
   the 50×50 board. **`pass 172/172 · fail 0 · declared-shortfall 122`.** The
   control with a `spur`-provenance parallel bit at 171/172, which is what pins
   the escape to the exception rather than to the siting. Four more landed from
   the mechanical side against the witness offers themselves — the `49,49` roster
   criticism 108 had just retired, an invented roster tile for tile, a magnitude
   inflated with a roster to match, and a magnitude DEFLATED so the kept
   anti-pattern looks cheap. **The fix is that the exemption is dead, not
   narrowed.** `offerBoard9` reconstructs the offer-time board as
   `shipped ∪ {from2} \ {t}`, where `from2` is the take's origin and is derived as
   the UNIQUE D8 rampart-neighbour of the target that carries no road today and
   yields a board on which this offer is measurably worse — **0 or >1 candidates
   is a FAILURE**, because a reconstruction that has to choose is not a
   reconstruction. `auditOffer9` runs shape, well-formedness and full
   re-derivation on EVERY priced offer of EVERY refusal in EVERY loop, and the
   container/extension-axis prices that loop 1 used to reject and loop 2 used to
   ignore are accepted and re-derived on all four axes. All six exploits die; the
   controls keep biting. **And the census that had been typed by hand for a round
   is settled BY DEFINITION rather than by picking a reviewer**: an OFFER is one
   *"moving it to X,Y — <price>"* segment of one `alongCutRefused` entry, counted
   in both loops, so the owner's 14/10/4 (all axes) and the mechanical reviewer's
   13/10/3 (road axis, three unre-derivable) were BOTH right, and the fleet line
   `validate.mjs` prints is **14 priced · 13 road-axis · 1 container-axis · 4
   taken-parallel · 14/14 reproduce exactly**. The 14th is E9S8 `19,24 → 20,25`,
   created by criticism 106's own fix, and the "unre-derivable" residue is gone.
   27 mutations, every one of them in a class no mutation could reach a round ago
   because the checker returned before it looked.
114. **BLOCKING (BOARD-CLASS, ZERO BOARD EFFECT): ROUND 23 FIXED THE STALE FLOOD
   AT THE INSTANCE, AND THE PREDICATE THAT DECIDES WHERE EVERY LAYER-7 ROAD MAY GO
   WAS STILL READING LAYER 2'S ANSWER.** `paveable()` took `const ext =
   plan.exterior` — assigned once in `pipeline.mjs`'s shell hand-off, from layer
   2, and never refreshed — while the rampart set the pass is standing on was
   already in scope a few lines away, and it is the sole
   expansion predicate of `fieldFromNetwork`, consumed by three board-writing
   stages: the stitch, the rampart spurs and the ext-face net. Nothing gated a
   spur, stitch or extFace road against the shipped wall; round 23's gate was
   scoped to `roadKind === "alongCutMoved"` and said so in its own comment.
   Instrumented at the predicate over all 172 rooms the two answers disagree on
   **35 tiles across 21 rooms in the dangerous direction** and **392 across 144 in
   the conservative one**; the owner-voice reviewer's independent re-compose,
   counting room verdict flips rather than reachable tiles, read **46/30** and
   **314/146** — a definitional gap between two instruments that agree about the
   defect, and both rosters contain the exact two tiles criticism 106 called
   BLOCKING. **The structural finding is the part that generalises**: nothing
   removes a rampart before layer 7, so layers 3–6 drift one-directionally —
   withhold-only, **0 exposed at every one of them** — and `paveable()` is the only
   consumer running after a rampart-REMOVING pass and therefore the only one whose
   drift can put a road outside the wall. **The board effect today is zero in
   172/172 rooms, and that is luck rather than safety**: 46 of 46 mis-accepted
   tiles are D8 of a shipped rampart, 36 of 46 are D8 of a cut tile — exactly what
   the spur search picks from — 18 of 46 are one step off the live network, and
   the single exposed tile carrying a road at all (E19S8 `34,36`) is `roadLayer`
   1. The fix is the class rather than the site: one `exteriorFlood` in
   `shared.mjs` (`layer-shell`'s copy deleted; `layer-ext` keeps its own because
   roads tunnel natural wall in that one, and it says so), a memoised
   `liveExterior(terrain, plan)` at every late consumer, `invalidateExterior` at
   **all eight rampart-mutation sites** — two of which had been riding on the memo
   key's length term without anyone having checked — and the four consumers that
   keep the frozen flood ON PURPOSE, because they are asking about layer 2's
   min-cut enclosure, now declare it: `meta.exteriorContract`, **688 records over
   172 rooms × four consumers in run order, exposed 0 fleet-wide, withheld
   2,786**, with a `gate:"exterior"` shortfall class behind it and the obligation
   gated in BOTH directions, so a layer that exposes owes a declaration and a
   declaration over a contract that exposed nothing fails. The validator's side is
   the invariant this bullet's own prose had been stating as an observation for a
   round: **every layer-7 road, of every kind, must be inside the shipped wall
   unless its kind is declared-legit**, derived and printed — 3 across the fleet,
   all `swampPave` or `conductBridge`. 10 mutations. The whole isolation build
   moved **0 of 172 boards, net totals delta `{}`** — which is the honest headline
   for a class fix whose instance cost two tiles and a haul lane one round earlier.
   **THIS ENTRY SHIPPED A CONTRACT WHOSE ONLY NUMBER WAS DECORATIVE AND A CLASS
   THAT COULD NOT PASS, AND SAID "GATED IN BOTH DIRECTIONS" OVER BOTH.** `withheld`
   was type-checked and never value-checked — zeroing all 688, inflating one by
   500, setting the fleet to 999999 and even a strictly DECREASING run all passed
   `172/172` — the six `exteriorContract` mutants this entry counts touch every
   other field and not that one, there was no `withheldTiles` and `plan.exterior`
   is stripped from the artifact, so the fleet's 2,786 was unre-derivable by
   anybody. And the `gate:"exterior"` class pushed a failure on any exposure
   whether or not it was declared, so the declared state bought nothing: an
   obligation with no reward is not a gated direction. Both are closed at
   criticisms 123 and 127, with 14 mutations between them, and the correction is
   this entry's own — **it is the third time in four rounds that a paragraph in
   this list described its gate as stronger than its code**, after 108's witness
   door and 112(b)'s absent instrument.
115. **MAJOR ×2 + MINOR ×2 + LOW: THE STALE-FIGURE GATE COULD NOT SEE A CLAIM
   THAT WRAPPED, ITS SELF-TEST ONLY CAUGHT ZEROS, IT WAS NOT CRLF-SAFE, AND A
   WAIVER COULD STILL BIND TO A CLAIM IT NEVER MENTIONS.** Round 23's answer to the
   harness was that a gate must be audited like anything it audits; round 24's is
   that the audit has to be run against the way the code is actually written.
   **(a) Line wrapping.** A `//` run was treated as one prose range per LINE and a
   block comment could not cross `' * '`, so every claim that wraps at this repo's
   80 columns was invisible. De-wrapping alone, no digit touched, produced 1 WRONG
   and 2 OPEN: `validate.mjs`'s *"one in 60 tiles across 53 shipped rooms"*
   against a shipped **62 across 55**, carrying a rider that *"push-plan.mjs says
   the same thing"* which is false in the worst direction — push-plan documents
   60/53 **as the rot** — and `layer-walls.mjs`'s *"the fleet's 14288 road tiles"*
   against **14,100**. **(b) The registry's self-test was one-directional.** It
   caught an extractor returning 0 or undefined and nothing else, so re-pointing
   `cut tiles` at `meta.shell.battlements` (5,823 against 7,234) or the road TOTAL
   at ramparts (8,208 against 14,100) produced BYTE-IDENTICAL output at exit 0.
   Every extractor with an independent second derivation is now checked against
   its twin, a disagreement exits 1, and the census prints **21 of 22 twinned, 0
   disagreeing** with the untwinned one named SINGLE-SOURCED rather than left
   looking checked. **(c) CRLF.** On this repo's `autocrlf` setting a fresh
   checkout flipped the gate to exit 1 — 1 WRONG, a dead waiver, 2 unowned — with
   **not one numeral changed**; green depended on the working tree happening to
   hold LF. Normalised, with a test. **(d) Waiver doors.** A tag written about an
   UNPARSED numeral waived the nearest PARSED claim, and `// ----` dividers were
   not paragraph breaks, so 9 of 27 tags reached 11–20 lines. A waiver binds only
   to a claim its own text substring-references; dividers break paragraphs; and
   every waiver an extractor can resolve now prints what the gate MEASURED beside
   what the prose CLAIMS, which is the LOW that rode along — the two fully
   resolvable waived sites (*"28 shallow extensions across 5 rooms"*, fleet 25
   across 3) were unfalsifiable on sight precisely where they were most
   falsifiable. One more defect fell out while closing (a): the
   `n-across-m-rooms` pattern used `\b([\d,]+)`, which fires between the hyphen
   and the digit of "stage-3", so push-plan's *"57 stage-3 tiles across 18 rooms"*
   was being read as a claim about "3 tiles" — a false READING that only the
   de-wrapping made visible, and the reason the fix is `(?<![\w-])` rather than an
   edit to somebody else's file. The census after all of it: **173 claims · 114
   re-derived · 59 waived at 35 sites · 0 unowned · 0 WRONG · 0 dead waivers**,
   over **1,482 occurrences scanned, 119 parsed, 1,363 NOT PARSED**. The pattern
   the four share is worth the sentence: not one of them was a wrong number. They
   were a scanner that could not see, a test that could not fail, an encoding, and
   a scope — the gate's own machinery, in the round after the gate's own scope.
116. **MEDIUM: A PASS VERDICT ON A LAP NOTHING JUDGED, IN THREE SENTENCES OF ONE
   PARAGRAPH, WITH THE DEFECT QUOTED IN THE COMMENT FOUR LINES ABOVE THE FIRST
   ONE.** `declprose-mobility.mjs` branched on `lap > target` — one hurdle, not the
   two the contract names — so a room whose gate judged ZERO pairs has `maxGated
   0`, fails that test, and lands in the INSIDE-the-target branch. E17S3
   (`gatedPairs 0`, ungated 1.27) and E7S9 (`gatedPairs 0`, ungated 1.67) shipped
   *"the defender lap is 0 over pairs costing more than 4 tiles of detour, INSIDE
   the 1.2 target (ungated over every pair it is 1.27/1.67)"* — a pass verdict
   whose supporting parenthetical is an ABOVE-target number. **The smoking gun is
   that the file already knew**: the comment immediately above quotes this exact
   headline as the defect and names E7S9, and the fix had been applied to the
   paragraph beneath it. Not artifact-falsifiable in the usual way — the renderer
   generates the sentence from the record — so the gate is scoped to the headline
   and requires a pass verdict to hold `gatedPairs > 0`. **Two further copies of
   the identical false verdict were found by sweeping the file rather than by
   waiting to be told**: the mass-share clause (*"THERE IS NO MISS TO ATTRIBUTE:
   the room's gated lap is 0, inside the 1.2 target"*) and the CAUSE clause. All
   three take the three-way branch now, `metric.maxDetour` and
   `metric.coveredPairs` are published on all **57** mobility declarations so the
   branch is derivable from the record rather than from the prose, and the
   remaining "inside the target" strings in that file are per-PAIR readings, which
   is stated as the result of the sweep rather than left as a count. 2 mutations,
   and they bite in the restoring direction: putting the verdict phrase back into
   an unjudged declaration fails the room.
117. **MEDIUM: THE FILM STATED A FALSE REASON FOR THE SAME ZERO, AND THE CORRECT
   HELPER WAS ALREADY BEING CALLED BY THE OTHER TWO CHANNELS.** `animNotes`
   hardcoded *"as-built gated lap 0 — no pair of wall tiles detours more than 4
   tiles, so no pair was judged"*. It is false in exactly the two rooms criticism
   85 already names as the reason its own label had to branch: E7S5 (worst pair 35
   in / 2 out — a **33-tile** detour, the fleet's worst, `over: 3`) and E6S3 (**5**
   tiles, `over: 26`), where pairs DO clear the floor and coverage excuses every
   one of them. `mobilityUnjudgedWhy` exists, branches on `maxDetour`, and was
   called by the room page and the index chip and not by the film — so one room
   shipped three reader channels and two answers, with the film asserting the one
   the other two were written to avoid. The helper is called now, and the round
   went further than the finding: the wording had been about to exist in two
   places, so it is ONE exported `unjudgedReason` with the film, the page, the chip
   and the declaration all reading it. **The gate is a source guard, not an
   artifact one, and is filed as such** — it fires on rooms with an as-built gated
   lap of 0 and `maxDetour` over the floor, requires the helper to exist, requires
   `animNotes` to call it, requires the old string to be absent from the code
   (comments stripped, so quoting the removed sentence is allowed), and requires
   at least three call sites. It is live rather than vacuous: it bit E6S3 and E7S5
   on the unfixed tree.
   **EVERY CLAUSE IN THAT LAST SENTENCE WAS TRUE AND THE GUARD WAS STILL A SIEVE
   — ROUND 25, WITH THREE WORKING BYPASSES.** "Requires the old string to be
   absent" is one literal regex, so a REWORD walks past it with a dead call left
   in place to satisfy the rest. "Requires `animNotes` to call it" reads a window
   ending at the first column-0 `}`, and a template literal contains one, so the
   verbatim removed sentence re-ships four lines below the cut. "At least three
   call sites" counts a regex over raw source INCLUDING COMMENTS, so deleting the
   room page's real call and writing `// see mobilityUnjudgedWhy(plan) for the
   shared wording` above a typed replacement restores the count. All three pass
   `172/172` and all three re-ship a caption that is FALSE in E6S3 and E7S5. The
   guard is an output comparison now — criticism 121 — and this paragraph is kept
   unedited above it because the useful thing about it is that it is a careful,
   specific, four-clause description of a gate, written by somebody who believed
   it, and it was wrong about what it enforced in every clause that had content.
118. **MINOR: THE DECLARATION ORDER WAS DERIVED ON NO BOARD WHILE THE NOTE SAID
   "IN DECLARATION ORDER".** `sig9` is sorted and drops `at`, so permuting two real
   declared keys' indices — with `preTakeShortfalls` and the ranking pass permuted
   to match — passed 172/172 on a MOVED board (E11S7) and an UNMOVED one (E11S4),
   and the note then printed a false claim about an order nothing had checked.
   The control (deleting a key) bit, which is the shape this document has learned
   to distrust: a gate on membership standing in for a gate on order. The order is
   gated against **two measured anchors** rather than asserted — the published
   pre-take channel equals the shipped declaration channel index by index once the
   one re-filed `eco` declaration and the retired `towers/clump` are set aside,
   and `declaredKeys` appear in the shipped channel's order — and the unmoved-board
   comparison is ORDER-SENSITIVE now, with skips left as a multiset because `eco`
   moves within them. Both anchors were measured over 134/134 records before the
   gates were written, which is the only reason they could be written as equalities.
   5 mutations. **The residue is stated rather than smoothed**: a key's index
   swapped with the `eco` skip's is invisible to the first anchor, because `eco` is
   never a key and in a one-key record the swap reorders nothing. It is at 119.
119. **STILL OPEN, AND IT IS ROUND 24'S OWN RESIDUE LIST, FILED BY THE CLUSTERS
   AGAINST THEIR OWN WORK.** Two items, both the honest kind, and both stated with
   the reason they cannot be demonstrated the usual way.
   (a) ~~**The OM4 channel-agreement gate is a SOURCE guard and cannot be reached by
   a mutation.** `mutate.mjs` mutates artifacts; this gate reads `plan.mjs`. It is
   exercised live — it bites the unfixed tree in E6S3 and E7S5 and passes the fix —
   and it is counted separately from the 44 artifact mutations rather than folded
   in with them, for the reason 112(a) gives.~~ **CLOSED IN ROUND 25, AND THE
   RESIDUE'S OWN FRAMING WAS THE PROBLEM.** This clause treats "source guard" as a
   COVERAGE limitation — a gate the harness cannot reach — and files it as honest.
   It was a SOUNDNESS one: a token grep over source is not a weak version of an
   output check, it is a check of a different thing, and round 25 walked past it
   three separate ways while `pass 172/172` held (criticism 121). What was
   unreachable by mutation was not the property, it was this implementation of it;
   the property has always lived in the emitted files, where mutation was never
   needed because string equality would do. Filing a soundness gap as a coverage
   gap is how a residue list buys a round of quiet, and this is the second one in
   this entry that did it.
   (b) ~~**One order permutation is invisible to its own anchor.** A declared key
   whose index is swapped with the `eco` skip's escapes the pre-take channel
   comparison, because `eco` is never a key and a one-key record reorders nothing
   under the swap.~~ **REWRITTEN IN ROUND 25 WITH THE MECHANISM THE MECHANICAL
   REVIEWER PROVED, WHICH IS BROADER THAN THIS ONE IN THE DIRECTION THAT MATTERS.**
   The escape has nothing to do with ONE-KEY records. The anchor filters `eco` out
   of BOTH sides before comparing, so the swap is invisible on ANY record where it
   does not move a key past another KEY — multi-key records included. Demonstrated
   on E12S7, whose `sealedRecovery` record carries two keys
   (`towers/weak-battery@0`, `misc/off-network@1`) and the skip at `2`: swapping
   `misc/off-network` with `eco` and reordering `preTakeShortfalls` to match passes
   `172/172`, and the record then publishes the pre-take channel as
   `[towers/weak-battery, eco, misc/off-network]` in a room that declared `eco`
   LAST. Fleet-wide, **36 of the 134 records carrying an eco skip, across 17 rooms,
   12 of them with two or more keys**. The control — two REAL keys permuted, same
   edit shape — bit at 171/172 the whole time, which is exactly the shape this
   document has learned to distrust: a live control standing in for a scope nobody
   measured. **The sentence written here was not a narrower version of the truth,
   it was a wrong account of the mechanism**, and it was wrong in the way that
   costs most — it named a structural reason (*"a one-key record reorders
   nothing"*) that a reader could use to bound the class, and the bound was false.
   Closed at criticism 125.
   (c) **And the item round 24 closed by refutation is worth naming here too**,
   because it is where the last two rounds' residues have gone: 112(b) said a
   measurement was dated behind a committed one-liner. It was dated behind
   nothing, at six sites rather than four, on a figure that had already rotted.
   **Two rounds running, this list's own bookkeeping has been the defect it was
   filed to prevent** — 104(c) on a count and a mechanism, 112(b) on all three of
   its clauses — which is a pattern about residue lists rather than about either
   entry, and the next round should read this one hostilely first.
   **THREE ROUNDS RUNNING NOW.** Round 25 read this entry hostilely first, exactly
   as the sentence above asked, and both of its live items were wrong rather than
   narrow — (a) mis-classified a soundness hole as a coverage one, (b) stated a
   mechanism that does not bound the class it claims to bound. **Every residue
   this list has filed in three rounds has turned out to be wrong in the same
   direction: too flattering to the gate it describes.** That is no longer a
   pattern about individual entries and it is written here as the rule the next
   round should apply — a residue written by the cluster that built the gate is a
   claim by an interested party, and the only ones in this document that have
   survived contact are the ones that came with a measurement attached.

Round-25 findings. **Two fresh reviewers, 15 confirmed findings between them,
and 0 of them on a board** — the owner-voice reviewer **1 BLOCKING**, 4 MEDIUM
and 2 LOW plus one carry-over re-confirmed; the mechanical reviewer 0 CRITICAL,
**1 MAJOR**, 5 MINOR and 2 LOW. **Every headline figure in this document was
reproduced from scratch before either reviewer found anything**, and the
from-scratch half is what makes the findings small: an independent auditor
running its own mongo dump, its own engine obstacle set and ZERO suite imports
re-derived 172 rooms · 10,320 extensions (60/60 × 172) · 14,100 roads · 688
containers · 4 links in all 172 · `road+rampart 274 = 231 cross + 30 bubble + 13
ring + 0 cover + 0 unclassified` · `prunedGhosts 2001` · the roadKind census
`491 = 370 + 82 + 21 + 11 + 4 + 3` · the tower clump histogram
`{0:12, 1:14, 2:53, 3:60, 4:30, 5:3}` byte for byte · shallow extensions 25
across 3 rooms · 0 orphan roads · 0 off-network beyond the 133 declared · the
upgrader station in range, standable and link-adjacent in 172/172 · battlements
5,823 with 0 unstandable and 0 off-cut · 0 rooms with adjacent spawns · and the
sealedFloor `named`/`tiles` rosters reproducing EXACTLY in all 172 rooms. Seven
of the note channel's nine live classes — 192 of 236 notes — survived full
fleet-wide adversarial re-derivation with zero false claims, including the
expensive ones: per-holder counterfactual recovery, exhaustive single-removal cut
tests, closure minimality and the 2,304-tile band scan.
**Four themes, and the first is the round.**
First: **the film had a second taxonomy, and the project's own rule was violated
one channel over.** This document's roadKind precedent is quoted in its own
prose — *"a tile the enum has no word for fails the room rather than joining the
largest class"*, *"a residue class that can absorb anything is not a taxonomy"* —
and the film's rampart chain had a class the enum could never assign (`denial`,
0 of 8,208, unreachable because `bubble` was tested first and 223 of the fleet's
237 stand-denial tiles are also bubbles) while its largest non-cut class absorbed
three occupant kinds under a caption naming one of them. **A rule this document
enforces on the board channel and does not enforce on the auto-fail film channel
is not a rule, it is a habit**, and it took a reviewer reading a film and a note
about the same tile to notice.
Second: **the token-grep source guard is a CLASS, not an instance.** Three
bypasses on one guard, each passing `172/172`, each re-shipping the same false
caption — and the replacement is not a better grep, it is the observation that
the claim lives in the OUTPUT, where string equality settles it on all 117
unjudged rooms instead of on the 2 the old guard could reach.
Third: **a witness that cannot be wrong is not a witness.** `withheld` was a
number nobody could re-derive and nothing would gate; `preTakeShortfallBasis` was
one byte-identical sentence in 134 records wearing the word "basis"; the
`exterior` declaration class had no state in which declaring correctly counted
for anything. All three read as evidence and none of them were, which is the
`prunedTiles`/`orphanRoads` lesson this file states twice and the newest channels
keep not inheriting.
Fourth, and it is about the review rather than the artifact: **the mechanical
reviewer filed a finding against the recovery census, then found the nested
fixpoint record it had missed by reading `meta.sealedRecovery` non-recursively,
and RETRACTED IN FULL — 63 records in 62 rooms, 976/976 candidates,
`allRefused` 4, `otherMoved` 26 across 15 rooms, every doc figure reproducing
exactly.** It could have been quietly dropped; it was written up as a retraction
with the mechanism that fooled it named. **That is the standard for reviewer
honesty in this campaign and it is recorded here as one**, because a review
process that only publishes its hits is measuring the reviewer and not the
artifact — and this document has spent three rounds learning the same thing about
its own residue lists from the other direction.
120. **BLOCKING: THE FILM'S RAMPART TAXONOMY HAD A CLASS THAT COULD NEVER BE
   ASSIGNED AND A CAPTION THAT WAS FALSE ON 63% OF THE TILES IT PAINTED — AND
   THIS DOCUMENT HAD PUBLISHED THE DEAD CLASS AS A CENSUS.** `export-anim.mjs`
   chained `cut → bubble → denial → cover`. Over all 8,208 shipped ramparts that
   yields **cut 7,234 · bubble 807 · denial 0 · cover 167**, and the zero is a
   construction rather than a reading: the fleet ships **237** `shell.standDenial`
   tiles, **223** of them are also in `shell.bubble`, and `bubble` is tested
   first — so `denial` is unreachable by definition and no film anywhere contains
   a stand-denial caption. `bubble`'s caption is *"bubble seat — a container
   outside the shell, covered where it stands"*, and its 807 members are
   **container 295 · link 299 · NOTHING AT ALL 213** (all 213 empties being
   stand-denial ring tiles): the caption names a container on **512 tiles that
   have none**. **The smoking gun is a single tile with two shipped answers in one
   gallery.** E11S3's film painted `21,12` as *"bubble seat — a container outside
   the shell, covered where it stands 4/4"*; E11S3's own shipped note, class
   `roadRampart`, says *"1 CONTROLLER STAND-DENIAL RING tile(s) (21,12) … these
   tiles are not on `meta.shell.cut` and carry no structure"*. Twelve more rooms
   carry the same contradiction on the 13 `ring` tiles of the published
   `roadRampart` census. **What makes this BLOCKING rather than MEDIUM is the
   precedent it violates, which this document wrote itself**: the roadKind enum
   fails a room rather than letting an unnamed tile join the largest class, and
   the note inventory declares its dead `pavingGap` class WITH its reason and
   mutation-covers it — while the film's dead class was never mentioned by
   anything. **The fix is one classifier, not two agreeing ones.**
   `rampartClassifier(plan)` is exported from `layer-walls.mjs` and lives there
   ONCE; `classifyRoadRamparts` is rebuilt on it (published census unchanged in
   172/172) and `export-anim.mjs` imports it and deletes its own copy, so the two
   channels cannot drift because there is nothing to drift. The order is the note
   channel's own — `crossing → seat → ring → cover → unclassified` — and over the
   full 8,208 it read, ON THAT BUILD, **7,234 crossing · 295 bubble seat · 141
   inside seat · 213
   stand-denial ring · 325 personal cover · 0 unclassified · 0 none**, with the
   cover class's occupants named (**link 299 · extension 25 · lab 1**) because
   "personal cover" is a claim about what is underneath. Captions derive PER
   MEMBER and runs break on the caption rather than on the class, so a container
   seat and a link bubble cannot share a sentence again. And the films ship
   `rampartCensus` — one row per facet over a FIXED seven-facet order, **empty
   facets included, each carrying an `emptyBecause`** — which is the `pavingGap`
   discipline moved to the channel that had been missing it: verified at
   **494 empty facet rows across the fleet and 494 of them carrying a reason** —
   a PRESENCE check, which is the thing criticism 131 then took apart, and today
   the same channel reads 555 rows over 41 derived reasons. The
   ring class is LIVE at 213 where it was 0, E11S3's `21,12` now reads *"controller
   stand-denial ring 2/2"*, and its film and its note finally say the same thing.
   **The gate is an artifact one and it was measured on both sides before it was
   written**: the film's painted class is compared tile by tile against the
   classifier this validator already runs for `meta.walls.roadRampart` — an
   unmapped caption fails, a shipped rampart the film never paints fails, a painted
   tile carrying no rampart fails. On the ROUND-24 artifact and films it finds
   **653 disagreements in 168 of 172 rooms**; on the rebuild, **0 — 172/172 agree**.
   5 mutations.
   **AND THE DEFECT RECURRED INSIDE THIS FIX, AT 48% OF THE CLASS, AND THE GATE
   IN THE PARAGRAPH ABOVE IS THE REASON IT COULD.** The entry's own headline is
   "a caption that was false on 63% of the tiles it painted"; round 26 measured
   this fix's replacement captions and found **210 of the 436 seat tiles wrong,
   in 149 rooms** — 79 painted *"outside the shell"* that are inside it and 131
   painted *"inside the wall, on shallow floor"* that are outside — because the
   `seat.bubble`/`seat.inside` split was `shell.bubble` membership and that list
   is a DISJUNCTION. The gate above compares the film's painted CLASS against
   `rampartClassifier`, and both channels read `inBubble` off the same list, so
   it agreed at 172/172 while both sides were wrong; `FILM_CAPTION_CLASS` mapped
   both seat sentences to the one class `seat`, so swapping every caption in
   every film passed byte-identically. **An output-equality gate between two
   readers of one wrong fact is an AGREEMENT test, not a truth test** — the
   lesson of round 26, written at criticism 129, where the disjunct is re-derived
   from the board per tile and the census reads 347/89.
121. **MAJOR + LOW: THE SOURCE GUARD WAS A CLASS OF DEFECT, NOT AN
   IMPLEMENTATION DETAIL — THREE WAYS PAST IT, AND IT ASKED ABOUT 2 ROOMS OF
   117.** Criticism 117's fix was held shut by a token grep over `plan.mjs`, and
   round 24 filed the fact that a mutation cannot reach it as an honest residue
   (119(a)). Round 25 reached it three times without a mutation, each landing at
   `pass 172/172 · fail 0` and each re-shipping a caption that is false in E6S3
   (worst detour 5, floor 4) and E7S5 (33): **(a) REWORD** — the guard's only
   content test is one literal regex, so leaving a dead
   `const _why = mobilityUnjudgedWhy(plan);` in place and printing *"the gate
   judged no pair: every wall pair sits inside the 4-tile detour floor"* satisfies
   every clause. **(b) WINDOW TRUNCATION** — the guard reads
   `src.slice(at, src.indexOf("\n}", at))`, and a column-0 `}` inside a TEMPLATE
   LITERAL ends that window early, so the call goes before it and the verbatim
   removed sentence goes after it. **(c) CALL-SITE LAUNDERING** — the call count is
   a regex over raw source including comments and strings, so deleting the room
   page's real call fails at *"called 2 time(s)"* until you add
   `// see mobilityUnjudgedWhy(plan) for the shared wording`, and then it passes.
   **The comment is the launderer, and that is the sentence this entry exists
   for.** The LOW rides here because it is the same defect one clause over: the
   call COUNT is file-wide, so three calls from one channel satisfied the
   "per-channel attribution" it was described as providing while the other two
   typed their own answers. **The replacement is not a stronger grep.**
   `unjudgedChannels(room)` reads the three channels back OUT of the files this
   suite writes — the film caption from the room page's `NOTES` map, the page from
   its `mob-sub` line, the index chip from its own card's `title` — and requires
   (i) all three to be the SAME sentence and (ii) that sentence to contain
   `unjudgedReason(maxDetour, floor)`, the producer's own exported helper run here
   on the room's published `maxDetour`, which the `walls/mobility-built` gate
   independently holds to the exact all-pairs walk over the shipped cut. Two
   checks, one on each side of the record. **It runs on every room whose published
   lap is 0 — 117 of 172, against the old guard's 2.** The only source test left is
   that the helper EXISTS, which is the one property no output can show. All four
   forgeries — the three bypasses plus a fourth channel, the index chip typing its
   own sentence — were reproduced as the OUTPUT each of them produces, patched
   into the shipped pages, validated and restored byte-identically (E7S5.html and
   index.html md5s unchanged after), and **all four are CAUGHT**, failing as
   *"this room's three channels print two different reasons for one unjudged
   lap"* with both quoted. OL5 is closed by construction rather than by a fix:
   the comparison is per-channel, so a call count proves nothing any more. 3
   mutations, plus the four channel forgeries, which are run against the shipped
   pages rather than by the suite for the reason stated at criticism 128.
122. **MEDIUM ×2: TWO DECLARATION CHANNELS DESCRIBING A BOARD NOBODY MEASURED —
   ONE SHIPPING A PASS VERDICT OVER IT, THE OTHER SHIPPING IT IN THE PRESENT
   TENSE.** (a) The `covered-detour` mobility declaration shipped
   *"THE VERDICT, for contrast: over the 0 pair(s) this room's gate does judge …
   the lap is 0, inside the target"* in E6S3 and E7S5 — a two-branch ternary
   (`gatedOver ? "over target" : "inside the target"`) over a measurement with
   THREE outcomes. Fleet census: 8 `covered-detour` declarations, the 6 with
   `gatedPairs > 0` read correctly, exactly the 2 with `gatedPairs === 0` say
   "inside the target", and those 2 rooms have no OTHER mobility declaration, so
   the declaration channel's sole verdict for them was the false pass. **The
   contradiction is on one page**: the same room's chip and mobility cell say
   *"UNJUDGED — every pair over the detour floor is covered"* and *"A zero here is
   the measurement not taken, not a room that passed"*, and the declaration never
   retracts. Two aggravations, both this document's own: criticism 116 claimed the
   file was swept and that the remaining "inside the target" strings are per-PAIR
   readings — this is a gated-LAP verdict — and E6S3/E7S5 are exactly the two rooms
   criticism 117 fixed the FILM for in the same round. It survived because the
   verdict gate was scoped `if (g === "mobility" && !k)`, skipping every KINDED
   mobility declaration; the `!k` is gone and the renderer has a third branch
   carrying UNJUDGED, derived from `record.gatedPairs` with no new record leaf.
   (b) `sealedRecovery` notes in 12 rooms described the PRE-TAKE board in the
   shipped board's present tense — E11S7's *"The candidates are EVERY MOVABLE SEAT
   this room ships — 61 of them"* names **114 seats fleet-wide that are not on the
   shipped board**, one of which ("extension@31,20", E2S1) ships an OBSERVER, and
   four rooms assert pockets they do not have in a paragraph that says two
   sentences later that the recovery took the sealed floor to zero. **The artifact
   already contained the fix in the same build**: `towerSwap`'s note says
   *"`declaredKeys` on this record is therefore the PRE-TAKE set and not what the
   room ships"*. The roster sentence says so now, and the gate's trigger is
   DERIVED rather than phrasing-matched — a `taken` record whose withdrawn seat is
   not a movable seat on the shipped board is provably about another board, true
   in 12/12 — with the 3 `allRefused` runs exempt, because taking nothing leaves
   the judged board and the shipped one the same room. That exemption is not a
   convenience: those runs measure 0/61 off-board, which is what proves the defect
   was provenance and not definition. 5 mutations.
123. **MEDIUM + MINOR: THE FLEET'S 2,786 WITHHELD TILES WERE A NUMBER A PRODUCER
   COULD WRITE FREELY, AND THE ROUND'S BRIEF QUESTION WAS UNANSWERABLE FROM THE
   SHIPPED ARTIFACT.** `meta.exteriorContract`'s `withheld` column was
   presence-and-type gated only — `Number.isInteger(w) && w >= 0`. Confirmed by
   mutation three separate ways by two reviewers: inflating ONE record by +500,
   zeroing all 688 fleet-wide, and setting the fleet to 999999, all passing
   `172/172` with **byte-identical summary lines**; a strictly DECREASING run
   across the four consumers passed too, which is the contract's OWN justification
   being unchecked. The six `exteriorContract` mutants criticism 114 counts touch
   withdrawal, dropped consumer, reversed order, undeclared exposure and a null —
   **not one of them touches `withheld`** — and only `exposed` was value-gated.
   **And there was no tile list**: the record is `{at, exposed, withheld}` plus
   `exposedTiles` when `exposed > 0`, so none of the 2,786 was named, and
   `plan.exterior` is stripped by `slim`, so a reader could not re-derive it
   either. **This is the shape this very file has already fixed twice and SAYS SO
   in its own prose** — `prunedTiles` must contain every ghost so *"the empty list
   is a missing-tile failure rather than a vacuous pass"*, `orphanRoads` names the
   roads *"that ended up connected to nothing"* because *"an absent list reads
   exactly like an empty one"*. The newest channel did not inherit it, and
   `numeral-audit` owned no extractor for it, so the doc's "withheld 2,786" was
   hand-carried on both sides. **The brief's question is answered and the answer is
   NO**: 797 DISTINCT withheld tiles fleet-wide, 795 of them within chebyshev 3 of
   a source, controller or mineral — the remote cluster bubbles — and the decisive
   argument is that layer 7's `paveable()` reads `liveExterior()`, not the frozen
   flood, so it was FREE to pave any of the 797 and paved ZERO. Paving all 209 bare
   ones changes a route in 5 rooms by 2–3 cost out of 25–56, in every case
   mid-corridor on a lane the room does not pave at all; 0 would bridge an
   off-network structure. **The fix publishes the witness rather than tightening
   the number.** `withheldTiles` per record (`withheld === withheldTiles.length` in
   688/688); `meta.exteriorContractBasis` stating in the room's own text that the
   four numbers are ONE TILE SET READ AT FOUR MOMENTS so nobody sums them —
   E11S1 publishes `[10,10,10,11] = 41` for 11 tiles, and the fleet's 2,786 against
   797 distinct is the same trap at scale; monotone by **SET CONTAINMENT** rather
   than by non-decreasing count, naming the tile that dropped out; and
   `meta.shell.cutAtFreeze`, the layer-2 cut snapshotted before layer 7's inert
   prune and seal reconciliation, which is what the frozen flood was actually taken
   against. **The correction this round owes itself is in the last of those**: the
   mechanical reviewer derived L3's roster from the SHIPPED cut and closed it in
   165/172, predicting **7** rooms of difference. With the snapshot published, the
   frozen cut differs from the shipped one in **29 rooms**, and the L3 derivation
   closes in **172/172** where the doc's own residue had it at 143/172. The 7 was
   a count of rooms where the difference MOVED the answer; the 29 is the count
   where the two cuts differ at all. **A residue that predicts a coverage figure
   and is out by a factor of four is worth more written down than smoothed**, and
   it is the reason the snapshot exists rather than a re-derivation off the shipped
   cut with an apology attached. 10 mutations — zeroed, inflated, decreasing sets,
   roster withdrawn, tile appended, tile dropped, snapshot withdrawn, snapshot
   re-pointed at the shipped cut in a room where that MOVES the derivation, basis
   dropped, and a repeat tile padded in to make the count.
124. **MEDIUM + LOW: A LADDER THAT GAVE THE WRONG REASON, AND A NOTE THAT
   WITHHELD THE CLAUSE MAKING ITS OWN ARITHMETIC CLOSE.** (a) E4S2's ladder prose
   printed the price-allowance verdict unconditionally: *"That is over the
   `<allowance>` rampart(s) the `<perRatio>`-per-1.0 mobility price allows"* — and
   on E4S2's own numbers `extra = 1`, `reclaimed = 1.5`, `allowance = min(12,
   floor(3 × 1.5)) = 4`, so **1 is not over 4 and the main clause is refuted by the
   figures in its own sentence**. The real reason is the parenthetical: `buyFloor`
   is 2 and the shipped lap is 1.5, so wall may not be spent on lap at all. **The
   VALIDATOR had this right in the same build** — its gate is guarded by
   `shipLap > buyFloor` and does not fire — and the prose had no such guard, which
   is the recurring shape of this whole section: two channels, one guard. Fleet
   sweep: 57 shortfalls carry a ladder, 13 have a materially-better complete rung,
   E4S2 is the ONLY room where the claim is false and there are 0 rooms giving no
   reason at all. The renderer takes the policy's own order now — floor first, then
   price — E4S2 reads *"REFUSED ON THE FLOOR AND NOT ON THE PRICE"* with the price
   still printed, and a third arm prints the inconsistency where neither refuses
   it. The gate re-derives the renderer's own pick (cheapest materially shorter
   COMPLETE rung) off the record's own per-1.0 price and cap, so a paragraph
   claiming the price refused a rung the price ALLOWS fails, and a room whose lap
   is not over the buy floor must say so.
   (b) E12S6's `shallowExt` note publishes *"found 2 already road-faced and 4 more
   that are ONE PLAIN PAVE … Of the one-pave class, 3 were TAKEN … and 0 were
   left"*: 4 − 3 = 1, not 0, in 7 rooms. **The code is right and deliberate** —
   `paveLeft` is RE-SCANNED against the shipped board rather than subtracted from
   the opening census, because a candidate can leave the class without being taken
   (in E12S6 `35,13` was taken as a NEIGHBOUR's pave), so the subtraction would be
   a fiction. The note publishes the correct number and withheld the one clause
   that makes the three close, which leaves a reader who checks the arithmetic
   holding a broken sum and no way to learn why. **That is a LOW and it is filed as
   cheapest-honest-telling rather than as correctness**, which is a distinction
   this list should make more often than it does — the sub-agent report that
   surfaced it called it "an event count wearing a tile label" and it is not that,
   and the downgrade is recorded because a finding inflated past its evidence costs
   the next round's credibility.
125. **MINOR: THE ECO-SKIP RESIDUE WAS BROADER THAN 119(b) DESCRIBED, AND THE
   ANCHOR THAT MISSED IT FILTERED THE EVIDENCE OUT OF BOTH SIDES BEFORE
   COMPARING.** Full mechanism and fleet numbers are written at 119(b), where the
   wrong sentence is; what belongs here is the fix and what it cannot reach. Two
   anchors, neither needing a new published field. **ACROSS A ROOM'S RECORDS**:
   every pass publishing a pre-take channel ran before `redeclareEcoTax`
   re-appended the declaration, so they must agree on `eco`'s spine POSITION — 0
   rooms disagree today. **ACROSS THE FLEET**: `ecoSidePartition(plans)` derives
   which declarations were filed before `eco` and which after, over every pre-take
   channel in the run; a record on the wrong side fails, and a fleet that
   contradicts ITSELF fails the rooms carrying an eco skip with the contradiction
   named. It is wired into the validator's fleet pass and into the mutation
   harness's FLEET, which is built from the UNMUTATED plans so a one-record forgery
   is held to the clean artifact. **Both kills re-run by the doc pass against the
   SHIPPED artifact rather than taken from the cluster's summary**, which is worth
   the sentence because the two figures came back the same and the summary's did
   not: the one-record E12S7 swap fails **18 rooms** and the whole-fleet sweep
   fails **18 rooms** on *"`towers/weak-battery` stands before `eco` in 6
   record(s) and after it in 4"* — the validator cluster wrote **19** for the
   first, and 18 is what `validate.mjs` prints today. The sweep touches **34** of
   the 36 eco-skip records, the other two carrying `eco` at index 0 with no
   predecessor to swap with, and that 34-against-36 is stated rather than rounded
   for the same reason. The two-real-key control still bites. **THE RESIDUE, STATED WITH ITS FLOOR
   RATHER THAN AS A HOPE**: a sweep that moved `eco` past the SAME declaration in
   every record of the artifact at once would leave the partition self-consistent,
   because nothing published names the MOMENT `eco` was filed. That is not a gap in
   this gate, it is the limit of what an artifact can prove about an order it does
   not record — and it is written that way, with the mechanism, because the
   sentence it replaces is the one 119(b) got wrong. 4 mutations.
126. **MINOR ×2: TWO LIVE STALE FIGURES IN `validate.mjs`, ONE OF THEM IN A
   MESSAGE A READER SEES, BOTH SITTING IN THE NUMERAL GATE'S ONE REMAINING BLIND
   SPOT — WHICH HAD EXACTLY TWO OCCUPANTS AND IS NOW CLOSED.** (a) The spec text
   for `negotiated.causeWalks.noWalls.detour` read *"16 of the fleet's 57 records
   are negative here, DOWN TO -3"*. Records 57 and negatives 16 are both correct;
   the minimum is **-9** (E9S9, `d 8 / dout 17`), three times the claimed floor,
   with the next four rooms at -3. (b) The `alongCutRefused` mislabel FAILURE
   MESSAGE — printed to a reader, not a comment — read *"27 refusals = 16
   breaks-network + 7 no-parallel + 4 seat"* against a fleet shipping **23 = 9 + 10
   + 4**. Three wrong numerals in one printed sentence, a round-21 reading carried
   unchanged through rounds 22, 23 and 24 — **while this document's own round-22
   block recorded the move `27 → 22 (16/7/4 → 10/8/4)` sixteen hundred lines
   away.** Both were invisible to `numeral-audit.mjs` for the same reason: the
   claim is split across a `+` between two string segments and `joinedText()`
   returned string ranges verbatim. **The blind spot was measured before it was
   closed, which is the part that makes this an entry rather than two edits**:
   sweeping the whole audited tree with segments joined surfaces exactly **two**
   claims not visible before — these two, and nothing else. `scanRanges` merges
   `+`-joined string literals with offset-preserving seams, the CRLF fixture gained
   a concatenated claim and a self-test assertion, and both figures are DERIVED
   now — a per-room `noWallsCensus` summed and printed by the validator's fleet
   pass, and a per-room refusal census inside the message itself with the fleet's
   printed beside it. Both retired figures are kept quoted in comments as their own
   findings' evidence, `[r22-waived: …]`-tagged. The audit ended at **183 claims ·
   122 re-derived · 61 waived at 37 sites · 0 unowned · 0 WRONG**, exit 0. 1
   mutation, on the census inside the message.
   **AND "ONE REMAINING BLIND SPOT … NOW CLOSED" WAS ONE SPOT TOO FEW, WHICH IS
   THE SAME SENTENCE THIS ENTRY WAS FILED AGAINST.** Every one of the gate's five
   shapes required DIGITS, so a figure spelled out in words was invisible to it —
   and round 25's own comments carried three of them, all saying *"seven"* where
   the fleet says **29**, in a round whose criticism 123 publishes the correct 29
   four thousand lines from here. Round 26 closed it the way this entry demands:
   the vocabulary (`zero`..`twenty` plus the dozen forms) reads into the same
   shapes, the fixture gained a spelled-out claim and its self-test, and the
   residue is PUBLISHED rather than argued — the audit now prints how many
   spelled-out numeral+noun occurrences it can see (**95** today, at the same
   ten-and-over threshold the digit census uses, **0** of them currently read by
   a claim shape). Today's census: **192 claims · 131 re-derived · 61 waived at
   37 sites · 0 unowned · 0 WRONG**, exit 0, over 1,697 numeral+noun occurrences
   of which it parses 132. Criticism 133 carries the four wrong sites and the
   threshold's own cost.
127. **MINOR + LOW ×2: A TWIN THAT WAS NOT A SECOND DERIVATION, A DECLARATION
   CLASS WITH NO PASSING STATE, AND A "PER-ROOM WITNESS" THAT WAS ONE SENTENCE
   REPEATED 134 TIMES.** (a) `numeral-audit`'s `declaration` twin declares
   `via = "the (gate/kind) partition of `fleetTotals`, added back up"` and calls
   neither `fleetTotals` nor its `byKind` partition, which exists — it re-walks the
   SAME array and counts entries with a truthy `.gate`. The registry's own header
   requires a second path to read DIFFERENT FIELDS or aggregate them DIFFERENTLY,
   and this does neither: one fabricated `meta.shortfalls` entry gives 301 and 301,
   AGREE, while the genuinely twinned `note` label gives 237 vs 236 on the same
   treatment. **So "21 of 22 extractors carry an independent second derivation"
   over-counted by one, and the over-count was inside the round-24 fix that
   introduced twinning** — a self-check certifying itself. `declarationPartition()`
   is extracted and the twin goes through it, poisoning the total on a class-less
   bucket or an inconsistent room/entry lattice; the `via` string says what it
   does; and the coverage limit is stated in the code comment rather than implied
   (a class-less planted declaration makes the twin DISAGREE, a well-formed one
   does not, and that is written down). (b) `gate:"exterior"` had no passing state:
   `exposed > 0` pushed a failure unconditionally, so the class was an obligation
   with no reward and the "gated in BOTH directions" of criticism 114 described one
   direction. The declared state PASSES the exposure now — and passes LOUDLY, with
   *"EXTERIOR DRIFT DECLARED — …"* on the record — while a roster with no
   declaration, a declaration with no roster and a declaration over other tiles all
   fail, distinguished by OUTCOME rather than by message text. 4 mutations, in a
   class that had none because there was nothing to break. (c)
   `preTakeShortfallBasis` was **byte-identical in all 134 records**, so a
   cross-room transplant was a no-op: a constant rule sentence wearing the word
   "basis". It carries a per-record census first — count, classes in order,
   keys and skips with the arithmetic that closes them — and then the constant rule
   text, taking the field from **1 distinct string to 24**.
128. **STILL OPEN — ROUND 25'S OWN RESIDUE LIST, AND IT IS WRITTEN UNDER THE
   RULE 119 JUST LEARNED: EVERY ITEM CARRIES A MEASUREMENT OR AN ADMISSION THAT
   IT DOES NOT.** Four items, none of them a coverage complaint dressed as an
   honest one.
   (a) **The artifact and the gallery must now ship together, and that is a real
   coupling this round created.** The OB1 and MM1 gates read `out-v2/` films and
   pages. A validator run pointed at a `PLANS_FILE` whose pages and films were
   written by a DIFFERENT build compares two builds — it fails rather than passes,
   which is the right direction for a gate, but it means the artifact is no longer
   checkable on its own. **Measured, not asserted**: this is exactly why the four
   channel forgeries at criticism 121 are run against the shipped pages by a
   scratch harness instead of by `mutate.mjs`, and that split is stated in the
   mutation roster rather than hidden inside the count.
   (b) **`FILM_CAPTION_CLASS` transcribes the film's caption VOCABULARY, and a
   renamed caption fails until a row is added.** This is deliberate and is written
   here so the next round does not report it as a find: the CLASSES are the shared
   classifier's, so nothing about the taxonomy is transcribed — but an unmapped
   caption is a class nobody compared, and failing is the only honest answer to
   that. It is a maintenance cost accepted with its reason, which is the shape
   criticism 30 asks for.
   (c) **The MM3 total-sweep residue, restated because it is the floor rather than
   a to-do**: see 125. Nothing published names the moment `eco` was filed, so an
   artifact-side gate cannot distinguish a fleet that moved it everywhere at once
   from one that always had it there. Closing this needs a published field, not a
   better anchor, and no field is proposed here because none is obviously worth
   the schema.
   (d) **A PROCESS gap, and it is filed against the round rather than against the
   code.** `shared.mjs`'s `checkEnclosureContract` — the producer for criticism
   123 — was edited by the producer cluster, and `shared.mjs` is in neither
   cluster's stated file ownership. The edit is correct and the function lives
   nowhere else, so there was no other place to make it; what is missing is that
   the ownership statement did not cover the file the round's own fix required.
   **Two clusters editing by convention rather than by declared boundary is how a
   round loses the ability to say who wrote a line**, and this campaign's whole
   determinism and attribution apparatus rests on being able to. Stated now, at
   the cost of a clean-round sentence, rather than after it collides.
   **FOUR ROUNDS RUNNING, AND ROUND 26 READ THIS LIST HOSTILELY FIRST AS THE
   ENTRY ABOVE ASKS — TWO ITEMS SURVIVED, ONE WAS TOO FLATTERING TO ITS GATE, AND
   ONE WAS ANSWERED BY BUILDING THE THING IT DESCRIBED AS A COST.** (a) SURVIVES
   and is now partly paid: the coupling is real, and the round-26 harness stopped
   working around it — `runFile` forges a shipped film, page or chip, runs the
   gate against the room's unmutated plan and restores the bytes under a verified
   byte-identity check, so **11 of this round's 37 cases are inside the count**
   where round 25's four controls were outside it. What stays open is the
   coupling itself: a `PLANS_FILE` from another build still compares two builds.
   (b) **WAS THE FLATTERING ONE.** It described a transcribed VOCABULARY whose
   worst failure mode is a loud false alarm; the actual failure mode was silence,
   because the table mapped the two seat sentences — which differ precisely in
   the half a reader cares about — onto ONE class, so swapping every seat caption
   in every film passed `172/172`. The residue named the maintenance cost and not
   the hole. `FILM_CAPTION_CLASS` is replaced by an exact-sentence table with the
   occupant captured, at criticism 129. (c) SURVIVES verbatim, re-confirmed by
   exploit: the fleet-wide eco sweep still leaves the partition self-consistent
   and the real validator at `pass 172/172`, and 0 of the 38 rooms filing `eco`
   carry an eco-less pre-take record, so the derivable anchor a reader might hope
   for has **zero coverage on this fleet** — measured, not assumed. (d)
   SURVIVES and recurred: round 26's producer cluster edited `shared.mjs` again,
   for `exteriorContractBasis`, and again it was in neither cluster's stated
   ownership. Two rounds is a convention.

Round-26 findings. **Two fresh reviewers, 10 confirmed findings between them, and
0 of them on a board** — the owner-voice reviewer **1 BLOCKING**, 1 MEDIUM and 2
LOW; the mechanical reviewer 0 CRITICAL, **1 MAJOR**, 2 MEDIUM, 2 MINOR and 1
LOW. **Both re-derived every headline figure in this document from scratch before
either found anything**, and both came back clean on the boards: the hard-gate
sweep from mongo terrain and the artifact alone found 0 failures on every class
this file gates, the road+rampart taxonomy re-derived `274 = 231 + 30 + 13 + 0 +
0` across 153 rooms with median 2 and max 5, the tower-clump histogram
`{0:12, 1:14, 2:53, 3:60, 4:30, 5:3}` reproduced byte for byte, every fleet total
(ramparts 8,208 · roads 14,100 · extensions 10,320 · declarations 300 · notes
236) re-derived EXACTLY, the enclosure contract passed every reader check at 688
records / `exposed 0` / 797 distinct withheld tiles, and the films were
digest-fresh at 172/172. **Two judgment calls were ruled in the plan's favour and
are recorded as rulings rather than dropped**: the 41 rooms containing a 2×2
all-extension square are the owner's "dense but walkable" corridor pattern with a
road on every extension's D4 face and not a brick, and the three `cover` zero-rows
are defensibly zero because a link standing on a cut tile is not renting a rampart
of its own.
**THE LESSON OF THE ROUND, AND IT IS THE ONE THIS CAMPAIGN WILL BE RE-LEARNING
LONGEST: AN OUTPUT-EQUALITY GATE BETWEEN TWO READERS OF ONE WRONG FACT IS AN
AGREEMENT TEST, NOT A TRUTH TEST.** Round 25 closed a BLOCKING film-caption
defect by hoisting ONE classifier so the two channels "cannot drift because there
is nothing to drift", and gated it by comparing the film's painted class against
that classifier tile by tile — 653 disagreements before, 0 after. Both sides of
that comparison read `shell.bubble`. The list was the wrong oracle, so the gate
agreed at 172/172 while criticism 120's own defect ran on inside criticism 120's
own fix at **48% of the class** — 210 of 436 seat captions on the wrong side of
the wall. **The corrective is not a better comparison, it is a different second
term**: every gate this round derives its expected value from the BOARD — the
terrain, the frozen cut and the shipped structure lists — and where it cannot,
it says so in the entry rather than in a comment.
129. **BLOCKING (NARRATION) + MINOR: THE FILM'S SEAT CAPTION WAS SWAPPED ON 210
   OF 436 TILES IN 149 ROOMS, AND THE GATE THAT WAS SUPPOSED TO CATCH EXACTLY
   THIS READ THE SAME WRONG LIST AS THE BUG.** `export-anim.mjs` split the `seat`
   class on `shell.bubble` membership and captioned the two halves *"bubble seat
   — a container outside the shell, covered where it stands"* and *"container
   cover — a container inside the wall, on shallow floor, renting a rampart of
   its own"*. But `addBubble` (`layer-shell.mjs`) bubbles a tile when it is
   OUTSIDE the shell **OR** inside it below `DEPTH_SAFE`, and non-membership does
   not mean inside either — every mineral-seat container is ramparted after layer
   2, is outside the shell, and is absent from the list. Measured on the shipped
   artifact against the flood over the room's own published
   `meta.shell.cutAtFreeze`, which answers all 436 of them: **79 tiles in 72
   rooms painted "outside" are inside the enclosure** (all 79 at depth < 4, which
   is the OTHER caption's own words) and **131 tiles in 131 rooms painted
   "inside, on shallow floor" are outside it** (all 131 mineral-seat containers).
   **E12S1's film contradicted itself in one gallery**: `36,24` painted *"a
   container outside the shell"* while the flood over the room's own published
   `meta.shell.cutAtFreeze` calls it INTERIOR, and `22,40` painted *"inside the
   wall, on shallow floor"* while that same flood calls it EXTERIOR. **THE FIX IS
   THE DISJUNCT, DERIVED PER TILE FROM THE BOARD**: `rampartFacets` floods
   `exteriorFlood(terrain, cutAtFreeze)` and cross-checks that flood against
   `plan.exterior`, THROWING if they differ on one tile — "the flood I derived is
   the flood the plan used" is the premise, so it is asserted rather than
   assumed. The facet is renamed `seat.outside` (the word "bubble" was the false
   oracle), the inside caption carries a per-tile shallow/deep variant measured
   off the frozen depth field, and **`rampartClassifier` LOSES `inBubble`
   entirely** — it had exactly one consumer and that consumer was the bug.
   Census: **`seat.outside` 347 · `seat.inside` 89** where the old pair read
   295/141, and all 89 of the inside seats re-derive below safe depth, so the
   deep caption fires 0 times today and the census shows that rather than a
   comment asserting it. **THE GATE IS CAPTION-VERSUS-BOARD, NOT
   CAPTION-VERSUS-CLASSIFIER.** `FILM_CAPTION_CLASS` — two sentences onto one
   class, no occupant test — is replaced by `FILM_RAMPART_CAPTIONS`, an EXACT
   sentence table over the seven facets plus the `personal cover — one <occupant>
   renting a rampart of its own` template with the occupant captured, and per
   painted tile the validator compares FACET + VARIANT + OCCUPANT, each derived
   from terrain and the shipped structure lists. The MINOR rides here because it
   is the same defect one clause over: the old gate compared the CLASS, so
   re-attributing EVERY `personal cover` run to *"one lab"* — over a class of 325
   tiles the classifier reads as **link 299 · extension 25 · lab 1** — also passed
   `172/172`, and fails 157/157 now. **MEASURED BOTH WAYS.** Re-applying the round-25 rule to the
   round-26 films makes exactly **210** captions wrong in **149** rooms; forged
   at the producer's run granularity that lands 209 of them on disk and the gate
   names **209 of 209**, failing 145 rooms. Post-rebuild: **0**, and all 436 seat
   captions agree with the frozen-cut flood tile for tile. **The owner's report
   said 158 distinct rooms and the true count is 149** — the tile figures
   (79 / 131 / 210 / 436) were exact and only the room count was not; it is
   corrected here rather than repeated, because both clusters re-derived 149
   independently. 4 mutations.
130. **MAJOR: THE WITHHELD WITNESS'S FREE PARAMETER WAS MOVED, NOT REMOVED — AND
   THE 34 "MYSTERY" ADOPTIONS HAD A RECORD THAT A LATER PASS ERASED.** Criticism
   123 published `withheldTiles` and `cutAtFreeze` and closed the number; what it
   left is that the ANCHOR was shape-checked only (array, non-empty, integer
   x/y), and that only `ec[0]` of the four consumers was re-derived — the other
   three, **516 of the fleet's 688 records**, were held by set containment alone.
   The reviewer landed both halves: appending invented tiles to `ext(L6)` ALONE
   takes the fleet's withheld sum from 2,786 to **37,186** at `pass 172/172`, and
   zeroing the fleet reproduces criticism 123's own condemning sentence
   verbatim — **all 688 records at 0/0/0/0, `pass 172/172 · fail 0`, and the full
   60KB validator output BYTE-IDENTICAL to the clean run**, which is the state
   that criticism condemned, one round after it was declared closed.
   **THE ROOT CAUSE OF THE 29-ROOM DRIFT IS THE GEM OF THE ROUND, AND IT IS THE
   OPPOSITE OF WHAT BOTH SIDES ASSUMED.** The reviewer measured 34 added and 46
   removed cut tiles that appear in NO published roster and concluded no record
   explains them. A record did: `pruneInertRamparts` and `reconcileSeal` each run
   TWICE — once inside `planWallRoads` (layer 7) and once inside `finalizeRoom`
   (layer 7b) — and each call OVERWRITES `plan.shell.cutAdopted`. The EARLY
   reconciliation adopts the 34; the LATE one adopts **0 in 172/172** and
   overwrites the roster with `[]`. `inertPruned` survived only because
   `finalizeRoom` concatenates the two passes by hand. **A record erased by a
   later pass reads exactly like a record that was never written**, which is this
   file's `orphanRoads` lesson arriving through a door nobody had checked.
   `meta.shell.cutDrift` publishes it — `{x, y, op, pass, why}` per move, in
   application order, `[]` in the 143 rooms that never move their cut — and
   **`cutAtFreeze` + adds − removes == the shipped cut in 172/172**, with the 34
   adds all `layer7-reconcileSeal` and the 46 removes all `layer7-inertPrune`
   across 29 rooms. **THE LADDER IS DERIVED AND THE ANCHOR IS BOUND THREE WAYS.**
   All four consumers re-derive from terrain now — `ec[0]` against layer 2's
   bubbles, `ec[1]` adding the ramparts under the towers, `ec[2]` adding the
   labs', `ec[3]` against EVERY shipped rampart — **172/172 each**, with the
   sandwich (`ec[0]`'s derivation a floor, `ec[3]`'s a ceiling) stated separately
   as the weaker claim that survives if a middle layer ever stops being
   reconstructible. The anchor: (i) the flood over it reaches no owned core
   structure, and not one of the first sixty rooms' **2,524** frozen cut tiles
   can be dropped with the core still sealed — so the whole shrink-the-anchor
   family exposes a spawn, and this is a MEASURED property rather than a free one
   (the SHIPPED cut seals only **170/172**); (ii) every freeze-only tile is in
   `meta.shell.inertPruned` and wears no rampart, **46/46**; (iii) the drift log
   REPLAYS, with **every move required to be a real move at the moment it is
   applied**. That last clause is not decoration and the validator cluster caught
   its own escape without it: re-pointing `cutAtFreeze` at the shipped cut
   reconciles perfectly — every add already there, every remove already gone — and
   the anchor is free again in every room whose two floods agree. **The
   self-catch is recorded because the alternative was a gate that looked closed.**
   Every exploit dies: the one-record inflation fails 1/1, the fleet-wide anchor
   swap 29/29, the co-forged fleet zeroing 171/171, the plain fleet zeroing
   165/165 (7 rooms genuinely publish 0/0/0/0), the `ext(L6)` append 172/172, and
   the drift log deleted 172/172. 15 mutations.
131. **MEDIUM + LOW: THE CENSUS THIS DOCUMENT PUBLISHED AS ROUND 25'S HEADLINE
   HAD NO CONSUMER, AND THE REASONS INSIDE IT WERE WRITTEN AS WORLD-FACTS ABOUT A
   CLASSIFICATION THAT IS A FUNCTION OF TEST ORDER.** `grep rampartCensus` over
   `validate.mjs`, `mutate.mjs`, `plan.mjs` and `numeral-audit.mjs` returned
   NOTHING; the round-25 gate read the film's `steps` and never the census beside
   them. Proven on the shipped tree by two forgeries that each passed `172/172`
   with output byte-identical to the clean run: an invented `wumpus` facet at
   **9999** with `crossing` zeroed and its reason replaced by *"THIS ROOM IS
   PERFECT AND SHIPS NO WALL"*, and all **494** `emptyBecause` reasons deleted
   fleet-wide with every room's `crossing` count set to 99999. **This is criticism
   123's own ruling — "a field that survives being set to anything is a
   decoration" — recurring in the channel shipped by the round that wrote it.**
   The LOW is the content: the reasons were sentences about the world, and the
   facets are decided by test ORDER, so 13 rows in 12 rooms were false —
   `seat.inside`'s *"every ramparted container this room ships stands outside the
   shell"* is refuted by a container of that room in 8 rooms, `seat.outside`'s
   *"no container of this room stands outside the shell wearing a rampart of its
   own"* by 5 more, and **E15S4 printed BOTH** over a room shipping two
   containers. The zero was true; the reason for it was not, because `crossing`
   had claimed the tile first. **Both halves are fixed by the same move: derive
   the reason from which of the two things actually happened.** Each facet now has
   an INTRINSIC test with no order in front of it, and an empty row either states
   the absence its zero really proves — with the room's own counts — or names the
   earlier facet that absorbed its tiles AND LISTS THEM, so a gate can re-run the
   intrinsic test and compare the roster exactly. The census gained a consumer
   that believes none of it: facet list held to the taxonomy's order (an invented
   facet fails on the ROW, before its count), `class` derived, `count` re-derived
   over the shipped rampart list, `captions` re-rendered per tile and compared as
   a MULTISET, and `emptyBecause` required exactly when the count is 0 with its
   content derived. **1,204 rows, 555 empty, 555 reasons, and 41 DISTINCT strings
   over 29 absorbed rows and 526 absence rows** — where round 25 shipped 494 rows
   over 7 constants, and where this document verified only that a reason was
   PRESENT. E15S4's contradictory pair is gone and both of its rows are true. 7
   mutations.
132. **MEDIUM + LOW: A "PER-RECORD CENSUS" NOTHING PARSED, AND THE SAME
   CONSTANT-BASIS DEFECT SHIPPED IN THE SAME BUILD THAT FIXED IT.** (a) Criticism
   127(c) took `preTakeShortfallBasis` from 1 distinct string to 24 by putting a
   census in front of the constant rule text — and the gate on it is
   `typeof === "string" && length >= 80`, so the census's count, its classes in
   declaration order with indices, its key/skip names and its `k + s = n against
   N` arithmetic were compared with NOTHING. All 134 basis strings replaced with
   an invented unicorn/rainbow census: `pass 172/172`, exit 0, output
   byte-identical. The producer's own header claims *"a record whose key set does
   not cover its own shortfall list now contradicts itself in prose as well as in
   arithmetic"* — true, and unread, and the MM3 residue sweep leaves **30 records
   contradicting their own `preTakeShortfalls` verbatim** and passing. The basis
   is PARSED against the record it is printed on now; the forgery fails 64/64 of
   the rooms carrying one and the eco-skip sweep fails 18/18. (b) The same round
   shipped `meta.exteriorContractBasis` **byte-identical in 172/172 rooms** — a
   constant `EXTERIOR_CONTRACT_BASIS` in `shared.mjs`, a plain `const` and not a
   template — while fixing exactly that defect one field over, and this document's
   own worked example for it (E11S1's `[10,10,10,11] = 41` over 11 distinct tiles,
   which re-derives EXACT) existed only in the document. It carries the room's own
   census first — consumer count, each consumer's `at`, withheld count, withheld
   tile roster and exposure, the sum of readings and the DISTINCT union with its
   roster — and the constant rule text as the SUFFIX, mirroring the `preTake` fix
   exactly: **172 records, 166 distinct strings**, and the validator parses the
   census against `meta.exteriorContract` rather than measuring its length.
   Reverting it to one constant fails 172/172. 6 mutations.
133. **MINOR + LOW: A BLIND-SPOT CLASS THE NUMERAL GATE COULD NOT SEE AT ALL, AND
   THREE READER CHANNELS THAT HAD TO CONTAIN THE TRUTH RATHER THAN EQUAL IT.**
   (a) All five of `numeral-audit.mjs`'s claim shapes required DIGITS, so
   **spelled-out numerals were an unnamed blind spot**, and round 25's own
   comments had three live wrong ones in it — `shared.mjs:320`,
   `validate.mjs:12958` and `pipeline.mjs:108` each say *"seven"* rooms where
   layer 7 moves the cut and the fleet says **29** — in the same round whose
   criticism 123 publishes the correct 29. A fourth site,
   `validate.mjs:10396`, named *"the three rooms that DO declare (E16S3, E17S5,
   E8S7 — all three at 3 seats)"* against a fleet where FOUR rooms file a
   `ctrlParks/seats` declaration (E12S5, E17S5, E8S2, E9S2), two of the three
   named file none, and the rosters are 6/6/5/8. **THE THRESHOLD IS THE HONEST
   PART OF THIS FIX AND IT IS MEASURED, NOT CHOSEN.** Reading the vocabulary
   without restriction manufactures **25 unowned claims out of ordinary English**
   — *"the fleet's four WORST rooms"*, *"the fleet ships one today"* — against
   zero real ones, so a spelled-out match becomes a claim only when its noun
   resolves to a registry quantity, and the CENSUS counts spelled-out occurrences
   at the same ten-and-over threshold the digit census already argues for. The
   cost is stated rather than implied: a wrong spelled-out figure about a noun
   with no registry extractor is still invisible, and the run publishes the size
   of that residue every time (**95** occurrences today). Three of the four sites
   are corrected to 29 with the reason attached and a pointer at `cutDrift`, and
   **the fourth was DELETED rather than refreshed**, because the
   containment it was illustrating is re-derived per room off `parksOf(ctrlLink)`
   and the honest fix is to derive the thing and type no roster. Audit today:
   **192 claims · 131 re-derived · 61 waived at 37 sites · 0 unowned · 0 WRONG**,
   exit 0. (b) The three unjudged channels criticism 121 closed were held by
   `String(v).includes(want)` — containment, not equality — so a false clause
   appended to E7S5's film caption, its page's `mob-sub` line and its index chip
   title at once passed with 0 caption failures. The wrapper grammar is stated
   and the comparison is whole-value now (`page = LEAD + reason`,
   `chip = LEAD + reason + CHIP_TAIL`, and the film's span taken between the
   note's own two markers rather than by a lazy regex), so there is no place left
   in any of the three for a clause; each channel bites alone. Filed LOW because
   criticism 121 did say "contain" — the containment was declared — but its own
   summing-up reads *"one room, one question, one answer"* and that is a pinned
   sentence. 5 mutations.
134. **STILL OPEN — ROUND 26'S OWN RESIDUE LIST, WRITTEN UNDER THE RULE THAT
   EVERY RESIDUE THIS DOCUMENT HAS FILED IN FOUR ROUNDS HAS BEEN TOO FLATTERING
   TO THE GATE IT DESCRIBES.** Five items, each with its measurement or its
   admission that there is none.
   (a) **`cutDrift`'s ADD side has no independent board roster, and the closure
   under it is measured rather than proved.** The removals are bound to
   `meta.shell.inertPruned` (46/46); the adoptions are bound only by the replay
   arithmetic, the real-move rule and the core-seal test. That test closes the
   family completely ON THIS FLEET — 0 of 2,524 frozen cut tiles droppable — but
   it is a property of these rooms and not a theorem: a room carrying a redundant
   frozen cut tile would let a forger shrink the anchor by one, log a matching
   `add`, re-derive all four rosters and INFLATE its withheld readings. The
   direction is conservative (it can only over-report withholding) and the
   producer-side view of the same gap is (b).
   (b) **The 34 early-pass cut adoptions are UNDECLARED, and that is a DESIGN
   QUESTION for round 27 rather than a defect this round declined to fix.**
   `declareAdoptedSeal` fires only off the layer-7b reconciliation, which adopts
   0 in 172/172; the layer-7 one adopts 34 across 29 rooms and says nothing in
   the declaration channel. `cutDrift` makes them visible without declaring them.
   Making them declare would move the fleet's **300-declaration** total, which
   this document states in fifteen places and a numeral-audit extractor twins, so
   it is stated as a question with its cost rather than smuggled into a
   reader-channel round.
   (c) **`ec[1]`/`ec[2]`'s exact derivation is an empirical identity, not a
   published contract.** The tower/lab ladder reproduces 172/172 today; if a
   future layer rents cover for something else between L3 and L6 it needs
   re-deriving. The sandwich underneath is checked separately, so the failure
   mode is a loud false failure rather than a silent hole — and that is the
   argument, not a promise.
   (d) **The census gate RE-IMPLEMENTS `rampartClassifier`'s test order rather
   than importing it, deliberately, and that makes it a contract and not a
   derivation.** Importing the classifier would make this an agreement test
   again — the exact lesson of criticism 129 — so the order is re-stated on the
   validator side, where a change to the producer's order fails loudly. It is
   written in the comment; it is not derived from anything, and a reader should
   know which of the two it is.
   (e) **88, 93 and 98 remain open**, re-confirmed by exploit rather than by
...
- Mutations: `.../mutate.mjs` (**1309/1309 bite** against a clean 172/172
  ...
  forges a shipped film, page or chip, runs the gate against the room's unmutated
  plan, restores the bytes in a `finally` and FAILS if the restore is not
  byte-identical or if the forgery changed nothing; 11 of the 37 `r26` cases use
  it. The coupling it implies is still open at criticism 134(a).
