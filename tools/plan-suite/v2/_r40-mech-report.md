# Round 40 mechanical review

Artifact `tools/plan-suite/out-v2/plans-hub.json` md5 `8e5d6725885bd3f3731379bedf408326` (27,780,512 bytes) — **matches expected**. HEAD `e469b99` (r29p18: derive leftover presence — `maxHubDist` layer-6 hub-field max of the pre-7b seats, `takeTowerSwap.from` D8 of `to`). Artifact bytes unchanged since r32. p18 is a gate + mutate-corpus pass (`r27-gates.mjs`, `mutate.mjs`). Rooms dump `tools/plan-suite/v2/_r28-mech/rooms.json` (172/172, 0 missing). Node `v22.13.0` (`fnm` install on PATH). Probes in `tools/plan-suite/v2/_r40-mech/`. No suite source committed. Artifact not written. **No board moved.** No `boards` field. Physicals still 10,320 / 14,100 / 8,208.
...
**Exploit.** E11S1 is a real shrink. After r29p10 the suffix is gone: `fullRun.reserved === lane.reserved` (6 tiles / 2 rounds), `shrunk.wanted=7`. The refused extra is a count. p18 did not touch this.
...
**Exploit.** E11S2 rungs: `1.56/46/37 ×3`, then `2.5/50/42` (last, eco-capped). Shipped wall 46/37 = freeze. Last fat is complete, 42 tiles, has `20,9` and `29,33`. p18 did not touch this.
...
**Exploit.** 39 presence names, 60 derived (p18 moved `maxHubDist`; p17 moved four: `floorUngated` / `radii` / `parkCap` / `takeTowerSwap`; `stubCap` was already derived in p15 and is now the pool walk). Zeroed every remaining `klass:"presence"` numeric/bool/nonempty-array leaf across all 172 rooms. 17 kinds, 1,250 events. `checkRoom`: **fail 172/172**, every first fail is `baseCut := 0`.

Exclude `baseCut` only: **fail 3/172** — first fail is the `shallowNow` note (E12S6, E2S3, E9S2). Exclude `baseCut`+`shallowNow`: **pass 172/172**. Same 172/172 if those two plus all newly derived names (p12–p18) are skipped. Same 172/172 if those two plus **every** `klass:"derived"` name are skipped (15 leftover kinds, 1,075 events). The r38 "minus derived+baseCut+shallowNow is silent" door is still open.
...
Closed this artifact (re-probed, BITES): `nukerInWindow`, `center`, `mineralSeatNetTiles`, `coveredDetourDeclared`, **p12 five**, **p13 three**, **p14 nine** + `boundLap`, **p15 ten**, **`seedScore`**, **p16 four**, **p17 five**, **p18 two** (`maxHubDist` walk / `takeTowerSwap.from` D8 of `to`).
...
**Root cause.** Criticism 141(c). They derived the cheap leaf this review named. They bound `maxHubDist` to the pre-7b hub-field max (a real walk) and `takeTowerSwap.from` to the D8 ring of `to` (a topology bound, not the vacated seat). The class is still the class.
...
### 4. Exact `baseCut` / `protectRadius` / `hubDistCap` picks are unread. `maxHubDist` / `stubCap` 43↔51 are closed.
...
Sample: E8S1 `baseCut=60` = cut (pricey). E17S8 `baseCut=31` / cut=35. E13S3 `baseCut=43` / cut=44 / freeze=46.
...
### 7. p18 two — named flatten BITES; `from` is a ring, not the vacated seat.

| attack | result |
|---|---|
| `maxHubDist := 0` | **BITES** (E11S1 walk 15) |
| `maxHubDist += 1` | **BITES** (16 vs 15) |
| `takeTowerSwap.from → 1,1` | **BITES** (not D8 of `to` 34,24) |
| `takeTowerSwap.from` onto a live tower | **BITES** |
| `takeTowerSwap.to` → another shipped tower, `from` left | **BITES** (breaks the D8 ring — not a destination bind) |
| `takeTowerSwap.from` → another D8 of `to` | **ESCAPE** |
| `to` → another shipped tower **and** `from` → a D8 of the new `to` | **ESCAPE** |

`maxHubDist` is the layer-6 hub-field max of the seats this room stood before the 7b reflow. `takeTowerSwap.from` is some D8 neighbour of `to`, not the vacated seat. `takeTowerSwap.to` is still some shipped tower once `from` is walked with it.

### 8. p17 five / p16 four / p15 ten — named flatten BITES; identities remain.
...
Independent single-removal over all **7,246** `cutAtFreeze` tiles: **0 loose**, 0 core-seal fails, 29 rooms differ from the shipped cut, 34 add + 46 remove, 27 adopt / 29 drift. Same census as r29–r39.
...
## Attacked and held (r39 closures + r29p18 named, re-probed)
...
`h(room) = fmix32(fnv1a32("round40-mech|" + room))` over the 172 names. Lowest five (not cherry-picked):
...
|---|---|---|---|---|
| E8S1 | 60/78/68/62/62 | 0 | 7,44 off | hashed #1, seed=hub, **pricey** 60, `protectRadius=8` |
| E14S3 | 60/70/80/70/70 | 0 | 26,34 off | **pricey** 70, seed `18,31` ≠ hub `20,31` |
| E17S8 | 60/69/39/35/35 | 0 | 23,18 off | pick 31 ≠ cut 35, seed `17,40` ≠ hub `15,41` |
| E16S5 | 60/87/60/52/52 | 0 | 42,19 off | **pricey** 52, seed=hub, `protectRadius=8` |
| E13S3 | 60/98/47/44/46 | **3.33** | 19,19 off | pick 43 ≠ cut 44 ≠ freeze 46, shrink 9 / wanted 20, seed=hub |
...
Nothing in the sample looks accidental. The expensive rooms say so (E13S3 lap 3.33 + shrink + pick-not-cut, E11S2 lap 1.5, E11S7 lap 8.67 + take, E8S1/E14S3/E16S5 pricey). Three of the hash five publish a `baseCut` that is not a free integer — two are pricey equals, one is a pick that is not the shipped cut.

---

## Clean re-derivations (terrain + shipped structure lists)

From `_r40-mech/rederive.mjs` (does not import `validate.mjs`):
...
Findings 1–3 are the round. r29p18 closed the *named* leftover flatten (`maxHubDist` walk) and bound `takeTowerSwap.from` to the D8 ring of `to`. The prefix is still a neighbor list. The last discarded rung is still a free enclosure that seals. A closer who only adds another identity over producer leaves will ship this review again.
...
6. **p18 take residues.** E14S1: `takeTowerSwap.from` onto another D8 of `to` (this probe: `34,25 →` another neighbour of `34,24`). `to` onto another shipped tower **and** `from` onto a D8 of the new `to` (`to=32,27` / `from=33,27`).
7. **p17 radii table.** E18S2: set `needDeepBonus := 25` **and** `radii := RADII_WIDE`.
...
**No.** Named p18 two leftover flattens / named p17 five leftover flattens / named p16 four leftover flattens / named p15 ten leftover walks + `seedScore` / named p14 nine + `boundLap` / named p13 three + stitch 0/1 / named p12 five / named 88 leaky+`complete=false` / named 98 `19,27` on one copy / 88 leaky complete / named 98 COORD bag / 88 shipped-cut / 88 8-tile box / 93 plant-back closed. The kept prefix is a D8-neighbor bag. `wanted` is a count. The last discarded rung is a free sealing enclosure. The presence class still flips once `baseCut`+`shallowNow` are left alone. `hubDistCap` / `protectRadius` are enums. `takeTowerSwap.from` is a D8 of `to`, not the vacated seat. `takeTowerSwap.to` is a tower plus a matching D8 `from`. `radii` is a bonus table. The reserved mineral pair is a topology bound. `clean_rounds` is 0. The bar is 2 consecutive all-clean. Boards did not move.
