...
Artifact md5 `8e5d6725885bd3f3731379bedf408326` — matches the brief. Same bytes as r32–r40. Tree HEAD is `c004ef4` (r29p19: `takeTowerSwap.from` the published take / `unreachedClusters` shipped-cut clusters ≥2 with no road D8 / `unreachableExts` extensions with no D4 road / `servedExts` 0 iff the filler laid nothing), parent `e469b99` (r29p18 maxHubDist walk / takeTowerSwap.from D8 of to). 172/172 rooms, 0 errors. Fleet physicals re-summed off the structure lists: **10,320 extensions · 14,100 roads · 8,208 ramparts · 300 declarations · 236 notes**. Same numbers as r26–r40. Boards did not move. Baseline `checkRoom` (fleet-median stripped): **pass 172/172**.

**Verdict.** No board defect in the sample. No film-vs-board caption lie in the sample. 134(b) is still 34 adds / 27 rooms. r29p19 closes the *stated* leftover flattens: `takeTowerSwap.from` other D8 of `to` **BITES** (fleet 3/3), `unreachedClusters += 1` **BITES** (fleet 172/172), `unreachableExts += 1` **BITES** (fleet 172/172), `servedExts += 1` **BITES** (fleet 172/172). `seedScore` still **BITES**. The p18 two + p17 five + p16 four + p15 ten + p14 nine + p12 five still **BITES**. The named 88 leak + `complete=false` still **BITES**. The named 98 one-list append still **BITES**. They do **not** close the classes. Twin of the named 98 append (also write `lane.reserved`, keep `wanted > tiles`) **ESCAPE**. `wanted += 1` **ESCAPE**. Sealing same-lap nudge `29,33→28,34` **ESCAPE**. `hubDistCap` 16→19 in-enum **ESCAPE** (fleet 170/170). Remaining META_DARK presence still flips 172/172 silent once `baseCut`+`shallowNow` are left alone. Exact pick still unread. `protectRadius` is still an enum. `hubDistCap` is still an enum. `takeTowerSwap.from` is now the published take, not any D8 of `to`. `unreachedClusters` / `unreachableExts` are now walks of this fleet's zeros. `servedExts` is now the filler-iff, not a service count. `cutPasses.rampartsDeleted` split still a log.

Throwaway probes: `tools/plan-suite/v2/_r41-owner/` (not committed).

---

## Sample

`h(room) = fmix32(fnv1a32("round41-owner|" + room))` over the 172 names. Lowest five (not cherry-picked):
...
The 27 adopting rooms (same set as r28–r40):
...
One of this hash sample adopts: **E6S5** (remove `23,5` / add `22,6`).
...
## Closed since r40 — re-probed

**r29p19 / four leftover presence names.** Mutated on rooms that ship them:
...
Fleet: `takeTowerSwap.from` other D8 **fail 3**, pass 169 (the 169 rooms with no swap were not mutated). `unreachedClusters += 1` **fail 172/172**. `unreachableExts += 1` **fail 172/172**. `servedExts += 1` **fail 172/172**. The named flatten is done.

Residues of the named close, not the named flatten:

| attack | result |
|--------|--------|
| `takeTowerSwap.from` other D8 + rewrite `acrossPriorTake.taken` / `towerSwapTaken` / `offer.best` | **BITES** (noteRecords still names the old take) |
| those twins + `noteRecords` + regen the note | **BITES** (`towerSwapOffer.basis` prose) |
| `servedExts += 1` and `fillerTiles += 1` | **BITES** (`fillerTiles` is `extFace`) |
| `servedExts` + `fillerTiles` + `extFace` += 1 | **BITES** (laid book: 1 laid, 0 ship with provenance) |
| `unreachedClusters := 0` / `unreachableExts := 0` / `servedExts := 0` | no-op (fleet already 0) |

`takeTowerSwap.from` is no longer any empty D8 of `to`. It is the published take (`acrossPriorTake.taken`, else the offer whose destination is a shipped tower). Moving it to another D8 of the destination fails. That is a twin of the published pair, not a vacated-seat walk off the board. `unreachedClusters` is the shipped-cut clusters of size ≥ 2 that no road D8-touches — this fleet has **0**. `unreachableExts` is the extensions with no D4 road — this fleet has **0**, matching the independent census. `servedExts` is **0 iff the filler laid nothing**. This fleet's filler laid nothing (172/172 `fillerTiles=0`, `extFace=0`), so `servedExts` is a constant 0. The two walks are walks. The iff is an iff.
...
Fleet: `maxHubDist := 0` **fail 172/172**. `takeTowerSwap.from → 1,1` **fail 3**.
...
Fleet: `hubDistCap := 17` **fail 172/172**. `hubDistCap` 16→19 on all 170: **pass 172/172**. `corridorPlaced := 0` **and** `fallback := 1` **ESCAPE**.
...
Fleet `seedScore := 0`: **fail 172/172**. Seat+approach moved together to another cheb-1 / D8 pair still **ESCAPE**. Approach to another D8 of the same seat still **ESCAPE**.
...
**Shallow extensions** re-derived against live depth < 4: **0** in all 8. Published 0. E19S6's and E11S1's `shallowExt` notes are search notes.
...
**Lab diamond:** 10 labs, 4×4 bbox, 6 holes in all 8. Mandated stamp. Haul cheb 2–5. E19S6 / E11S1 haul 5, E6S5 haul 4, E3S9 / E2S7 / E1S4 haul 3, E14S5 / E12S2 haul 2.
...
**E19S6.** 44-cut, 45 ramparts, 71 roads. `baseCut=44` = freeze = cut. `protectRadius=12`. Mobility 0. Controller enclosed. Mineral `44,23` off-network, declared (seat `43,24`). Seed=hub `12,9`. `seedScore=3.7`. Dropped reservation. 0 2×2. 0 D4-blind. Labs haul 5. Film: 44 crossing / 1 seat.outside. Hash #1 is a dropped room with a positive seed score and a 44-tile wall the room does not price.

**E14S5.** 28-cut, 36 ramparts, 101 roads. `baseCut=30 ≠ 28`. `protectRadius=12`. Mobility 0. Controller unenclosed. Mineral `16,26` **on** the network (seat `17,27`). Seed=hub `28,33`. `seedScore=-70.2`. 4 redundant cut tiles, all explained (load-bearing on interior floor). Sealed-recovery `allRefused` 2/4. Weak-battery + 26-tile controller eco, both declared. Labs haul 2. 0 2×2. 0 D4-blind. Film: 28 crossing / 2 seat.inside / 2 seat.outside / 3 cover / 1 ring. The mineral being on-network is the one thing this sample does not have to excuse. The pick is 30 against a freeze and a shipped cut of 28.

**E6S5.** 36-cut, 38 ramparts, 75 roads. `baseCut=34 ≠ 36`. `protectRadius=11`. Mobility 0. Controller enclosed. Mineral `25,24` off-network, declared (seat `24,23`). Seed=hub `9,21`. `seedScore=-22.4`. Adopts: remove `23,5` / add `22,6`. Labs haul 4. 0 2×2. 0 D4-blind. Film: 36 crossing / 1 cover / 1 seat.outside. The pick is 34 against a freeze and a shipped cut of 36. Hash #3 is a 134(b) room and an unread pick.

**E12S2.** 65-cut, 69 ramparts, 76 roads. `baseCut=65` = freeze = cut (pricey). `protectRadius=8`. Mobility 0. Controller enclosed. Mineral `20,19` off-network, declared (seat `19,18`). Seed=hub `26,27`. `seedScore=-21.6`. Labs haul 2. Tower clump 4. 0 2×2. 0 D4-blind. Film: 65 crossing / 2 seat.outside / 2 cover. The 65-tile wall is the ugliness the room does not price.

**E3S9.** 39-cut, 42 ramparts, 75 roads. `baseCut=39` = freeze = cut. `protectRadius=10`. As-built gated lap **2.57**, declared. Controller enclosed. Mineral `38,30` off-network, declared (seat `37,31`). Seed `36,16` ≠ hub `35,18`. `seedScore=-13.5`. Labs haul 3. 0 2×2. 0 D4-blind. Film: 39 crossing / 1 seat.outside / 1 seat.inside / 1 cover. Escalation last fat `4/50/48` against shipped `2.57/42/39`. Hash #5 is a declared 2.57 lap and an unread last fat.

### Mandated churn

**E11S1.** 32-cut, 43 ramparts, 101 roads. Real shrink: `fullRun.reserved` === `lane.reserved` === 6 tiles / 2 rounds. `wanted=7`, `used=3`, `ext=60`, `shallow=3`. Suffix COORD list is gone. Seed `25,39` ≠ hub `24,41`. `seedScore=-92.9`. Controller unenclosed. Mineral `23,7` off-network, declared (seat `24,8`). Eco 27-tile controller walk, declared. 3 2×2 on a road spine. 0 D4-blind. Labs haul 5. Film: 32 crossing / 4 seat.outside / 3 cover / 4 ring. Reserved board: `18,27 19,28 19,29 | 24,28 25,27 25,29`. `19,27` is D8 of `18,27` and `19,28` and is not in the set. The 98 residue room, mandated. `maxHubDist=15` — still the p18 witness.

**E2S7** (golden). 18-cut, 21 ramparts, 72 roads. `baseCut=18` = freeze = cut. `protectRadius=12`. Clean pocket. 0 redundant. Mineral `33,4` off-network (seat `32,3`). Seed=hub `13,27`. `seedScore=-21.5`. Labs haul 3. Quiet.
...
No sampled room is a checkerboard, a solid brick, or a maze. Extension mass is corridor-flanked (**0 D4-blind extensions in all 8**). 2×2 squares exist only in E11S1 (3) and they sit on a road spine. E19S6 / E14S5 / E11S1 spend roads on eco, not on a city grid. Towers are not a hub clump except E12S2 (clump 4); the rest of the sample clumps 0–2.

Nothing in the sample looks accidental. The ugly rooms are ugly for a named, declared reason (E19S6 dropped reservation + 44-tile wall, E14S5 26-tile controller + weak battery, E12S2 65-tile pricey wall, E3S9 2.57 lap, E11S1 27-tile controller walk) — except the unread pick on E14S5 (`baseCut=30` against a 28-cut) / E6S5 (`baseCut=34` against a 36-cut) / E1S4 (`baseCut=24` against a 26-cut) and the unread last fat on E3S9 (`4/50/48` against a shipped 39), which the rooms do not price as compositions.
...
**Exploit.** E11S1 after r29p10/p11/p12/p13/p14/p15/p16/p17/p18/p19: `fullRun.reserved` === `lane.reserved` === 6 tiles / 2 rounds. `wanted=7`. The refused tail is a count.
...
r29p19 did not touch this. A tile that *is* this room's floor and *touches* the walk can still be added, or swapped for a kept prefix tile, if both lists stay twins. The refused walk is now four unread integers (`wanted`, `used`, `ext`, `shallow`) plus `premium`.
...
`discardedCutSeals` no longer trusts `row.complete`. A leak bites either way. That is the named close. A 42-tile nudged cut that *does* seal is longer than the shipped 37, so the would-have-taken rule returns before it looks. Any other 42-tile COORD list that walks the claimed lap and seals passes. Flipping `complete` on a sealing rung is free. This sample has one more last fat of the same class (E3S9 `4/50/48`).

**Root cause.** Discarded `cutTiles` are a free list held to `enclosureMobility` of themselves, plus "not freeze/shipped", plus "not shorter-and-prettier", plus "seals". The composed enclosure is not on the board. r29p11 gated leaks on the flood, not the flag. It did not bind the list. r29p19 did not touch this.
...
**Exploit.** Zeroed every remaining `klass:"presence"` numeric/bool/array leaf across all 172 rooms. 36 presence names, 63 derived (r40 was 39 / 60; p19 moved `servedExts` / `unreachableExts` / `unreachedClusters`). 17 kinds, 1,250 events. `checkRoom`: **fail 172/172**, every first fail is `baseCut := 0`.

Exclude `baseCut` and `shallowNow`: **pass 172/172**. 15 kinds, 1,075 events, silent. Same 172/172 if those two plus all newly derived names (p12–p19) are skipped. The r39 "minus those two is silent" door is still open — p19 reclassified three leaves that were already 0 fleet-wide. They were never in the 1,250-event flatten. It did not derive the class.

Single-room ESCAPE, flattering direction: `deepBudget` decrement-keep-nonzero / cleared. `spurred` decrement 5→4 still ESCAPE — and `spurred` is classified `derived`. `hubDistCap` 16→19 **ESCAPE** — classified `derived`, only an enum. `corridorPlaced := 0` + `fallback := 1` **ESCAPE** — classified `derived`, only an iff.

r29p3 / r29p9 / r29p12 / r29p13 / r29p14 / r29p15 / r29p16 / r29p17 / r29p18 / r29p19 derived names still BITES. The class is still the class. `protectRadius` is *classified* `derived` and is only an enum. `hubDistCap` is *classified* `derived` and is only an enum. `servedExts` is *classified* `derived` and is only an iff against a fleet-constant 0.

**Root cause.** Criticism 141(c). They derived the four cheap leaves this review named. Two of them (`unreachedClusters`, `unreachableExts`) are real walks of this fleet's zeros. One of them (`takeTowerSwap.from`) is now the published take. One of them (`servedExts`) is an iff. The rest are still comments.

**Board moved?** No.

### M5 — MEDIUM, not board-affecting

**Exact `baseCut` / `protectRadius` / `hubDistCap` pick is unread. `seedScore` is not. `stubCap` is not. `maxHubDist` is not. `unreachedClusters` is not. `unreachableExts` is not.**

`protectRadius := 0` **BITES** (not in `RADII_WIDE`). `12 → 6` inside the enum **ESCAPE**. `baseCut := 0` **BITES**. `baseCut += 1` keeping `priceyWall` on the same side of `MAX_CUT=45` **ESCAPE**. `hubDistCap := 17` **BITES** (not in `[16,19,23,999]`). `16 → 19` inside the enum **ESCAPE**. `seedScore := 0` **BITES**. `seedScore += 999` **BITES**. `stubCap` 43→51 **BITES** (the pool walk). `maxHubDist := 0` / `+= 1` **BITES** (the pre-7b walk). `unreachedClusters += 1` **BITES**. `unreachableExts += 1` **BITES**.

Sample: E19S6 `baseCut=44` = cut. E14S5 `baseCut=30` / cut=28. E6S5 `baseCut=34` / cut=36. E12S2 `baseCut=65` = cut (pricey). E3S9 `baseCut=39` = cut. E1S4 `baseCut=24` / cut=26. The pick is not freeze and not the shipped cut. `protectRadius` is an enum, not this room's pick. `hubDistCap` is an enum, not this room's cohesion rung. `seedScore` is the window max. `maxHubDist` is the walk. 46 rooms still have seed ≠ hub. The coordinates exist. The score is the walk. The cap is four legal integers.
...
- Remaining META_DARK presence (36 names, 172/172 silent once `baseCut`+`shallowNow` are left alone)
