# Round 34 mechanical review

Artifact `tools/plan-suite/out-v2/plans-hub.json` md5 `8e5d6725885bd3f3731379bedf408326` (27,780,512 bytes) — **matches expected**. HEAD `4c68a10` (r29p12). Planner sources and this artifact are unchanged since `1d7ca82` / r32 (p12 is a gate + mutate-corpus pass). Rooms dump `tools/plan-suite/v2/_r28-mech/rooms.json` (172/172, 0 missing). Node `v22.13.0` (`fnm` install on PATH). Probes in `tools/plan-suite/v2/_r34-mech/`. No suite source committed. Artifact not written. **No board moved.** No `boards` field. Physicals still 10,320 / 14,100 / 8,208.
...
A gate that agrees with itself is not a gate. r29p12 closed the *named* leftover presence walks (`stitched` / `stitchTiles` / `roadsEaten` / `towerOnly` / `stubRoads`). r29p11 closed leaky+`complete=false`. r29p10 closed one-copy `"19,27"`. The kept prefix is still a D8-neighbor bag. `wanted` is still a count. The last discarded rung is still a free enclosure that walks its own lap. The presence class still flips.

---

## BLOCKING

None on the board. r28 mineral-why / swap-offer doors stay closed. 93 plant-back stays closed. 88 leaky + `complete=false` stays closed. Named 98 one-copy stays closed. The five p12 walks stay closed on the named flatten.
...
**Exploit.** 71 presence names, 28 derived (p12 moved five: `stitched`, `stitchTiles`, `roadsEaten`, `towerOnly`, `stubRoads`). Zeroed every remaining `klass:"presence"` numeric/bool/nonempty-array leaf across all 172 rooms. 39 kinds, 3,964 events. `checkRoom`: **fail 172/172**, every first fail is `baseCut := 0` (three rooms also `shallowNow` note-identity).

Exclude `baseCut` only: **pass 169/172**. The three fails are `shallowNow` note identity (E12S6, E2S3, E9S2). Exclude `baseCut`+`shallowNow`: **pass 172/172**. Same 172/172 if those two plus the five p12 names are skipped (the five are already off the presence walk).

Single-room flatten, flattering direction — **33 ESCAPE** on the first copy:

`baseOverGated`, `boundHeld`, `boundLap`, `corridorFallback`, `corridorPlaced`, `deepExhausted`, `deepReach`, `digRoads`, `floorGated`, `floorOver`, `floorOverGated`, `freeDin`, `freeLeft`, `haulCost`, `hubDistCap`, `inertPromoted`, `lapCeilingFloor`, `massAdds`, `maxDist`, `maxHubDist`, `minDmgArray`, `minDmgPicked`, `mineralContainer`, `parkCap`, `radii`, `rescuedLap`, `servedFree`, `shallowCost`, `shallowRamparts`, `shallowRefused`, `stubCap`, `stubExhausted`, `wasLap`
...
Closed this artifact (re-probed, BITES): `nukerInWindow`, `center`, `mineralSeatNetTiles`, `coveredDetourDeclared`, **`stitched`**, **`stitchTiles`**, **`roadsEaten`**, **`towerOnly`**, **`stubRoads`**.

`stitched := 1` on E11S9 (laid.stitch=1, shipped stitched>0) **ESCAPE**. The p12 stitch gate is polarity (`=== 0` vs laid), not the count. `stitchTiles` is the count. Named zero still BITES.

`baseCut` is still *classified* `presence` and is only bounded (`>= 1` + `priceyWall` sync). The klass lies.

**Root cause.** Criticism 141(c). They derived the five cheap walks this review named. The class is still the class.
...
Sample: E8S5 `baseCut=72` = cut (pricey). E18S8 `baseCut=32` / freeze=32 / cut=31 (adopter). E19S1 `baseCut=49` = cut (pricey). The pick is not freeze and not the shipped cut.
...
Independent single-removal over all **7,246** `cutAtFreeze` tiles: **0 loose**, 0 core-seal fails, 29 rooms differ from the shipped cut, 34 add + 46 remove, 27 adopt / 29 drift. Same census as r29–r33.
...
## Attacked and held (r33 closures + r29p12 named, re-probed)
...
| **`stitched` / `stitchTiles` / `roadsEaten` / `towerOnly` / `stubRoads` flattened** | **BITES** |
...
`h(room) = fmix32(fnv1a32("round34-mech|" + room))` over the 172 names. Lowest five (not cherry-picked):
...
|---|---|---|---|---|
| E3S9 | 60/75/42/39/39 | **2.57** | 37,31 off | seed `36,16` ≠ hub `35,18` |
| E8S5 | 60/86/81/72/72 | 0 | 27,25 off | **pricey** 72, seed=hub, `protectRadius=8` |
| E19S1 | 60/78/51/49/49 | **2.27** | 43,23 off | **pricey** 49, seed `27,35` ≠ hub `28,35` |
| E18S8 | 60/69/33/31/32 | 0 | 10,35 off | **adopter** freeze≠cut, `baseCut=32`, seed=hub |
...
Nothing in the sample looks accidental. The expensive rooms say so (E3S9 lap 2.57, E8S5 72-cut pricey, E19S1 lap 2.27 + pricey, E11S2 lap 1.5, E11S7 lap 8.67 + take, E19S2 taken).

---

## Clean re-derivations (terrain + shipped structure lists)

From `_r34-mech/rederive.mjs` (does not import `validate.mjs`):
...
3. Remaining META_DARK presence: derive the cheap flattering ones (`corridorPlaced`, `stubCap`, hub-kit bookkeeping that is a walk) or stop shipping them as evidence. Reclassify `baseCut` — the klass still says `presence`. Twin-identity is not a derivation. `stitched` is polarity; if the count matters, bind it like `stitchTiles`.
...
Findings 1–3 are the round. r29p12 closed the *named* five leftover walks. The prefix is still a neighbor list. The last discarded rung is still a free enclosure that seals. A closer who only adds another identity over producer leaves will ship this review again.
...
8. **`stitched` polarity.** E11S9: `meta.walls.stitched = 1` while `laidByKind.stitch ≥ 1`. Named zero BITES; any positive integer is free.

---

## Could this be a clean round?

**No.** Named p12 five leftover walks / named 88 leaky+`complete=false` / named 98 `19,27` on one copy / 88 leaky complete / named 98 COORD bag / 88 shipped-cut / 88 8-tile box / 93 plant-back / four cheap presence walks closed. The kept prefix is a D8-neighbor bag. `wanted` is a count. The last discarded rung is a free sealing enclosure. The presence class still flips. `clean_rounds` is 0. The bar is 2 consecutive all-clean. Boards did not move.
