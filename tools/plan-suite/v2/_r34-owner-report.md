...
Artifact md5 `8e5d6725885bd3f3731379bedf408326` — matches the brief. Same bytes as r32–r33. Tree HEAD is `4c68a10` (r29p12), matching the brief. 172/172 rooms, 0 errors. Fleet physicals re-summed off the structure lists: **10,320 extensions · 14,100 roads · 8,208 ramparts · 300 declarations · 236 notes**. Same numbers as r26–r33. Boards did not move. Baseline `checkRoom` (fleet-median stripped): **pass 172/172**.

**Verdict.** No board defect in the sample. No film-vs-board caption lie in the sample. 134(b) is still 34 adds / 27 rooms. r29p12 closes the *stated* five leftover presence names: `stitched` / `stitchTiles` / `roadsEaten` / `towerOnly` / `stubRoads` zeroed now **BITES** (fleet 172/172 when those five are flattened). The named 88 leak + `complete=false` still **BITES**. The named 98 one-list append still **BITES**. They do **not** close the classes. Twin of the named 98 append (also write `lane.reserved`, keep `wanted > tiles`) **ESCAPE**. `wanted += 1` **ESCAPE**. Prefix identity-swap `18,27→19,27` **ESCAPE**. Sealing same-lap nudge `29,33→28,34` **ESCAPE**. Remaining META_DARK presence still flips 172/172 once `baseCut`+`shallowNow` are left alone. Exact pick and `seedScore` still unread. `cutPasses.rampartsDeleted` split still a log.

Throwaway probes: `tools/plan-suite/v2/_r34-owner/` (not committed).

---

## Sample

`h(room) = fmix32(fnv1a32("round34-owner|" + room))` over the 172 names. Lowest five (not cherry-picked):
...
The 27 adopting rooms (same set as r28–r33):
...
E2S5 (in this hash sample) is a net-zero-count adopter: freeze 28, shipped 28, one remove (`23,43`) and one add (`22,42`).
...
## Closed since r33 — re-probed

**r29p12 / five leftover presence names.** Zeroed on rooms that ship them non-zero:

| attack | room | result |
|--------|------|--------|
| `stitched := 0` | E11S9 (1 vs `laidByKind.stitch` 1) | **BITES** |
| `stitchTiles := 0` | E11S9 (1 vs laid 1) | **BITES** |
| `roadsEaten := 0` | E9S2 (0 vs 3 lab-road-eat tiles on shipped labs) | **BITES** |
| `towerOnly := 0` | E11S1 (0 vs fullest 5×5 = 7) | **BITES** |
| `stubRoads := 0` | E11S1 (0 vs 39 layer-6 `roadLayer` tags) | **BITES** |

Fleet flatten of those five: **fail 172/172** (`towerOnly` + `stubRoads` on every room; `stitched`/`stitchTiles` on 5; `roadsEaten` on 1). The named close is done. `stitched`/`stitchTiles` are twins of the laid book, `stubRoads` is a tag count that includes later-deleted roads, `towerOnly` is a real 5×5 walk, `roadsEaten` is tiles-on-labs. Klass moved. The rest of the presence class did not.
...
**Lab diamond:** 10 labs, 4×4 bbox, 6 holes in all 8. Mandated stamp. Haul cheb 2–5. E11S1 haul 5, E4S2 haul 4, E2S5 / E2S7 / E1S4 haul 3, E17S9 / E15S8 / E3S4 haul 2.
...
**E17S9.** 36-cut, 40 ramparts, 83 roads, 60/60. `baseCut=32 ≠ 36`. `protectRadius=8`. Seed=hub `17,27`. Mobility **1.54** declared (page and film agree). Controller enclosed. Mineral `28,39` **on** the network, seat `29,38`. 0 2×2. 0 redundant. 0 D4-blind. Labs haul 2. Film: 36 crossing / 2 seat.outside / 1 seat.inside / 1 cover. Fat last rung `2/47/44` against shipped `1.54/40/36`. The pick is not the shipped cut. The last discarded enclosure is a free 44-tile neighbour of the 36. Quiet board, unread pick, unread last fat.

**E2S5.** 28-cut, 33 ramparts, 77 roads. Adopter (in the 27): freeze 28 → shipped 28, remove `23,43` / add `22,42`. Dropped reservation: `fullRun.reserved` 6 tiles, `lane.reserved` empty. Mobility **1.92** declared. Controller enclosed. Mineral `28,22` **on** the network. Seed=hub `29,31`. **3 2×2** on a road spine. 0 D4-blind. 0 remaining redundant (one already pruned). Labs haul 3. Film: 28 crossing / 2 seat.inside / 1 seat.outside / 2 cover. Fat last rung `3.83/50/47`. Hash #2 is an adopter, a drop, and a fat discarded cut. All three residues sit on one board.

**E15S8.** 19-cut, 24 ramparts, 78 roads. `baseCut=19` = freeze = cut. `protectRadius=11`. Mobility 0. Controller unenclosed. Mineral `12,12` off-network NW, declared. Seed=hub `26,23`. 0 2×2. 0 redundant. Labs haul 2. Film: 19 crossing / 2 seat.outside / 2 ring / 1 cover. Compact pocket. Sealed-floor notes, quiet.

**E3S4.** 23-cut, 27 ramparts, 79 roads. `baseCut=23`. Mobility **2.83** declared (page and film agree). Controller enclosed. Mineral `31,7` off-network north, declared. Seed=hub `27,35`. 0 2×2. 0 redundant. Labs haul 2. Film: 23 crossing / 2 seat.outside / 2 cover. Fat last rung `7.33/32/29`. Ugly for a named reason: the 2.83 lap.

**E4S2.** 31-cut, 40 ramparts, 98 roads. `baseCut=31`. Mobility **1.5** declared. Controller unenclosed. Mineral `30,40` **on** the network. Seed=hub `27,29`. Labs haul 4. 0 2×2. 0 redundant. Film: 31 crossing / 2 seat.outside / 1 seat.inside / 4 ring / 2 cover. Fat last rung `3.17/49/41`. Roads are the basin tax; the room prices the lap, not the 98.
...
No sampled room is a checkerboard, a solid brick, or a maze. Extension mass is corridor-flanked (**0 D4-blind extensions in all 8**). 2×2 squares exist in E2S5 (3) and E11S1 (3) and they sit on a road spine. E17S9 / E3S4 / E4S2 / E2S5 spend a named lap. E11S1 / E4S2 spend roads on eco, not on a city grid. Towers are not a hub clump (sample clumps 2–3).

Nothing in the sample looks accidental. The ugly rooms are ugly for a named, declared reason (E3S4 lap 2.83, E2S5 lap 1.92, E17S9 lap 1.54, E4S2 lap 1.5 + 98 roads, E11S1 27-tile controller walk) — except the unread pick on E17S9 (`baseCut=32` against a 36-cut) and the unread last fats, which the rooms do not price as compositions.
...
**Exploit.** E11S1 after r29p10/p11/p12: `fullRun.reserved` === `lane.reserved` === 6 tiles / 2 rounds. `wanted=7`. The refused tail is a count.
...
`discardedCutSeals` no longer trusts `row.complete`. A leak bites either way. That is the named close. A 42-tile nudged cut that *does* seal is longer than the shipped 37, so the would-have-taken rule returns before it looks. Any other 42-tile COORD list that walks the claimed lap and seals passes. Flipping `complete` on a sealing rung is free. This sample has three more last fats of the same class (E17S9 `2/47/44`, E2S5 `3.83/50/47`, E3S4 `7.33/32/29`).

**Root cause.** Discarded `cutTiles` are a free list held to `enclosureMobility` of themselves, plus "not freeze/shipped", plus "not shorter-and-prettier", plus "seals". The composed enclosure is not on the board. r29p11 gated leaks on the flood, not the flag. It did not bind the list. r29p12 did not touch this.
...
**Exploit.** Zeroed every remaining `klass:"presence"` numeric/bool/array leaf across all 172 rooms. 71 presence names, 28 derived (r33 was 76 / 23). 39 kinds, 3,964 events. `checkRoom`: **fail 172/172**, every first fail is `baseCut := 0`. Exclude `baseCut` and `shallowNow`: **pass 172/172**. 37 kinds, 3,789 events, silent.
...
r29p3 / r29p9 / r29p12 derived names still BITES. The class is still the class. `protectRadius` is *classified* `derived` and is only an enum.
...
Sample: E17S9 `baseCut=32` / freeze=36 / cut=36. E2S5 `baseCut=28` = cut. E15S8 `baseCut=19` = cut. E3S4 `baseCut=23` = cut. E4S2 `baseCut=31` = cut. E1S4 `baseCut=24` / cut=26. The pick is not freeze and not the shipped cut. `protectRadius` is an enum, not this room's pick. `seedScore` is a finite number, not the confluence walk. 46 rooms still have seed ≠ hub. The coordinates exist. The score does not.
...
- Not the r33 leftover on `stitched` / `stitchTiles` / `roadsEaten` / `towerOnly` / `stubRoads`. Zeroing those five now fails.
...
- r29p12 `stitched`, `stitchTiles`, `roadsEaten`, `towerOnly`, `stubRoads`
...
- Remaining META_DARK presence (71 names, 172/172 silent once `baseCut` is left alone)
...
Boards are clean. The gallery in the sample is readable and matches the tiles. 134(b) as a **number** stays done. 93, the *stated* 98 one-list append, the *stated* 88 leak-on-complete **and** leak-on-incomplete, the four r29p9 presence names, and the five r29p12 presence names are done. The 98 class is not done: reserved is still a free set and the refused walk is still a count. The 88 class is not done: a sealing same-lap neighbour still passes. Plus the presence class, plus the exact pick, plus `seedScore`, plus one `cutPasses` column.
