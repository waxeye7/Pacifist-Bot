/**
 * Round-34 hostile mutations. In-memory checkRoom only. Never writes the artifact.
 * Named BITES re-probes: p12 stitched/stitchTiles/roadsEaten/towerOnly/stubRoads;
 * 88 leaky+complete=false; 98 one-copy 19,27.
 * ESCAPE class: 98 both-lists, wanted+=1, 88 same-lap, seedScore, protectRadius enum.
 */
import fs from "fs";
...
// r29p12 named closes — flatten the five newly derived leftover presence walks.
...
// Hostile: stitched gate is polarity, not the count (laid.stitch twin is zero-vs-nonzero).
rec(run("PRESENCE-stitched-nonzero-to-1", rooms.stitched, (p) => { p.meta.walls.stitched = 1; }));
