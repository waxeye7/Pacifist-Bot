...
Artifact md5 `8e5d6725885bd3f3731379bedf408326` — matches the brief. Same bytes as r32–r37. Tree HEAD is `0b81233` (r29p16: bind `hubDistCap` / `lapCeilingFloor` / `corridorPlaced`), parent `27bc2fb` (r29p15 seedScore). 172/172 rooms, 0 errors. Fleet physicals re-summed off the structure lists: **10,320 extensions · 14,100 roads · 8,208 ramparts · 300 declarations · 236 notes**. Same numbers as r26–r37. Boards did not move. Baseline `checkRoom` (fleet-median stripped): **pass 172/172**.

**Verdict.** No board defect in the sample. No film-vs-board caption lie in the sample. 134(b) is still 34 adds / 27 rooms. r29p16 closes the *stated* leftover flattens: `hubDistCap` off-ladder **BITES** (fleet 172/172), `lapCeilingFloor := 0` **BITES** (fleet 172/172), `corridorPlaced := 0` against `fallback=0` **BITES** (168/168 of the 60/0 rooms). `seedScore` still **BITES**. The p15 ten + p14 nine + p12 five still **BITES**. The named 88 leak + `complete=false` still **BITES**. The named 98 one-list append still **BITES**. They do **not** close the classes. Twin of the named 98 append (also write `lane.reserved`, keep `wanted > tiles`) **ESCAPE**. `wanted += 1` **ESCAPE**. Sealing same-lap nudge `29,33→28,34` **ESCAPE**. `hubDistCap` 16→19 in-enum **ESCAPE** (fleet 170/170). Remaining META_DARK presence still flips 172/172 silent once `baseCut`+`shallowNow` are left alone. Exact pick still unread. `protectRadius` is still an enum. `hubDistCap` is now the same kind of enum. `corridorPlaced` is 60 iff fallback is 0, not this room's count. `cutPasses.rampartsDeleted` split still a log.

Throwaway probes: `tools/plan-suite/v2/_r38-owner/` (not committed).

---

## Sample

`h(room) = fmix32(fnv1a32("round38-owner|" + room))` over the 172 names. Lowest five (not cherry-picked):
...
Also churn-read as required: **E11S1 E2S7 E1S4**. Seven unique rooms (E2S7 is both #5 and mandated).
...
The 27 adopting rooms (same set as r28–r37):
...
## Closed since r37 — re-probed

**r29p16 / three leftover presence names.** Mutated on rooms that ship them:
...
Fleet: `hubDistCap := 17` **fail 172/172**. `lapCeilingFloor := 0` **fail 172/172** (344 events; both reflow copies). `corridorPlaced := 0` on the 168 rooms that ship 60/0: **fail 168**, pass 4 (the four already-fallback rooms were not mutated). The named flatten is done.

Residues of the named close, not the named flatten:

| attack | result |
|--------|--------|
| `hubDistCap` 16 → 19 (E11S1; fleet `{16:170, 19:1, 999:1}`) | **ESCAPE** |
| `hubDistCap` 16 → 23 | **ESCAPE** |
| fleet `hubDistCap` 16→19 on all 170 | **pass 172/172** |
| `corridorPlaced := 0` **and** `fallback := 1` | **ESCAPE** |

`hubDistCap` is an enum, same shape as `protectRadius` / `stubCap`. Fleet `deepReach` is 18 everywhere, so `min(any legal cap+2, 18)=18`. The two rooms that left 16 are E9S7 (19) and E5S3 (999) — both also failed to place 60 corridors (51/3 and 46/5). `corridorPlaced` is 60 iff fallback is 0, not this room's count. The four leftover rooms are E2S5 59/1, E9S7 51/3, E5S3 46/5, E9S2 48/6.
...
Fleet forge of those ten: **fail 172/172**. Fleet `seedScore := 0`: **fail 172/172**. `stubCap` 43→51 still **ESCAPE**. Seat+approach moved together to another cheb-1 / D8 pair still **ESCAPE**. Approach to another D8 of the same seat still **ESCAPE**.
...
**Shallow extensions** re-derived against live depth < 4: **0** in all 7. Published 0. E11S1's `shallowExt` note is a search note.
...
**Lab diamond:** 10 labs, 4×4 bbox, 6 holes in all 7. Mandated stamp. Haul cheb 2–5. E7S2 / E11S1 haul 5, E11S8 haul 4, E2S7 / E1S4 haul 3, E17S3 / E21S1 haul 2.
...
**E17S3.** 58-cut, 68 ramparts, 93 roads, 60/60. `baseCut=58` = freeze = cut (pricey). `protectRadius=10`. As-built gated lap **0** (no pair over the detour floor), declared. Controller unenclosed. Mineral `19,36` off-network, declared (seat `18,35`). Seed `12,23` ≠ hub `15,24`. `seedScore=-46.4`. 0 2×2. 0 redundant. 0 D4-blind. Labs haul 2. Film: 58 crossing / 3 cover / 1 seat.inside / 3 seat.outside / 3 ring. Eco 23-tile controller walk, declared. Fat last rungs `2.71/68/58`, `4.2/69/60` ×2, then `1.83/77/70` against shipped `0/68/58`. Hash #1 is a pricey 58-wall, a seed-not-hub, and a last discarded 70-tile neighbour. The unread last fat sits on a room that prices the basin, not the discarded enclosure.

**E21S1.** 15-cut, 22 ramparts, 98 roads. `baseCut=15` = freeze = cut. `protectRadius=7`. Mobility 0. Controller unenclosed. Mineral `17,9` off-network north, declared (seat `18,10`). Seed=hub `31,41`. `seedScore=-51.1`. Labs variant `anti`, haul 2. 0 2×2. 0 redundant. 0 D4-blind. Film: 15 crossing / 2 ring / 2 cover / 3 seat.outside. Eco 25-tile controller walk west, declared. Compact pocket, long eco arm, named. Not an adopter (E21S2 / E21S8 are; this one is not).

**E7S2.** 25-cut, 28 ramparts, 72 roads. `baseCut=22 ≠ 25`. `protectRadius=12`. Mobility 0. Controller enclosed. Mineral `35,12` off-network, declared (seat `36,11`). Seed=hub `21,35`. `seedScore=-14.8`. Taken-room `sealedRecovery.outcome=taken`, **0** published holders. 0 2×2. 0 D4-blind. Labs haul 5. Film: 25 crossing / 1 seat.outside / 1 cover / 1 seat.inside. The pick is 22 against a freeze and a shipped cut of 25. Quiet board, unread pick.

**E11S8.** 48-cut, 51 ramparts, 85 roads. `baseCut=48` = freeze = cut (pricey). `protectRadius=10`. Mobility 0. Controller enclosed. Mineral `25,24` **on** the network (seat `24,25`). Seed=hub `16,30`. `seedScore=-23.2`. **0 shortfalls.** 0 2×2. 0 redundant. 0 D4-blind. Labs haul 4. Film: 48 crossing / 1 cover / 1 seat.outside / 1 seat.inside. The 48-tile wall is the ugliness the room does not price. The mineral being on-network is the one thing this sample does not have to excuse.

**E2S7** (golden, and hash #5). 18-cut, 21 ramparts, 72 roads. `baseCut=18` = freeze = cut. `protectRadius=12`. Clean pocket. 0 redundant. Mineral `33,4` off-network (seat `32,3`). Seed=hub `13,27`. `seedScore=-21.5`. Labs haul 3. Quiet.
...
No sampled room is a checkerboard, a solid brick, or a maze. Extension mass is corridor-flanked (**0 D4-blind extensions in all 7**). 2×2 squares exist only in E11S1 (3) and they sit on a road spine. E17S3 / E21S1 / E11S1 spend roads on eco, not on a city grid. Towers are not a hub clump (sample clumps 2–3).

Nothing in the sample looks accidental. The ugly rooms are ugly for a named, declared reason (E17S3 58-tile pricey wall + 23-tile controller walk, E21S1 25-tile controller arm, E11S8 48-tile pricey wall, E11S1 27-tile controller walk) — except the unread pick on E7S2 (`baseCut=22` against a 25-cut) / E1S4 (`baseCut=24` against a 26-cut) and the unread last fat on E17S3 (`1.83/77/70` against a shipped 58), which the rooms do not price as compositions.
...
**Exploit.** E11S1 after r29p10/p11/p12/p13/p14/p15/p16: `fullRun.reserved` === `lane.reserved` === 6 tiles / 2 rounds. `wanted=7`. The refused tail is a count.
...
r29p16 did not touch this. A tile that *is* this room's floor and *touches* the walk can still be added, or swapped for a kept prefix tile, if both lists stay twins. The refused walk is now four unread integers (`wanted`, `used`, `ext`, `shallow`) plus `premium`.
...
`discardedCutSeals` no longer trusts `row.complete`. A leak bites either way. That is the named close. A 42-tile nudged cut that *does* seal is longer than the shipped 37, so the would-have-taken rule returns before it looks. Any other 42-tile COORD list that walks the claimed lap and seals passes. Flipping `complete` on a sealing rung is free. This sample has one more last fat of the same class (E17S3 `1.83/77/70`).

**Root cause.** Discarded `cutTiles` are a free list held to `enclosureMobility` of themselves, plus "not freeze/shipped", plus "not shorter-and-prettier", plus "seals". The composed enclosure is not on the board. r29p11 gated leaks on the flood, not the flag. It did not bind the list. r29p16 did not touch this.
...
**Exploit.** Zeroed every remaining `klass:"presence"` numeric/bool/array leaf across all 172 rooms. 44 presence names, 55 derived (r37 was 48 / 51; p16 moved `hubDistCap` / `lapCeilingFloor` / `corridorPlaced` / `corridorFallback`). 21 kinds, 1,778 events. `checkRoom`: **fail 172/172**, every first fail is `baseCut := 0`.

Exclude `baseCut` and `shallowNow`: **pass 172/172**. 19 kinds, 1,603 events, silent. The r37 "minus those two fails on `hubDistCap`" door is shut — because p16 moved the leaf to `derived`, not because the class was derived. Zero the class minus the two bounded integers and the class is silent again.

Single-room ESCAPE, flattering direction: `deepBudget` decrement-keep-nonzero / cleared. `spurred` decrement 5→4 still ESCAPE — and `spurred` is classified `derived`. `stubCap` 43→51 **ESCAPE** — classified `derived`, only an enum. `corridorPlaced := 0` + `fallback := 1` **ESCAPE** — classified `derived`, only an iff.

r29p3 / r29p9 / r29p12 / r29p13 / r29p14 / r29p15 / r29p16 derived names still BITES. The class is still the class. `protectRadius` is *classified* `derived` and is only an enum. `hubDistCap` is now *classified* `derived` and is only an enum.

**Root cause.** Criticism 141(c). They derived the three cheap identities this review named. The rest are still comments.

**Board moved?** No.

### M5 — MEDIUM, not board-affecting

**Exact `baseCut` / `protectRadius` / `hubDistCap` pick is unread. `seedScore` is not.**

`protectRadius := 0` **BITES** (not in `RADII_WIDE`). `12 → 6` inside the enum **ESCAPE**. `baseCut := 0` **BITES**. `baseCut += 1` keeping `priceyWall` on the same side of `MAX_CUT=45` **ESCAPE**. `hubDistCap := 17` **BITES** (not in `[16,19,23,999]`). `16 → 19` inside the enum **ESCAPE**. `seedScore := 0` **BITES**. `seedScore += 999` **BITES**.

Sample: E17S3 `baseCut=58` = cut (pricey). E21S1 `baseCut=15` = cut. E7S2 `baseCut=22` / freeze=25 / cut=25. E11S8 `baseCut=48` = cut (pricey). E2S7 `baseCut=18` = cut. E1S4 `baseCut=24` / cut=26. The pick is not freeze and not the shipped cut. `protectRadius` is an enum, not this room's pick. `hubDistCap` is now an enum, not this room's cohesion rung. `seedScore` is the window max. 46 rooms still have seed ≠ hub. The coordinates exist. The score is the walk. The cap is four legal integers.
...
- Remaining META_DARK presence (44 names, 172/172 silent once `baseCut`+`shallowNow` are left alone)
...
Boards are clean. The gallery in the sample is readable and matches the tiles. 134(b) as a **number** stays done. 93, the *stated* 98 one-list append, the *stated* 88 leak-on-complete **and** leak-on-incomplete, the four r29p9 presence names, the five r29p12 presence names, the four r29p13 walks, the nine r29p14 walks, the ten r29p15 walks, `seedScore` as the rank-1 confluence walk, and the three r29p16 identities (`hubDistCap` ladder / `lapCeilingFloor=1.2` / `corridorPlaced` iff fallback) are done. The 98 class is not done: reserved is still a free set and the refused walk is still a count. The 88 class is not done: a sealing same-lap neighbour still passes. Plus the presence class, plus the exact pick (now including in-enum `hubDistCap`), plus one `cutPasses` column.
