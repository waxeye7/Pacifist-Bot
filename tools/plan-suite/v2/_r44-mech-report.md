# Round 44 mechanical review

Artifact `tools/plan-suite/out-v2/plans-hub.json` md5 `8e5d6725885bd3f3731379bedf408326` (27,780,512 bytes) — **matches expected**. HEAD `55960b1` (r29p22: bind `towers.baseLap` — layer-2 empty-room freeze-cut lap with only the hub kit standing, on `towers.mobilityVeto.baseLap`). Artifact bytes unchanged since r32. p22 is a gate + mutate-corpus pass (`r27-gates.mjs`, `mutate.mjs`). Rooms dump `tools/plan-suite/v2/_r28-mech/rooms.json` (172/172, 0 missing). Node `v22.13.0` (`fnm` install on PATH). Probes in `tools/plan-suite/v2/_r44-mech/`. No suite source committed. Artifact not written. **No board moved.** No `boards` field. Physicals still 10,320 / 14,100 / 8,208.
...
**Exploit.** E11S1 is a real shrink. After r29p10 the suffix is gone: `fullRun.reserved === lane.reserved` (6 tiles / 2 rounds), `shrunk.wanted=7`. The refused extra is a count. p22 did not touch this.
...
**Exploit.** E11S2 rungs: last fat is complete, 42 tiles, lap 2.5, ramparts 50, has `20,9` and `29,33`. Shipped wall 46/37 = freeze. p22 did not touch this.
...
**Exploit.** 34 presence names, 66 derived (p22 did not add a META_DARK name — `baseLap` was already derived). Zeroed every remaining `klass:"presence"` numeric/bool/nonempty-array leaf across all 172 rooms. 15 kinds, 1,189 events. `checkRoom`: **fail 172/172**, every first fail is `baseCut := 0`.
...
Closed this artifact (re-probed, BITES): `nukerInWindow`, `center`, `mineralSeatNetTiles`, `coveredDetourDeclared`, **p12 five**, **p13 three**, **p14 nine** + `boundLap`, **p15 ten**, **`seedScore`**, **p16 four**, **p17 five**, **p18 two**, **p19 four**, **p20 two**, **p21 `baseLap`** (veto copy / labs copy / `:= 0` / `+= 1` / both copies), **p22 `towers.baseLap`** (`:= 0` / `+= 1` / set to p21 7.67 / rewrite `repair.tower` twin + regen). Independent empty-room walk: `towers.baseLap` **172/172** on `enclosureMobility(freezeCut)`. Independent freeze-cut-with-battery walk: veto/labs `baseLap` **172/172**, `wasLap` **6/6**. `towers.baseLap` vs that p21 walk is **163/172** — 9 rooms differ, E11S7 7.33 vs 7.67. That is the point of p22: it is not that walk.
...
### 4. Exact `baseCut` / `protectRadius` / `hubDistCap` picks are unread. `maxHubDist` / `stubCap` 43↔51 / p19 three / p20 two / p21 `baseLap` / p22 `towers.baseLap` are closed.
...
Sample: E15S6 `baseCut=16` = cut. E17S9 `baseCut=32` ≠ cut 36. E7S1 `baseCut=42` = cut. E9S5 `baseCut=49` ≠ cut 51 (pricey). E3S9 `baseCut=39` = cut.
...
### 7. p22 `towers.baseLap` — named flatten BITES; it is the empty-room freeze-cut lap, not the p21 walk.

| attack | result |
|---|---|
| `towers.mobilityVeto.baseLap := 0` (E11S7, 7.33) | **BITES** (twin + walk) |
| `towers.mobilityVeto.baseLap += 1` | **BITES** |
| set to p21 freeze-cut **7.67** on E11S7 | **BITES** (empty-room walk is 7.33) |
| `:= 0` **and** rewrite every `repair.tower.baseLap` + regen | **BITES** (walk still 7.33; twin-to-`ladder.shippedLap` also fires) |
| `+= 1` **and** rewrite twin + regen | **BITES** |
| one copy on E11S2 (1.56) | **BITES** |
| `misc.mobilityVeto.baseLap := 0` / `+= 1` | **BITES** (p21, still) |
| `labs.lapVeto.baseLap := 0` / `+= 1` | **BITES** (p21, still) |
| `baseOverGated := 0` / `wasLap := 0` | **BITES** (p20, still) |

Independent empty-room walk (`enclosureMobility` of the freeze cut, hub kit only): `towers.baseLap` **172/172**. Independent p21 walk (mass / nuker / observer lifted): veto **172/172**, labs **172/172**, `wasLap` **6/6**. `towers` vs p21: **163/172** — 9 rooms differ (E11S7 7.33 vs 7.67, E13S4 3.83 vs 4, E14S8 1.62 vs 1.69, E16S3 1.45 vs 1.64, E17S5 4 vs 4.4, E19S2 1.29 vs 1.5, …). Fleet `towers.baseLap` `{0:115, …}` — 57 nonzero. They bound the unread sibling the last review named. It is that empty-room walk, not the battery walk and not a comment. Flattening it used to pass once the declaration twin was rewritten with it.

### 8. p21 two / p20 two / p19 four / p18 two / p17 five / p16 four / p15 ten — named flatten BITES; identities remain.
...
Independent single-removal over all **7,246** `cutAtFreeze` tiles: **0 loose**, 0 core-seal fails, 29 rooms differ from the shipped cut, 34 add + 46 remove, 27 adopt / 29 drift. Same census as r29–r43.
...
## Attacked and held (r43 closures + r29p22 named, re-probed)
...
| **p22** `towers.baseLap := 0` / `+= 1` / set to p21 7.67 / rewrite twin + regen | **BITES** |
...
`h(room) = fmix32(fnv1a32("round44-mech|" + room))` over the 172 names. Lowest five (not cherry-picked):
...
|---|---|---|---|---|
| E15S6 | 60/81/18/16/16 | 0 | 34,43 off | hashed #1, seed=hub, **taken** 4 holders, `protectRadius=11` |
| E17S9 | 60/83/40/36/36 | **1.54** | 29,38 off | hashed #2, seed=hub, pick 32 ≠ cut 36, `protectRadius=8` |
| E7S1 | 60/83/45/42/42 | **3.5** | 18,27 off | hashed #3, seed=hub, pick 42 = cut, `protectRadius=9` |
| E9S5 | 60/71/53/51/51 | **3** | 12,27 off | hashed #4, seed=hub, **pricey** 49 ≠ cut 51, `protectRadius=10` |
| E3S9 | 60/75/42/39/39 | **2.57** | 37,31 off | hashed #5, seed `36,16` ≠ hub `35,18`, pick 39 = cut |
...
| E11S7 | 60/69/53/50/50 | **8.67** | 8,42 off | taken, 0 fixedHolders, pricey 50, `towers.baseLap=7.33` vs p21 7.67 |
...
Nothing in the sample looks accidental. The expensive rooms say so (E11S2 lap 1.5, E11S7 lap 8.67 + take, E7S1 lap 3.5, E9S5 lap 3 + pricey, E3S9 lap 2.57, E17S9 lap 1.54 + unread pick). Hash five publishes `baseCut ===` shipped cut on three of five (E15S6, E7S1, E3S9); E17S9 32 vs 36 and E9S5 49 vs 51 are the unread picks.

---

## Clean re-derivations (terrain + shipped structure lists)

From `_r44-mech/rederive.mjs` (does not import `validate.mjs`):
...
Findings 1–3 are the round. r29p22 closed the *named* leftover flatten (`towers.baseLap` on the empty-room walk). The prefix is still a neighbor list. The last discarded rung is still a free enclosure that seals. A closer who only adds another identity over producer leaves will ship this review again.
...
**No.** Named p22 `towers.baseLap` / named p21 `baseLap` / named p20 two leftover flattens / named p19 four leftover flattens / named p18 two leftover flattens / named p17 five leftover flattens / named p16 four leftover flattens / named p15 ten leftover walks + `seedScore` / named p14 nine + `boundLap` / named p13 three + stitch 0/1 / named p12 five / named 88 leaky+`complete=false` / named 98 `19,27` on one copy / 88 leaky complete / named 98 COORD bag / 88 shipped-cut / 88 8-tile box / 93 plant-back closed. The kept prefix is a D8-neighbor bag. `wanted` is a count. The last discarded rung is a free sealing enclosure. The presence class still flips once `baseCut`+`shallowNow` are left alone. No leftover presence name has a 172/172 board walk. `hubDistCap` / `protectRadius` are enums. `towers.baseLap` is now the empty-room freeze-cut lap. `baseLap` is the freeze-cut-with-battery lap on both p21 copies. `takeTowerSwap.from` is the published take pair. `servedExts` is a boolean of the filler book. `radii` is a bonus table. The reserved mineral pair is a topology bound. `clean_rounds` is 0. The bar is 2 consecutive all-clean. Boards did not move.
