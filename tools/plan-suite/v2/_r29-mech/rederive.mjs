/**
 * Independent physicals + 134(a) + mineral census + seed inventory.
...
const totals = {
  ...
  seed: { hasSeed: 0, noSeed: 0, missing: [], fields: {} },
  fullRun: { n: 0, ran: 0, shrunk: 0, dropped: 0, plain: 0 },
...
const BLOCK = ["storage", "terminal", "spawn", "link", "extension", "lab", "tower", "nuker", "observer", "factory", "powerSpawn"];
...
for (const plan of plans) {
  ...
  // stack / object / border — engine-side physicals
  ...
  for (const [k, types] of byTile) {
    ...
    if (objects.has(k) && !types.every((t) => t === "extractor" && plan.mineral && K(plan.mineral) === k)) {
      if (!(types.length === 1 && types[0] === "extractor" && plan.mineral && K(plan.mineral) === k)) {
        totals.onObject++;
      }
    }
  ...
  if (typeof why === "string" && seat) {
    ...
    if (why === want) totals.mineralWhy.exactOfficial++;
      ...
      totals.mineralWhy.lieRooms.push({
        room: plan.room,
        seat: K(seat),
        seatHasRoad: roads.has(K(seat)),
        publishedHead: why.slice(0, 180),
        derivedHead: want.slice(0, 180),
      });
    }
  }

  const seedFields = ["seed", "seedScore", "seedSkip", "seedPool", "seedSkipTrail"];
  for (const f of seedFields) {
    const v = plan.meta?.[f];
    if (v !== undefined && v !== null) totals.seed.fields[f] = (totals.seed.fields[f] || 0) + 1;
  }
  if (plan.meta?.seed && typeof plan.meta.seed === "object" && Number.isInteger(plan.meta.seed.x)) {
    totals.seed.hasSeed++;
  } else {
    totals.seed.noSeed++;
    totals.seed.missing.push(plan.room);
  }
  ...
  if (sf) {
    ...
    const hasCut = sf.ladder.rungs.some((r) => Array.isArray(r?.cutTiles) && r.cutTiles.length) ||
  ...
  if (plan.meta?.sealedRecovery?.outcome === "taken") {
    totals.taken.push({
      room: plan.room,
      fixed: (plan.meta.sealedRecovery.fixedHolders || []).length,
      recovers: (plan.meta.sealedRecovery.fixedHolders || []).map((h) => `${h.type}@${K(h)}=${h.recovers}/${h.recoversDeep}`),
    });
  }
}
...
const out = {
  ...
  seed: {
    hasSeed: totals.seed.hasSeed,
    noSeed: totals.seed.noSeed,
    missingN: totals.seed.missing.length,
    missing: totals.seed.missing,
    fields: totals.seed.fields,
  },
