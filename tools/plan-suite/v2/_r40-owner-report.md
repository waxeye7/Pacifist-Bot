...
Artifact md5 `8e5d6725885bd3f3731379bedf408326` — matches the brief. Same bytes as r32–r39. Tree HEAD is `e469b99` (r29p18: `maxHubDist` layer-6 hub-field walk / `takeTowerSwap.from` D8 of `to`), parent `80601a0` (r29p17 stubCap pool / floorUngated / radii / parkCap / takeTowerSwap.to). 172/172 rooms, 0 errors. Fleet physicals re-summed off the structure lists: **10,320 extensions · 14,100 roads · 8,208 ramparts · 300 declarations · 236 notes**. Same numbers as r26–r39. Boards did not move. Baseline `checkRoom` (fleet-median stripped): **pass 172/172**.

**Verdict.** No board defect in the sample. No film-vs-board caption lie in the sample. 134(b) is still 34 adds / 27 rooms. r29p18 closes the *stated* leftover flattens: `maxHubDist := 0` **BITES** (fleet 172/172), `maxHubDist += 1` **BITES**, `takeTowerSwap.from → 1,1` **BITES** (fleet 3/3). `seedScore` still **BITES**. The p17 five + p16 four + p15 ten + p14 nine + p12 five still **BITES**. The named 88 leak + `complete=false` still **BITES**. The named 98 one-list append still **BITES**. They do **not** close the classes. Twin of the named 98 append (also write `lane.reserved`, keep `wanted > tiles`) **ESCAPE**. `wanted += 1` **ESCAPE**. Sealing same-lap nudge `29,33→28,34` **ESCAPE**. `hubDistCap` 16→19 in-enum **ESCAPE** (fleet 170/170). Remaining META_DARK presence still flips 172/172 silent once `baseCut`+`shallowNow` are left alone. Exact pick still unread. `protectRadius` is still an enum. `hubDistCap` is still an enum. `maxHubDist` is now the pre-7b hub-field max. `takeTowerSwap.from` is a D8 of `to`, not the vacated seat. `cutPasses.rampartsDeleted` split still a log.

Throwaway probes: `tools/plan-suite/v2/_r40-owner/` (not committed).

---

## Sample

`h(room) = fmix32(fnv1a32("round40-owner|" + room))` over the 172 names. Lowest five (not cherry-picked):
...
The 27 adopting rooms (same set as r28–r39):
...
## Closed since r39 — re-probed

**r29p18 / two leftover presence names.** Mutated on rooms that ship them:
...
Fleet: `maxHubDist := 0` **fail 172/172**. `takeTowerSwap.from → 1,1` **fail 3**, pass 169 (the 169 rooms with no swap were not mutated). The named flatten is done.

Residues of the named close, not the named flatten:

| attack | result |
|--------|--------|
| `takeTowerSwap.from` other D8 of `to` (E14S1 `35,24`, empty, not a live tower) | **ESCAPE** |

`maxHubDist` is no longer a comment. It is the layer-6 hub-field max of the extension seats this room stood before the 7b reflow (shipped mass minus `reflow.added`, each `reflow.moved` wound back). `+= 1` fails. That one is a walk. `takeTowerSwap.from` is a D8 neighbour of `to`. It is not the vacated seat. Any other empty D8 of the destination passes.
...
**Shallow extensions** re-derived against live depth < 4: **0** in all 8. Published 0. E11S1's `shallowExt` note is a search note. E16S3's `shallowExt` note is a search note.
...
**Lab diamond:** 10 labs, 4×4 bbox, 6 holes in all 8. Mandated stamp. Haul cheb 2–5. E11S1 haul 5, E16S3 / E12S4 haul 4, E13S8 / E2S7 / E1S4 haul 3, E9S5 / E15S8 haul 2.
...
**E16S3.** 31-cut, 33 ramparts, 84 roads. `baseCut=31` = freeze = cut. `protectRadius=9`. As-built gated lap **1.64**, declared. Controller enclosed. Mineral `6,7` **on** the network (seat `7,6`). Seed=hub `20,14`. `seedScore=-29.1`. 0 2×2. 0 D4-blind. Labs haul 4. Film: 31 crossing / 1 cover / 1 seat.outside. Fat last rungs `2/38/36`, `2.86/39/37`, then `4.4/41/39` against shipped `1.64/33/31`. Hash #1 is a declared 1.64 lap and an unread last fat. The mineral being on-network is the one thing this sample does not have to excuse.

**E12S4.** 30-cut, 40 ramparts, 82 roads. `baseCut=30` = freeze = cut. `protectRadius=11`. Mobility 0. Controller unenclosed. Mineral `18,11` off-network, declared (seat `17,12`). Seed=hub `28,31`. `seedScore=-39.4`. Labs variant `anti`, haul 4. 0 2×2. 0 D4-blind. Film: 30 crossing / 3 seat.outside / 2 cover / 5 ring. Quiet pocket, named off-network, no discarded fat.

**E9S5.** 51-cut, 53 ramparts, 71 roads. `baseCut=49 ≠ 51`. `protectRadius=10`. As-built gated lap **3**, declared. Controller enclosed. Mineral `11,26` off-network, declared (seat `12,27`). Seed=hub `15,13`. `seedScore=-4`. Labs haul 2. 0 2×2. 0 D4-blind. Film: 51 crossing / 1 seat.inside / 1 seat.outside. Escalation rungs are all `3/53/51` — same as shipped. The pick is 49 against a freeze and a shipped cut of 51. Hash #3 is a 51-wall, a declared 3 lap, and an unread pick.

**E13S8.** 80-cut, 85 ramparts, 53 roads. `baseCut=80` = freeze = cut (pricey). `protectRadius=9`. Mobility 0. Controller unenclosed. Mineral `7,28` off-network, declared (seat `6,29`). Seed=hub `28,28`. `seedScore=-7.7`. Labs haul 3. 0 2×2. 0 D4-blind. Film: 80 crossing / 1 cover / 3 ring / 1 seat.outside. The p17 `radii` witness (`needDeepBonus=25` → WIDE). The 80-tile wall is the ugliness the room does not price.

**E15S8.** 19-cut, 24 ramparts, 78 roads. `baseCut=19` = freeze = cut. `protectRadius=11`. Mobility 0. Controller unenclosed. Mineral `12,12` off-network, declared (seat `11,11`). Seed=hub `26,23`. `seedScore=-34.6`. Labs haul 2. Sealed-floor / sealed-recovery notes, **0** published holders. 0 2×2. 0 D4-blind. Film: 19 crossing / 2 seat.outside / 2 ring / 1 cover. Compact, named, quiet.

### Mandated churn

**E11S1.** 32-cut, 43 ramparts, 101 roads. Real shrink: `fullRun.reserved` === `lane.reserved` === 6 tiles / 2 rounds. `wanted=7`, `used=3`, `ext=60`, `shallow=3`. Suffix COORD list is gone. Seed `25,39` ≠ hub `24,41`. `seedScore=-92.9`. Controller unenclosed. Mineral `23,7` off-network, declared (seat `24,8`). Eco 27-tile controller walk, declared. 3 2×2 on a road spine. 0 D4-blind. Labs haul 5. Film: 32 crossing / 4 seat.outside / 3 cover / 4 ring. Reserved board: `18,27 19,28 19,29 | 24,28 25,27 25,29`. `19,27` is D8 of `18,27` and `19,28` and is not in the set. The 98 residue room, mandated. `maxHubDist=15` — the p18 witness.
...
No sampled room is a checkerboard, a solid brick, or a maze. Extension mass is corridor-flanked (**0 D4-blind extensions in all 8**). 2×2 squares exist only in E11S1 (3) and they sit on a road spine. E16S3 / E12S4 / E11S1 spend roads on eco, not on a city grid. Towers are not a hub clump except E16S3 (clump 4); the rest of the sample clumps 2–3.

Nothing in the sample looks accidental. The ugly rooms are ugly for a named, declared reason (E16S3 1.64 lap, E9S5 51-tile wall + 3 lap, E13S8 80-tile pricey wall, E11S1 27-tile controller walk) — except the unread pick on E9S5 (`baseCut=49` against a 51-cut) / E1S4 (`baseCut=24` against a 26-cut) and the unread last fat on E16S3 (`4.4/41/39` against a shipped 31), which the rooms do not price as compositions.
...
**Exploit.** E11S1 after r29p10/p11/p12/p13/p14/p15/p16/p17/p18: `fullRun.reserved` === `lane.reserved` === 6 tiles / 2 rounds. `wanted=7`. The refused tail is a count.
...
r29p18 did not touch this. A tile that *is* this room's floor and *touches* the walk can still be added, or swapped for a kept prefix tile, if both lists stay twins. The refused walk is now four unread integers (`wanted`, `used`, `ext`, `shallow`) plus `premium`.
...
`discardedCutSeals` no longer trusts `row.complete`. A leak bites either way. That is the named close. A 42-tile nudged cut that *does* seal is longer than the shipped 37, so the would-have-taken rule returns before it looks. Any other 42-tile COORD list that walks the claimed lap and seals passes. Flipping `complete` on a sealing rung is free. This sample has one more last fat of the same class (E16S3 `4.4/41/39`).

**Root cause.** Discarded `cutTiles` are a free list held to `enclosureMobility` of themselves, plus "not freeze/shipped", plus "not shorter-and-prettier", plus "seals". The composed enclosure is not on the board. r29p11 gated leaks on the flood, not the flag. It did not bind the list. r29p18 did not touch this.
...
**Exploit.** Zeroed every remaining `klass:"presence"` numeric/bool/array leaf across all 172 rooms. 39 presence names, 60 derived (r39 was 40 / 59; p18 moved `maxHubDist`). 17 kinds, 1,250 events. `checkRoom`: **fail 172/172**, every first fail is `baseCut := 0`.

Exclude `baseCut` and `shallowNow`: **pass 172/172**. 15 kinds, 1,075 events, silent. Same 172/172 if those two plus all newly derived names (p12–p18) are skipped. The r39 "minus those two is silent" door is still open — p18 reclassified one leaf, it did not derive the class.

Single-room ESCAPE, flattering direction: `deepBudget` decrement-keep-nonzero / cleared. `spurred` decrement 5→4 still ESCAPE — and `spurred` is classified `derived`. `hubDistCap` 16→19 **ESCAPE** — classified `derived`, only an enum. `corridorPlaced := 0` + `fallback := 1` **ESCAPE** — classified `derived`, only an iff. `takeTowerSwap.from` other D8 **ESCAPE** — classified `derived`, only a ring.

r29p3 / r29p9 / r29p12 / r29p13 / r29p14 / r29p15 / r29p16 / r29p17 / r29p18 derived names still BITES. The class is still the class. `protectRadius` is *classified* `derived` and is only an enum. `hubDistCap` is *classified* `derived` and is only an enum.

**Root cause.** Criticism 141(c). They derived the two cheap leaves this review named. One of them (`maxHubDist`) is a real walk. One of them (`takeTowerSwap.from`) is a ring that now agrees with a sibling. The rest are still comments.

**Board moved?** No.

### M5 — MEDIUM, not board-affecting

**Exact `baseCut` / `protectRadius` / `hubDistCap` pick is unread. `seedScore` is not. `stubCap` is not. `maxHubDist` is not.**

`protectRadius := 0` **BITES** (not in `RADII_WIDE`). `12 → 6` inside the enum **ESCAPE**. `baseCut := 0` **BITES**. `baseCut += 1` keeping `priceyWall` on the same side of `MAX_CUT=45` **ESCAPE**. `hubDistCap := 17` **BITES** (not in `[16,19,23,999]`). `16 → 19` inside the enum **ESCAPE**. `seedScore := 0` **BITES**. `seedScore += 999` **BITES**. `stubCap` 43→51 **BITES** (the pool walk). `maxHubDist := 0` / `+= 1` **BITES** (the pre-7b walk).

Sample: E16S3 `baseCut=31` = cut. E12S4 `baseCut=30` = cut. E9S5 `baseCut=49` / cut=51. E13S8 `baseCut=80` = cut (pricey). E15S8 `baseCut=19` = cut. E1S4 `baseCut=24` / cut=26. The pick is not freeze and not the shipped cut. `protectRadius` is an enum, not this room's pick. `hubDistCap` is an enum, not this room's cohesion rung. `seedScore` is the window max. `maxHubDist` is now the walk. 46 rooms still have seed ≠ hub. The coordinates exist. The score is the walk. The cap is four legal integers.
...
- **p18 takeTowerSwap.from** is a D8 of `to`, not the vacated seat.
...
- Remaining META_DARK presence (39 names, 172/172 silent once `baseCut`+`shallowNow` are left alone)
...
- `takeTowerSwap.from` as the vacated seat (any other empty D8 of `to`)
