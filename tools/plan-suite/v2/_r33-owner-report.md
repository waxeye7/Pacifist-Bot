...
Artifact md5 `8e5d6725885bd3f3731379bedf408326` — matches the brief. Same bytes as r32. Tree HEAD is `6dd763c` (spawn-queue interleave after r29p11), not the brief's `1d7ca82`. Planner sources (`r27-gates.mjs` / `mutate.mjs`) and this artifact are unchanged since `1d7ca82`. 172/172 rooms, 0 errors. Fleet physicals re-summed off the structure lists: **10,320 extensions · 14,100 roads · 8,208 ramparts · 300 declarations · 236 notes**. Same numbers as r26–r32. Boards did not move. Baseline `checkRoom` (fleet-median stripped): **pass 172/172**.

**Verdict.** No board defect in the sample. No film-vs-board caption lie in the sample. 134(b) is still 34 adds / 27 rooms. r29p11 closes the *stated* 88 leak skip: E11S2 `20,9→19,9` + `complete=false` + regen now **BITES** (`leaks the sitter`, complete or not). The named 98 one-list append still **BITES**. They do **not** close the classes. Twin of the named 98 append (also write `lane.reserved`, keep `wanted > tiles`) **ESCAPE**. `wanted += 1` **ESCAPE**. Prefix identity-swap `18,27→19,27` **ESCAPE**. Sealing same-lap nudge `29,33→28,34` **ESCAPE**. Remaining META_DARK presence still flips 172/172 once `baseCut`+`shallowNow` are left alone. Exact pick and `seedScore` still unread. `cutPasses.rampartsDeleted` split still a log.

Throwaway probes: `tools/plan-suite/v2/_r33-owner/` (not committed).

---

## Sample

`h(room) = fmix32(fnv1a32("round33-owner|" + room))` over the 172 names. Lowest five (not cherry-picked):
...
Also churn-read as required: **E12S1 E15S4 E11S1 E2S7 E1S4**. Nine unique rooms (E11S1 is both hash #1 and mandated).
...
The 27 adopting rooms (same set as r28–r32):
...
E21S2 (in this hash sample) is a net-zero-count adopter: freeze 56, shipped 56, one remove (`17,45`) and one add (`18,44`). E15S4 (churn) is the same class: one remove `37,7`, one add `36,8`.
...
## Closed since r32 — re-probed

**r29p11 / 88 leak skip.** E11S2 last fat has `20,9` and is complete. Nudge `20,9→19,9` (sitter leaks, lap 0) **BITES**. Same nudge **and** `complete=false` + regen the paragraph **BITES** — first fail is now `leaks the sitter` (`complete or not`), not a declaration-content miss. Fleet: **0** incomplete discarded rungs, **0** of 327 complete discarded cuts leak the sitter. The flag no longer skips the seal.
...
**Lab diamond:** 10 labs, 4×4 bbox, 6 holes in all 9. Mandated stamp. Haul cheb 2–5. E11S1 haul 5, E21S5 / E11S3 / E12S1 / E15S4 / E2S7 / E1S4 haul 3, E21S2 / E8S6 haul 2.
...
**E11S1.** 32-cut, 43 ramparts, 101 roads, 60/60. Real shrink: `fullRun.reserved` === `lane.reserved` === 6 tiles / 2 rounds. `wanted=7`, `used=3`, `ext=60`, `shallow=3`. Suffix COORD list is gone. Seed `25,39` ≠ hub `24,41`. Controller unenclosed. Mineral `23,7` off-network, declared (extractor 23,7). Eco 27-tile controller walk, declared. 3 2×2 on a road spine. 0 D4-blind. 0 redundant. Labs haul 5. Film: 32 crossing / 4 seat.outside / 3 cover / 4 ring. Reserved board: `18,27 19,28 19,29 | 24,28 25,27 25,29`. `19,27` is D8 of `18,27` and `19,28` and is not in the set. The 98 residue room hashed first. Quiet board, unread reserved set.

**E21S2.** 56-cut, 59 ramparts, 83 roads. Adopter (in the 27): freeze 56 → shipped 56, remove `17,45` / add `18,44`. `baseCut=56`. Mobility **3.8** declared (page and film agree). Controller enclosed. Mineral `44,41` far SE off-network, declared. Labs variant `anti`, haul 2. 0 2×2. 0 D4-blind. Seed=hub `30,32`. Film: 56 crossing / 1 seat.outside / 2 cover. Long wall + expensive lap, both named.

**E8S6.** 60-cut, 65 ramparts, 83 roads. A basin. `baseCut=60` = cut. `protectRadius=8`. Mobility 0. Controller enclosed. Mineral `17,21` **on** the network, next to the sitter. Seed `22,22` ≠ hub `23,21`. 2 2×2 on a spine. 0 D4-blind. 0 shortfalls. 0 notes. Film: 60 crossing / 2 seat.outside / 3 cover. The 60-tile wall is the ugliness the room does not price at all.

**E21S5.** 42-cut, 47 ramparts, 105 roads. `baseCut=39 ≠ 42`. `protectRadius=9`. Controller enclosed. Mineral `6,25` off-network west, declared. Eco source-path tax 53 (gate 52), declared. Seed=hub `24,30`. 0 2×2. 0 redundant. Labs haul 3. Film: 42 crossing / 3 seat.outside / 2 cover. Roads are the basin tax, and the room says so.

**E11S3.** 32-cut, 37 ramparts, 80 roads. Plain 60/0 (`fullRun.ran=false`, reserved 3 tiles, no shrink). The invent-shrink control room. `baseCut=32`. Controller unenclosed. Mineral `13,36` off-network, declared. Eco 33-tile controller walk, declared. Seed=hub `28,28`. 0 2×2. 0 redundant. Film: 32 crossing / 2 seat.outside / 1 cover / 2 ring. Quiet shell, expensive controller, named.
...
No sampled room is a checkerboard, a solid brick, or a maze. Extension mass is corridor-flanked (**0 D4-blind extensions in all 9**). 2×2 squares exist in E11S1 (3) and E8S6 (2) and they sit on a road spine. E21S2 is a forced 56-tile wall at lap 3.8. E8S6 is a forced 60-tile basin. E21S5 / E11S1 / E11S3 / E12S1 spend roads on eco, not on a city grid. Towers are not a hub clump (sample clumps 2–3).

Nothing in the sample looks accidental. The ugly rooms are ugly for a named, declared reason (E21S2 lap 3.8 + 56-tile wall, E21S5 105-road eco, E11S1 27-tile controller walk, E11S3 33-tile controller walk) — except **E8S6**, whose 60-cut is silent.
...
**Exploit.** E11S1 after r29p10/p11: `fullRun.reserved` === `lane.reserved` === 6 tiles / 2 rounds. `wanted=7`. The refused tail is a count.
...
r29p10 dropped the suffix COORD list and required `fullRun.reserved === lane.reserved` and `wanted > tiles`. r29p11 did not touch this. A tile that *is* this room's floor and *touches* the walk can still be added, or swapped for a kept prefix tile, if both lists stay twins. The refused walk is now four unread integers (`wanted`, `used`, `ext`, `shallow`) plus `premium`.
...
**Root cause.** Discarded `cutTiles` are a free list held to `enclosureMobility` of themselves, plus "not freeze/shipped", plus "not shorter-and-prettier", plus "seals". The composed enclosure is not on the board. r29p11 gated leaks on the flood, not the flag. It did not bind the list.
...
Sample: E11S1 `baseCut=32` = freeze = cut. E21S2 `baseCut=56` = cut. E8S6 `baseCut=60` = cut. E21S5 `baseCut=39` / freeze=42 / cut=42. E1S4 `baseCut=24` / cut=26. The pick is not freeze and not the shipped cut. `protectRadius` is an enum, not this room's pick. `seedScore` is a finite number, not the confluence walk. 46 rooms still have seed ≠ hub. The coordinates exist. The score does not.
...
Boards are clean. The gallery in the sample is readable and matches the tiles. 134(b) as a **number** stays done. 93, the *stated* 98 one-list append, the *stated* 88 leak-on-complete **and** leak-on-incomplete, and the four r29p9 presence names are done. The 98 class is not done: reserved is still a free set and the refused walk is still a count. The 88 class is not done: a sealing same-lap neighbour still passes. Plus the presence class, plus the exact pick, plus `seedScore`, plus one `cutPasses` column.
