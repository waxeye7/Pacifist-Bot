/**
 * Round-37 fleet inventory + hashed sample. Terrain + shipped lists. checkRoom
 * on sample and a stripped 172-room baseline. Never writes the artifact.
 */
import fs from "fs";
...
const fullRun = { n: 0, ran: 0, shrunk: 0, dropped: 0, plain: 0, reservedMissing: 0, reservedEqLane: 0, reservedGtLane: 0 };
const shrunk = [];
const dropped = [];
const plain60 = [];
const ladders = [];
...
for (const p of plans) {
  ...
  if (L?.fullRun) {
    ...
    if (Array.isArray(L.fullRun.reserved) && Array.isArray(L.reserved)) {
      ...
      if (a === b) fullRun.reservedEqLane++;
      else if (L.fullRun.reserved.length > L.reserved.length) fullRun.reservedGtLane++;
    }
    const row = {
      room: p.room,
      tiles: L.tiles,
...
function inspectRoom(plan) {
  ...
  return {
    ...
    lane: L && {
      ...
      ran: L.fullRun?.ran,
      shrunk: L.shrunk || null,
      reserved: Array.isArray(L.reserved) ? L.reserved : null,
      frReserved: Array.isArray(L.fullRun?.reserved) ? L.fullRun.reserved : null,
    },
...
let basePass = 0, baseFail = 0, baseDecl = 0;
const baseFirst = [];
for (const p of plans) {
  const d = byRoom.get(p.room);
  if (!d) {
    baseFail++;
    baseFirst.push({ room: p.room, fails: ["no terrain"] });
    continue;
  }
  const res = checkRoom(p, d.terrain, d.objects, null);
  const fails = realFails(res);
  baseDecl += res.declared || 0;
  if (fails.length) {
    baseFail++;
    if (baseFirst.length < 8) baseFirst.push({ room: p.room, fails: fails.slice(0, 2) });
  } else basePass++;
}
...
const out = {
  ...
  cutAdopted,
  cutPasses,
  baseline: { pass: basePass, fail: baseFail, declared: baseDecl, firstFails: baseFirst },
  sample,
};
...
console.log(JSON.stringify({
  ...
  seed,
  film: out.film,
  cutPasses: { ...cutPasses, swapable: cutPasses.swapable.slice(0, 4) },
  ...
  sample: sample.map((s) => ({
    ...
    reserved: s.lane && { n: s.lane.reserved?.length, fr: s.lane.frReserved?.length, wanted: s.lane.shrunk?.wanted },
  })),
}, null, 2));
