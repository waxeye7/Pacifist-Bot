 ...
 * checkRoom only. Does not write the artifact. Round 44.
 */
import fs from "fs";
...
const mutatedTwPlus = JSON.parse(JSON.stringify(plans));
let twPlusN = 0;
for (const p of mutatedTwPlus) {
  if (typeof p.meta?.towers?.mobilityVeto?.baseLap === "number") {
    p.meta.towers.mobilityVeto.baseLap += 1;
    twPlusN++;
  }
}
const fleetTwPlus = await summarize("p22-towers-baseLap-plus-1", mutatedTwPlus);
...
console.log(JSON.stringify({
  ...
  twPlusN,
  fleetTwPlus: { pass: fleetTwPlus.pass, fail: fleetTwPlus.fail, first: fleetTwPlus.firstFails.slice(0, 4) },
