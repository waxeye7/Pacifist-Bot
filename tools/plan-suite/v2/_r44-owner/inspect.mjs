/**
 * Hash sample + residue-room inventory. No validate import. Round 44.
 */
import fs from "fs";
...
const churn = ["E11S1", "E2S7", "E1S4", "E11S7"];
...
for (const p of plans) {
  ...
  if (typeof mob?.floorGated === "number") p15.floorGated.push({ room: p.room, n: mob.floorGated });
  if (typeof mob?.floorOver === "number") p15.floorOver.push({ room: p.room, n: mob.floorOver });
  if (typeof mob?.floorOverGated === "number") p15.floorOverGated.push({ room: p.room, n: mob.floorOverGated });
  if (typeof mob?.worst?.freeDin === "number") p15.freeDin.push({ room: p.room, n: mob.worst.freeDin });
  if (typeof mob?.massAdds === "number") p15.massAdds.push({ room: p.room, n: mob.massAdds, worst: mob.worst?.massAdds });
  if (typeof p.meta?.roadOrder?.maxDist === "number") p15.maxDist.push({ room: p.room, n: p.meta.roadOrder.maxDist });
  if (typeof p.meta?.extensions?.deepReach === "number") {
    p15.deepReach.push({
      room: p.room,
      n: p.meta.extensions.deepReach,
      cap: p.meta.extensions.hubDistCap,
    });
  }
  if (typeof p.meta?.extensions?.stubCap === "number") {
    p15.stubCap.push({ room: p.room, n: p.meta.extensions.stubCap });
  }
  const seat = p.meta?.mineralSeatAtReservation;
  if (seat && Number.isInteger(seat.x)) p15.mineralSeatAtReservation.push({ room: p.room, k: `${seat.x},${seat.y}` });
  const appr = p.meta?.mineralApproachAtReservation;
  if (appr && Number.isInteger(appr.x)) p15.mineralApproachAtReservation.push({ room: p.room, k: `${appr.x},${appr.y}` });
}
const p15sum = {
  floorGatedN: p15.floorGated.length,
  floorGatedNz: p15.floorGated.filter((r) => r.n !== 0).length,
  floorOverN: p15.floorOver.length,
  floorOverNz: p15.floorOver.filter((r) => r.n !== 0).length,
  floorOverGatedN: p15.floorOverGated.length,
  floorOverGatedNz: p15.floorOverGated.filter((r) => r.n !== 0).length,
  freeDinN: p15.freeDin.length,
  freeDinNz: p15.freeDin.filter((r) => r.n !== 0).length,
  massAddsN: p15.massAdds.length,
  massAddsNz: p15.massAdds.filter((r) => r.n !== 0).length,
  maxDistN: p15.maxDist.length,
  maxDistNz: p15.maxDist.filter((r) => r.n !== 0).length,
  deepReachN: p15.deepReach.length,
  deepReachVals: [...new Set(p15.deepReach.map((r) => r.n))].sort((a, b) => a - b),
  stubCapN: p15.stubCap.length,
  stubCapVals: [...new Set(p15.stubCap.map((r) => r.n))].sort((a, b) => a - b),
  stubCap43: p15.stubCap.filter((r) => r.n === 43).length,
  stubCap51: p15.stubCap.filter((r) => r.n === 51).length,
  mineralSeatN: p15.mineralSeatAtReservation.length,
  mineralApproachN: p15.mineralApproachAtReservation.length,
};
...
const out = {
  ...
  p16: (() => {
    ...
    const fallback = {};
    for (const p of plans) {
      const c = p.meta?.extensions?.hubDistCap;
      if (typeof c === "number") caps[c] = (caps[c] || 0) + 1;
      const lap = p.meta?.walls?.reflow?.lapCeilingFloor ?? p.meta?.extensions?.reflow?.lapCeilingFloor;
      if (typeof lap === "number") laps[lap] = (laps[lap] || 0) + 1;
      const pl = p.meta?.extensions?.corridorPlaced;
      const fb = p.meta?.extensions?.corridorFallback;
      if (typeof pl === "number") placed[pl] = (placed[pl] || 0) + 1;
      if (typeof fb === "number") fallback[fb] = (fallback[fb] || 0) + 1;
    }
    return { hubDistCap: caps, lapCeilingFloor: laps, corridorPlaced: placed, corridorFallback: fallback };
  })(),
  p17: (() => {
    const stub = {};
    const fu = { n: 0, nz: 0 };
    const radii = { present: 0, absent: 0, vals: {} };
    const park = [];
    const swaps = [];
    for (const p of plans) {
      const sc = p.meta?.extensions?.stubCap;
      if (typeof sc === "number") stub[sc] = (stub[sc] || 0) + 1;
      const fuv = p.meta?.extensions?.laneMeta?.floorUngated;
      if (typeof fuv === "number") {
        fu.n++;
        if (fuv !== 0) fu.nz++;
      }
      const rad = p.meta?.composeOpts?.radii;
      if (Array.isArray(rad)) {
        radii.present++;
        const k = rad.join(",");
        radii.vals[k] = (radii.vals[k] || 0) + 1;
      } else if (p.meta?.composeOpts) radii.absent++;
      if (typeof p.meta?.composeOpts?.parkCap === "number") {
        park.push({
          room: p.room,
          parkCap: p.meta.composeOpts.parkCap,
          floorCap: p.meta.ctrlParkFloorCap,
        });
      }
      if (p.meta?.composeOpts?.takeTowerSwap?.to) {
        swaps.push({
          room: p.room,
          to: p.meta.composeOpts.takeTowerSwap.to,
          from: p.meta.composeOpts.takeTowerSwap.from,
        });
      }
    }
    return { stub, floorUngated: fu, radii, park, parkN: park.length, swaps, swapN: swaps.length };
  })(),
  p18: (() => {
    const maxHub = {};
    const swaps = [];
    for (const p of plans) {
      ...
      if (typeof mh === "number") maxHub[mh] = (maxHub[mh] || 0) + 1;
      if (p.meta?.composeOpts?.takeTowerSwap) {
        const sw = p.meta.composeOpts.takeTowerSwap;
        const cheb = sw.from && sw.to
          ? Math.max(Math.abs(sw.from.x - sw.to.x), Math.abs(sw.from.y - sw.to.y))
          : null;
        swaps.push({ room: p.room, from: sw.from, to: sw.to, cheb });
      }
    }
    return { maxHub, swaps, swapN: swaps.length };
  })(),
  p19: (() => {
    const served = {};
    const unreach = {};
    ...
    const swaps = [];
    for (const p of plans) {
      const se = p.meta?.walls?.servedExts;
      const ue = p.meta?.walls?.unreachableExts;
      const uc = p.meta?.walls?.unreachedClusters;
      const ft = p.meta?.walls?.fillerTiles;
      if (typeof se === "number") served[se] = (served[se] || 0) + 1;
      if (typeof ue === "number") unreach[ue] = (unreach[ue] || 0) + 1;
      if (typeof uc === "number") clusters[uc] = (clusters[uc] || 0) + 1;
      if (typeof ft === "number") filler[ft] = (filler[ft] || 0) + 1;
      if (p.meta?.composeOpts?.takeTowerSwap) {
        const sw = p.meta.composeOpts.takeTowerSwap;
        const taken = p.meta?.towers?.acrossPriorTake?.taken;
        const offer = p.meta?.towers?.towerSwapOffer?.best;
        swaps.push({
          room: p.room,
          from: sw.from,
          to: sw.to,
          takenFrom: taken?.from || null,
          offerFrom: offer?.from || null,
          fromEqTaken: !!(taken?.from && sw.from && taken.from.x === sw.from.x && taken.from.y === sw.from.y),
        });
      }
    }
    return { served, unreach, clusters, filler, swaps, swapN: swaps.length };
  })(),
