/**
 * Named r29p11–p21 closes + leftover. checkRoom on clones only. Regen decls.
 */
import fs from "fs";
...
{
  const lapR = findRoom((p) => typeof p.meta?.misc?.mobilityVeto?.baseLap === "number" && p.meta.misc.mobilityVeto.baseLap !== 0);
  const lap0 = findRoom((p) => typeof p.meta?.misc?.mobilityVeto?.baseLap === "number");
  const labsR = findRoom((p) => typeof p.meta?.labs?.lapVeto?.baseLap === "number" && p.meta.labs.lapVeto.baseLap !== 0);
  const nkR = findRoom((p) => typeof p.meta?.misc?.mobilityVeto?.nuker?.baseLap === "number" && p.meta.misc.mobilityVeto.nuker.baseLap !== 0);
  const obR = findRoom((p) => typeof p.meta?.misc?.mobilityVeto?.observer?.baseLap === "number" && p.meta.misc.mobilityVeto.observer.baseLap !== 0);
  const twR = findRoom((p) => typeof p.meta?.towers?.mobilityVeto?.baseLap === "number" && p.meta.towers.mobilityVeto.baseLap !== 0);
  const rpR = findRoom((p) => typeof p.meta?.walls?.mobility?.repair?.tower?.baseLap === "number" && p.meta.walls.mobility.repair.tower.baseLap !== 0);
  out.p21_targets = {
    baseLap: lapR?.room,
    baseLapN: lapR?.meta?.misc?.mobilityVeto?.baseLap,
    baseLapAny: lap0?.room,
    baseLapAnyN: lap0?.meta?.misc?.mobilityVeto?.baseLap,
    labsLap: labsR?.room,
    labsLapN: labsR?.meta?.labs?.lapVeto?.baseLap,
    nukerLap: nkR?.room,
    nukerLapN: nkR?.meta?.misc?.mobilityVeto?.nuker?.baseLap,
    observerLap: obR?.room,
    observerLapN: obR?.meta?.misc?.mobilityVeto?.observer?.baseLap,
    towersLap: twR?.room,
    towersLapN: twR?.meta?.towers?.mobilityVeto?.baseLap,
    repairLap: rpR?.room,
    repairLapN: rpR?.meta?.walls?.mobility?.repair?.tower?.baseLap,
  };
  if (lapR) zeroJudge("p21_baseLap_zeroed", lapR.room, (p) => { p.meta.misc.mobilityVeto.baseLap = 0; });
  if (lap0 && (!lapR || lap0.room !== lapR.room)) {
    zeroJudge("p21_baseLap_plus_1", lap0.room, (p) => { p.meta.misc.mobilityVeto.baseLap += 1; });
  } else if (lapR) {
    zeroJudge("p21_baseLap_plus_1", lapR.room, (p) => { p.meta.misc.mobilityVeto.baseLap += 1; });
  }
  if (labsR) zeroJudge("p21_labs_baseLap_zeroed", labsR.room, (p) => { p.meta.labs.lapVeto.baseLap = 0; });
  if (labsR) zeroJudge("p21_labs_baseLap_plus_1", labsR.room, (p) => { p.meta.labs.lapVeto.baseLap += 1; });
  if (nkR) zeroJudge("p21_nuker_baseLap_zeroed", nkR.room, (p) => { p.meta.misc.mobilityVeto.nuker.baseLap = 0; });
  if (obR) zeroJudge("p21_observer_baseLap_zeroed", obR.room, (p) => { p.meta.misc.mobilityVeto.observer.baseLap = 0; });
  if (twR) zeroJudge("p21_towers_baseLap_zeroed", twR.room, (p) => { p.meta.towers.mobilityVeto.baseLap = 0; });
  if (rpR) zeroJudge("p21_repair_baseLap_zeroed", rpR.room, (p) => { p.meta.walls.mobility.repair.tower.baseLap = 0; });
}

{
  const e7 = clone("E11S7");
  ...
  out.p22_E11S7 = {
    ...
    repair: ((e7.meta?.shortfalls || []).find((s) => s?.repair?.tower) || {}).repair?.tower?.baseLap,
  };
  out.p22_towers_baseLap_zeroed = judge("p22 E11S7 towers.baseLap := 0", (() => {
    const p = clone("E11S7");
    p.meta.towers.mobilityVeto.baseLap = 0;
    return p;
  })(), d7).status;
  out.p22_towers_baseLap_plus_1 = judge("p22 E11S7 towers.baseLap += 1", (() => {
    const p = clone("E11S7");
    p.meta.towers.mobilityVeto.baseLap += 1;
    return p;
  })(), d7).status;
  out.p22_towers_baseLap_to_veto = judge("p22 E11S7 towers.baseLap := veto", (() => {
    const p = clone("E11S7");
