/**
 * Extra r44 hunts: remaining-presence skip after p22; p22/p21/p20/p19 named; 98/88 residues.
 */
import fs from "fs";
...
}
{
  ...
  if (swFrom) {
    ...
    if (alt) extra.push(await run(`p19 takeTowerSwap.from other D8 ${alt.x},${alt.y} composeOpts only`, swFrom.room, (p) => { p.meta.composeOpts.takeTowerSwap.from = { x: alt.x, y: alt.y }; }));
    if (alt) extra.push(await run("p19 takeTowerSwap.from other D8 + rewrite published take", swFrom.room, (p) => {
...
}
{
  ...
  if (ucR) extra.push(await run("p19 unreachedClusters += 1", ucR.room, (p) => { p.meta.walls.unreachedClusters += 1; }));
  if (ueR) extra.push(await run("p19 unreachableExts += 1", ueR.room, (p) => { p.meta.walls.unreachableExts += 1; }));
  if (seR) extra.push(await run("p19 servedExts += 1", seR.room, (p) => { p.meta.walls.servedExts += 1; }));
  if (seR) extra.push(await run("p19 servedExts += 1 and fillerTiles += 1", seR.room, (p) => {
...
}
{
  ...
  if (bogR) extra.push(await run("p20 baseOverGated := 0", bogR.room, (p) => { p.meta.misc.mobilityVeto.baseOverGated = 0; }));
  if (bogR) extra.push(await run("p20 baseOverGated += 1", bogR.room, (p) => { p.meta.misc.mobilityVeto.baseOverGated += 1; }));
  if (wasR) extra.push(await run("p20 wasLap := 0", wasR.room, (p) => {
    for (const r of p.meta.misc.mobilityVeto.refused || []) {
      if (r && typeof r.wasLap === "number") r.wasLap = 0;
    }
  }));
  if (wasR) extra.push(await run("p20 wasLap += 1", wasR.room, (p) => {
    for (const r of p.meta.misc.mobilityVeto.refused || []) {
...
}
{
  ...
  if (lapR) extra.push(await run("p21 baseLap := 0", lapR.room, (p) => { p.meta.misc.mobilityVeto.baseLap = 0; }));
  if (lapR) extra.push(await run("p21 baseLap += 1", lapR.room, (p) => { p.meta.misc.mobilityVeto.baseLap += 1; }));
  if (labsR) extra.push(await run("p21 labs.baseLap := 0", labsR.room, (p) => { p.meta.labs.lapVeto.baseLap = 0; }));
  if (labsR) extra.push(await run("p21 labs.baseLap += 1", labsR.room, (p) => { p.meta.labs.lapVeto.baseLap += 1; }));
  if (nkR) extra.push(await run("p21 nuker.baseLap := 0", nkR.room, (p) => { p.meta.misc.mobilityVeto.nuker.baseLap = 0; }));
  if (obR) extra.push(await run("p21 observer.baseLap := 0", obR.room, (p) => { p.meta.misc.mobilityVeto.observer.baseLap = 0; }));
  if (twR) extra.push(await run("p21 towers.baseLap := 0", twR.room, (p) => { p.meta.towers.mobilityVeto.baseLap = 0; }));
  if (rpR) extra.push(await run("p21 repair.tower.baseLap := 0", rpR.room, (p) => { p.meta.walls.mobility.repair.tower.baseLap = 0; }));
}
{
  extra.push(await run("p22 E11S7 towers.baseLap := 0", "E11S7", (p) => { p.meta.towers.mobilityVeto.baseLap = 0; }));
  extra.push(await run("p22 E11S7 towers.baseLap += 1", "E11S7", (p) => { p.meta.towers.mobilityVeto.baseLap += 1; }));
  extra.push(await run("p22 E11S7 towers.baseLap := veto 7.67", "E11S7", (p) => {
    p.meta.towers.mobilityVeto.baseLap = p.meta.misc.mobilityVeto.baseLap;
  }));
}

const hashed = hashedRooms(plans).slice(0, 5).map((r) => r.room);
const names = [...new Set([...hashed, "E11S1", "E2S7", "E1S4", "E11S7"])];
