# Round 41 mechanical review

Artifact `tools/plan-suite/out-v2/plans-hub.json` md5 `8e5d6725885bd3f3731379bedf408326` (27,780,512 bytes) — **matches expected**. HEAD `c004ef4` (r29p19: derive leftover presence — `takeTowerSwap.from` is the vacated seat the published swap names, `unreachedClusters` shipped-cut clusters of size ≥2 that no road D8-touches, `unreachableExts` extensions with no D4 road, `servedExts` 0 iff the filler laid nothing). Artifact bytes unchanged since r32. p19 is a gate + mutate-corpus pass (`r27-gates.mjs`, `mutate.mjs`). Rooms dump `tools/plan-suite/v2/_r28-mech/rooms.json` (172/172, 0 missing). Node `v22.13.0` (`fnm` install on PATH). Probes in `tools/plan-suite/v2/_r41-mech/`. No suite source committed. Artifact not written. **No board moved.** No `boards` field. Physicals still 10,320 / 14,100 / 8,208.
...
A gate that agrees with itself is not a gate. r29p19 closed the *named* leftover flatten (`takeTowerSwap.from` other D8 / `unreachedClusters += 1` / `unreachableExts += 1` / `servedExts += 1`). r29p18 closed `maxHubDist` walk and bound `from` to the D8 ring of `to`. r29p17 closed the five leftover flattens (`stubCap` 43↔51 / `floorUngated` / `radii` rewrite / `parkCap` / `takeTowerSwap.to` off-board). r29p16 closed the four leftover flattens (`hubDistCap` ladder / `lapCeilingFloor` / `corridorPlaced` iff). r29p15 closed the ten leftover walks and bound `seedScore`. r29p14 closed the nine leftover walks and `boundLap`. r29p13 closed the three leftover walks and the stitch 0/1 flag. r29p12 closed the five leftover presence walks. r29p11 closed leaky+`complete=false`. r29p10 closed one-copy `"19,27"`. The kept prefix is still a D8-neighbor bag. `wanted` is still a count. The last discarded rung is still a free enclosure that walks its own lap. `hubDistCap` is an enum. The presence class still flips once `baseCut`+`shallowNow` are left with the derived set. `protectRadius` is an enum. `takeTowerSwap.from` is the published take pair (other D8 BITES; rewriting the twins BITES the note / adjacency / offered book). `servedExts` is 0 iff filler laid nothing — fleet 172/172 is `0/0`.

---

## BLOCKING

None on the board. r28 mineral-why / swap-offer doors stay closed. 93 plant-back stays closed. 88 leaky + `complete=false` stays closed. Named 98 one-copy stays closed. The five p12 walks stay closed. p13 three walks + stitch 0/1 stay closed. p14 nine names (and `boundLap`) stay closed. p15 ten named flattens stay closed. `seedScore := 0` / `+= 999` stay closed. Named p16 four stay closed. Named p17 five stay closed. Named p18 two stay closed. Named p19 four stay closed.
...
**Exploit.** E11S1 is a real shrink. After r29p10 the suffix is gone: `fullRun.reserved === lane.reserved` (6 tiles / 2 rounds), `shrunk.wanted=7`. The refused extra is a count. p19 did not touch this.
...
**Exploit.** E11S2 rungs: `1.56/46/37 ×3`, then `2.5/50/42` (last, eco-capped). Shipped wall 46/37 = freeze. Last fat is complete, 42 tiles, has `20,9` and `29,33`. p19 did not touch this.
...
**Exploit.** 36 presence names, 63 derived (p19 moved three: `unreachedClusters` / `unreachableExts` / `servedExts`; `takeTowerSwap` was already derived in p17 and is now the published take pair). Zeroed every remaining `klass:"presence"` numeric/bool/nonempty-array leaf across all 172 rooms. 17 kinds, 1,250 events. `checkRoom`: **fail 172/172**, every first fail is `baseCut := 0`.

Exclude `baseCut` only: **fail 3/172** — first fail is the `shallowNow` note (E12S6, E2S3, E9S2). Exclude `baseCut`+`shallowNow`: **pass 172/172**. Same 172/172 if those two plus all newly derived names (p12–p19) are skipped. Same 172/172 if those two plus **every** `klass:"derived"` name are skipped (15 leftover kinds, 1,075 events). The r38 "minus derived+baseCut+shallowNow is silent" door is still open.

Single-room flatten, flattering direction — **12 ESCAPE** on the first copy:

`baseOverGated`, `deepExhausted`, `digRoads`, `freeLeft`, `haulCost`, `inertPromoted`, `rescuedLap`, `rescuedTo`, `rolledBackFrom`, `shallowRamparts`, `stubExhausted`, `wasLap`
...
Closed this artifact (re-probed, BITES): `nukerInWindow`, `center`, `mineralSeatNetTiles`, `coveredDetourDeclared`, **p12 five**, **p13 three**, **p14 nine** + `boundLap`, **p15 ten**, **`seedScore`**, **p16 four**, **p17 five**, **p18 two**, **p19 four** (`takeTowerSwap.from` other D8 / `unreachedClusters` walk / `unreachableExts` walk / `servedExts` 0-iff-filler).
...
**Root cause.** Criticism 141(c). They derived the three cheap leaves this review named and bound `from` to the published take. Two of the three are real walks (both 0 on this fleet). The third is 0 iff filler laid nothing. The class is still the class.
...
### 4. Exact `baseCut` / `protectRadius` / `hubDistCap` picks are unread. `maxHubDist` / `stubCap` 43↔51 / p19 three are closed.
...
Sample: E12S7 `baseCut=32` / cut=35. E6S2 `baseCut=48` = cut (pricey). E6S3 `baseCut=60` / cut=64 (pricey).
...
### 7. p19 four — named flatten BITES; `servedExts` is a boolean; `from` is the published pair.

| attack | result |
|---|---|
| `takeTowerSwap.from` → another D8 of `to` (E14S1 `34,25 → 35,24`) | **BITES** (published swap is `34,25 → 34,24`) |
| `to` → another shipped tower **and** `from` → a D8 of the new `to` | **BITES** (same published-pair gate) |
| rewrite `from` **and** the published twins (`acrossPriorTake.taken` / `towerSwapTaken` / offer / sap) | **BITES** (note + adjacency occupancy + offered book) |
| `unreachedClusters += 1` | **BITES** (0 vs 0 shipped-cut clusters of size ≥2 that D8-touch no road) |
| `unreachableExts += 1` | **BITES** (0 vs 0 shipped extensions with no D4 road) |
| `servedExts += 1` | **BITES** (1 vs filler laid 0) |
| `servedExts := 0` | **ESCAPE** (already 0 — no-op) |
| `servedExts` **and** `fillerTiles`/`extFace` both `+= 1` | **BITES** (late-road book: laid vs ships+lost) |

Fleet `{servedExts: {0:172}, unreachableExts: {0:172}, unreachedClusters: {0:172}, fillerTiles: {0:172}}`. 172/172 `served === 0` iff `filler === 0`.

`unreachedClusters` is the walk. `unreachableExts` is the walk. `servedExts` is 0 iff the filler laid nothing — not a count of served extensions. On this fleet the filler laid nothing, so the integer is constantly 0. A future room with `fillerTiles > 0` can still publish any positive integer. `takeTowerSwap.from` is the published take pair, not "a D8 of `to`". Forging it means rewriting the take record (note / adjacency / offered), not moving one COORD.

### 8. p18 two / p17 five / p16 four / p15 ten — named flatten BITES; identities remain.

| attack | result |
|---|---|
| `maxHubDist := 0` / `+= 1` | **BITES** |
| `takeTowerSwap.from → 1,1` | **BITES** (D8 ring, and now the published pair) |
| `stubCap` 43↔51 / `floorUngated := 0` / `radii` rewritten or planted / `parkCap := 0` / `takeTowerSwap.to → 1,1` | **BITES** |
| late radii → wide, bonus held / wide → late, bonus held | **BITES** |
| late radii **and** `needDeepBonus := 25` | **ESCAPE** |
| `hubDistCap := 0` / `:= 17` | **BITES** |
| `lapCeilingFloor := 0` / `corridorPlaced := 0` on 60/`fallback=0` / `corridorFallback := 1` on 60 | **BITES** |
| `hubDistCap` 16↔19 / 16→23 / 16→999 | **ESCAPE** |
| `corridorPlaced := 0` on a fallback room (E2S5, placed 59) | **ESCAPE** |
| `floorGated` / `floorOver` / `floorOverGated` / `freeDin` / `massAdds` / `maxDist` / `deepReach` / `stubCap` zeroed | **BITES** |
| reserved seat / approach moved to `1,1` | **BITES** |
...
Independent single-removal over all **7,246** `cutAtFreeze` tiles: **0 loose**, 0 core-seal fails, 29 rooms differ from the shipped cut, 34 add + 46 remove, 27 adopt / 29 drift. Same census as r29–r40.
...
## Attacked and held (r40 closures + r29p19 named, re-probed)
...
`h(room) = fmix32(fnv1a32("round41-mech|" + room))` over the 172 names. Lowest five (not cherry-picked):
...
|---|---|---|---|---|
| E12S7 | 60/116/45/35/35 | 0 | 25,32 off | hashed #1, seed=hub, pick 32 ≠ cut 35, `protectRadius=11` |
| E6S2 | 60/72/57/48/48 | 0 | 28,6 off | **pricey** 48, seed `35,28` ≠ hub `36,28`, `protectRadius=10` |
...
| E16S9 | 60/83/53/50/50 | 0 | 21,30 off | **pricey** 50, seed=hub, `protectRadius=10` |
...
Nothing in the sample looks accidental. The expensive rooms say so (E11S2 lap 1.5, E11S7 lap 8.67 + take, four of the hash five pricey). Two of the hash five publish a `baseCut` that is not the shipped cut (E12S7 32≠35, E6S3 60≠64).

---

## Clean re-derivations (terrain + shipped structure lists)

From `_r41-mech/rederive.mjs` (does not import `validate.mjs`):
...
Findings 1–3 are the round. r29p19 closed the *named* leftover flatten (`from` other D8 / three service census leaves). The prefix is still a neighbor list. The last discarded rung is still a free enclosure that seals. A closer who only adds another identity over producer leaves will ship this review again.
...
**No.** Named p19 four leftover flattens / named p18 two leftover flattens / named p17 five leftover flattens / named p16 four leftover flattens / named p15 ten leftover walks + `seedScore` / named p14 nine + `boundLap` / named p13 three + stitch 0/1 / named p12 five / named 88 leaky+`complete=false` / named 98 `19,27` on one copy / 88 leaky complete / named 98 COORD bag / 88 shipped-cut / 88 8-tile box / 93 plant-back closed. The kept prefix is a D8-neighbor bag. `wanted` is a count. The last discarded rung is a free sealing enclosure. The presence class still flips once `baseCut`+`shallowNow` are left alone. `hubDistCap` / `protectRadius` are enums. `takeTowerSwap.from` is the published take pair. `servedExts` is a boolean of the filler book. `radii` is a bonus table. The reserved mineral pair is a topology bound. `clean_rounds` is 0. The bar is 2 consecutive all-clean. Boards did not move.
