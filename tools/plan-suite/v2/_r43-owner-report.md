...
Artifact md5 `8e5d6725885bd3f3731379bedf408326` — matches the brief. Same bytes as r32–r42. Tree HEAD is `9abfe87` (r29p21: `baseLap` the freeze-cut gated lap with the extension mass / nuker / observer lifted, on `misc.mobilityVeto` and `labs.lapVeto`), parent `9806dd8` (r29p20 `baseOverGated` / `wasLap` that walk). 172/172 rooms, 0 errors. Fleet physicals re-summed off the structure lists: **10,320 extensions · 14,100 roads · 8,208 ramparts · 300 declarations · 236 notes**. Same numbers as r26–r42. Boards did not move. Baseline `checkRoom` (fleet-median stripped): **pass 172/172**.

**Verdict.** No board defect in the sample. No film-vs-board caption lie in the sample. 134(b) is still 34 adds / 27 rooms. r29p21 closes the *stated* leftover flatten: `baseLap := 0` **BITES** (fleet 57/57), `baseLap += 1` **BITES** (fleet 172/172), `labs.lapVeto.baseLap` the same. `wasLap` still **BITES**. `baseOverGated` still **BITES**. `seedScore` still **BITES**. The p20 two + p19 four + p18 two + p17 five + p16 four + p15 ten + p14 nine + p12 five still **BITES**. The named 88 leak + `complete=false` still **BITES**. The named 98 one-list append still **BITES**. They do **not** close the classes. Twin of the named 98 append (also write `lane.reserved`, keep `wanted > tiles`) **ESCAPE**. `wanted += 1` **ESCAPE**. Sealing same-lap nudge `29,33→28,34` **ESCAPE**. `hubDistCap` 16→19 in-enum **ESCAPE** (fleet 170/170). Remaining META_DARK presence still flips 172/172 silent once `baseCut`+`shallowNow` are left alone. Exact pick still unread. `protectRadius` is still an enum. `hubDistCap` is still an enum. `baseLap` is now the freeze-cut gated lap on the two copies they named. `wasLap` is still that number copied onto every refusal (6/6 === `baseLap`). `towers.mobilityVeto.baseLap` is a declaration twin of `shortfalls.repair.tower.baseLap`, not that walk — E11S7 ships 7.33 against a freeze-cut 7.67. `cutPasses.rampartsDeleted` split still a log.

Throwaway probes: `tools/plan-suite/v2/_r43-owner/` (not committed).

---

## Sample

`h(room) = fmix32(fnv1a32("round43-owner|" + room))` over the 172 names. Lowest five (not cherry-picked):
...
The 27 adopting rooms (same set as r28–r42):
...
One of this hash sample adopts: **E18S6** (remove `30,8` `31,8` / add `32,6` `32,7`).
...
## Closed since r42 — re-probed

**r29p21 / leftover `baseLap`.** Mutated on rooms that ship it:

| attack | room | result |
|--------|------|--------|
| `misc.mobilityVeto.baseLap := 0` | E11S2 (1.56 vs freeze-cut gated lap 1.56) | **BITES** |
| `misc.mobilityVeto.baseLap += 1` | E11S2 (2.56 vs 1.56) | **BITES** |
| `labs.lapVeto.baseLap := 0` | E11S2 (1.56 vs 1.56) | **BITES** |
| `labs.lapVeto.baseLap += 1` | E11S2 (2.56 vs 1.56) | **BITES** |

Fleet: `baseLap := 0` **fail 57**, pass 115 (the 115 rooms already at 0 were not mutated). `baseLap += 1` **fail 172/172**. The named flatten is done.

`baseLap` is no longer a comment. It is the gated lap on the freeze cut with the extension mass, nuker and observer lifted — the same walk `wasLap` was bound to last round. `+= 1` fails. Those two copies are walks.

Residues of the named close, not the named flatten:

| attack | result |
|--------|--------|
| `towers.mobilityVeto.baseLap := 0` (E11S2 1.56 / E11S7 7.33) | **BITES** (declaration twin of `shortfalls.repair.tower.baseLap`) |
| `towers.baseLap :=` freeze-cut 7.67 on E11S7 | **BITES** (twin broken; repair still 7.33) |
...
`towers.baseLap` is not the freeze-cut walk. E11S7 ships **7.33** against veto/labs **7.67**. Flattening the towers copy fails because the mobility paragraph reprints `repair.tower.baseLap`. It is a battery twin, not the p21 walk.
...
Fleet: `baseOverGated := 0` **fail 57**, pass 115. `wasLap := 0` **fail 4**, pass 168.
...
**Shallow extensions** re-derived against live depth < 4: **0 bare** in all 8. E11S1's `shallowExt` note is a search note (0 live shallow).
...
**Lab diamond:** 10 labs, 4×4 bbox, 6 holes in all 8. Mandated stamp. Haul cheb 2–5. E11S1 haul 5, E18S6 / E2S7 / E1S4 haul 3, E17S9 / E5S5 / E1S7 / E4S4 haul 2. E5S5 variant `anti`.
...
**E18S6.** 74-cut, 84 ramparts, 62 roads. `baseCut=71 ≠ 74` (pricey). `protectRadius=8`. Mobility 0. Controller unenclosed. Adopts: remove `30,8` `31,8` / add `32,6` `32,7`. Mineral `45,36` off-network, declared (seat `44,35`). Seed `26,17` ≠ hub `27,18`. `seedScore=-8.4`. Labs haul 3. 0 2×2. 0 D4-blind. Film: 74 crossing / 2 cover / 1 seat.inside / 5 ring / 2 seat.outside. Unreachable battlement + unmanned link on `32,7`, both declared. Hash #1 is a 134(b) room, a 74-tile wall the room prices as 71, and a seed that is not the hub.

**E17S9.** 36-cut, 40 ramparts, 83 roads. `baseCut=32 ≠ 36`. `protectRadius=8`. As-built gated lap **1.54**, declared. Controller enclosed. Mineral `28,39` **on** the network (seat `29,38`). Seed=hub `17,27`. `seedScore=-21`. Labs haul 2. 0 2×2. 0 D4-blind. Film: 36 crossing / 2 seat.outside / 1 cover / 1 seat.inside. Escalation last fat `2/47/44` against shipped `1.54/40/36`. The mineral being on-network is the one thing this sample does not have to excuse. The pick is 32 against a freeze and a shipped cut of 36.

**E5S5.** 27-cut, 34 ramparts, 80 roads. `baseCut=27` = freeze = cut. `protectRadius=10`. Mobility 0. Controller unenclosed. Mineral `18,19` **on** the network (seat `17,20`). Seed `18,30` ≠ hub `18,28`. `seedScore=-44.8`. Taken recovery, 0 holders / 11-tile pocket. Labs variant `anti`, haul 2. 0 2×2. 0 D4-blind. Film: 27 crossing / 1 seat.inside / 2 seat.outside / 2 cover / 2 ring. Quiet pocket, named on-network, unread seed.

**E1S7.** 55-cut, 61 ramparts, 72 roads. `baseCut=55` = freeze = cut (pricey). `protectRadius=8`. Mobility 0. Controller unenclosed. Mineral `18,21` off-network, declared (seat `17,20`). Seed=hub `26,12`. `seedScore=-25.9`. **Tower clump 5**, declared. Labs haul 2. 0 2×2. 0 D4-blind. Film: 55 crossing / 1 seat.inside / 2 seat.outside / 2 cover / 1 ring. Hash #4 is a 55-tile pricey wall and a five-tower huddle the room does name.

**E4S4.** 58-cut, 63 ramparts, 75 roads. `baseCut=55 ≠ 58` (pricey). `protectRadius=8`. As-built gated lap **1.64**, declared. Controller enclosed. Mineral `23,24` **on** the network (seat `23,25`). Seed=hub `22,34`. `seedScore=-26.4`. Labs haul 2. 0 2×2. 0 D4-blind. Film: 58 crossing / 3 cover / 2 seat.outside. Escalation last fat `3/73/67` against shipped `1.64/63/58`. Hash #5 is a 1.64 lap, an unread last fat, and a 55-pick against a 58-cut.

### Mandated churn

**E11S1.** 32-cut, 43 ramparts, 101 roads. Real shrink: `fullRun.reserved` === `lane.reserved` === 6 tiles / 2 rounds. `wanted=7`, `used=3`, `ext=60`, `shallow=3`. Suffix COORD list is gone. Seed `25,39` ≠ hub `24,41`. `seedScore=-92.9`. Controller unenclosed. Mineral `23,7` off-network, declared (seat `24,8`). Eco 27-tile controller walk, declared. 3 2×2 on a road spine. 0 D4-blind. Labs haul 5. Film: 32 crossing / 4 seat.outside / 3 cover / 4 ring. Reserved board: `18,27 19,28 19,29 | 24,28 25,27 25,29`. `19,27` is D8 of `18,27` and `19,28` and is not in the set. The 98 residue room, mandated. `baseLap=0` on this room — the p21 witness that *has* a nonzero walk is E11S2 (1.56).
...
No sampled room is a checkerboard, a solid brick, or a maze. Extension mass is corridor-flanked (**0 D4-blind extensions in all 8**). 2×2 squares exist only in E11S1 (3) and they sit on a road spine. E18S6 / E11S1 / E1S4 spend roads on eco, not on a city grid. Towers clump 5 on E1S7 (declared); the rest of the sample clumps 2–3.

Nothing in the sample looks accidental. The ugly rooms are ugly for a named, declared reason (E18S6 74-tile wall + unmanned link, E17S9 1.54 lap, E1S7 five-tower huddle + 55-tile wall, E4S4 1.64 lap, E11S1 27-tile controller walk) — except the unread pick on E18S6 (`baseCut=71` against a 74-cut) / E17S9 (`baseCut=32` against a 36-cut) / E4S4 (`baseCut=55` against a 58-cut) / E1S4 (`baseCut=24` against a 26-cut) and the unread last fats on E17S9 (`2/47/44`) and E4S4 (`3/73/67`), which the rooms do not price as compositions.
...
**Exploit.** E11S1 after r29p10/p11/p12/p13/p14/p15/p16/p17/p18/p19/p20/p21: `fullRun.reserved` === `lane.reserved` === 6 tiles / 2 rounds. `wanted=7`. The refused tail is a count.
...
r29p21 did not touch this. A tile that *is* this room's floor and *touches* the walk can still be added, or swapped for a kept prefix tile, if both lists stay twins. The refused walk is now four unread integers (`wanted`, `used`, `ext`, `shallow`) plus `premium`.
...
`discardedCutSeals` no longer trusts `row.complete`. A leak bites either way. That is the named close. A 42-tile nudged cut that *does* seal is longer than the shipped 37, so the would-have-taken rule returns before it looks. Any other 42-tile COORD list that walks the claimed lap and seals passes. Flipping `complete` on a sealing rung is free. This sample has two more last fats of the same class (E17S9 `2/47/44`, E4S4 `3/73/67`).

**Root cause.** Discarded `cutTiles` are a free list held to `enclosureMobility` of themselves, plus "not freeze/shipped", plus "not shorter-and-prettier", plus "seals". The composed enclosure is not on the board. r29p11 gated leaks on the flood, not the flag. It did not bind the list. r29p21 did not touch this.
...
**Exploit.** Zeroed every remaining `klass:"presence"` numeric/bool/array leaf across all 172 rooms. 34 presence names, 66 derived (r42 was 34 / 65; p21 added `baseLap` as derived). 15 kinds, 1,189 events. `checkRoom`: **fail 172/172**, every first fail is `baseCut := 0`.

Exclude `baseCut` and `shallowNow`: **pass 172/172**. 13 kinds, 1,014 events, silent. Same 172/172 if those two plus `hubDistCap` are skipped. The r41 "minus those two is silent" door is still open — p21 reclassified a leaf that was not even in META_DARK last round. It did not derive the class.
...
r29p3 / r29p9 / r29p12 / r29p13 / r29p14 / r29p15 / r29p16 / r29p17 / r29p18 / r29p19 / r29p20 / r29p21 derived names still BITES. The class is still the class. `protectRadius` is *classified* `derived` and is only an enum. `hubDistCap` is *classified* `derived` and is only an enum. `baseLap` is *classified* `derived` and is now the walk — on two of four copies.

**Root cause.** Criticism 141(c). They derived the one cheap leaf this review named, on the two paths the last mech hunt listed. The rest are still comments.

**Board moved?** No.

### M5 — MEDIUM, not board-affecting

**Exact `baseCut` / `protectRadius` / `hubDistCap` pick is unread. `seedScore` is not. `stubCap` is not. `maxHubDist` is not. `baseOverGated` is not. `wasLap` is not. `baseLap` (the two named copies) is not.**

`protectRadius := 0` **BITES** (not in `RADII_WIDE`). `12 → 6` inside the enum **ESCAPE**. `baseCut := 0` **BITES**. `baseCut += 1` keeping `priceyWall` on the same side of `MAX_CUT=45` **ESCAPE**. `hubDistCap := 17` **BITES** (not in `[16,19,23,999]`). `16 → 19` inside the enum **ESCAPE**. `seedScore := 0` **BITES**. `seedScore += 999` **BITES**. `stubCap` 43→51 **BITES** (the pool walk). `maxHubDist := 0` / `+= 1` **BITES** (the pre-7b walk). `baseOverGated := 0` / `+= 1` **BITES** (the freeze-cut walk). `wasLap := 0` / `+= 1` **BITES** (that walk's gated lap). `baseLap := 0` / `+= 1` **BITES** (that walk, on veto + labs).

Sample: E18S6 `baseCut=71` / cut=74 (pricey). E17S9 `baseCut=32` / cut=36. E5S5 `baseCut=27` = cut. E1S7 `baseCut=55` = cut (pricey). E4S4 `baseCut=55` / cut=58 (pricey). E1S4 `baseCut=24` / cut=26. The pick is not freeze and not the shipped cut. `protectRadius` is an enum, not this room's pick. `hubDistCap` is an enum, not this room's cohesion rung. `seedScore` is the window max. `baseLap` is the walk — on two copies. `towers.baseLap` is the battery twin. 46 rooms still have seed ≠ hub. The coordinates exist. The score is the walk. The cap is four legal integers.
...
- **p21 baseLap** is that walk on `misc.mobilityVeto` and `labs.lapVeto`. `towers.mobilityVeto.baseLap` is the battery twin of `shortfalls.repair.tower.baseLap` (E11S7 7.33 ≠ 7.67). Flattening the towers copy fails the paragraph. It is not the freeze-cut walk.
...
- Not an anti-pattern auto-fail. No maze, no brick, no road-on-every-rampart, no silent cap in the sample. E1S7's five-tower huddle is declared.
...
- Not the r42 leftover on `baseLap` as a comment. Zeroing it, or adding 1, now fails — on the two copies they named.
...
- `towers.baseLap` as the freeze-cut walk (it is the battery twin)
