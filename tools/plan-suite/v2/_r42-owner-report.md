...
Artifact md5 `8e5d6725885bd3f3731379bedf408326` — matches the brief. Same bytes as r32–r41. Tree HEAD is `9806dd8` (r29p20: `baseOverGated` freeze-cut walk with the extension mass / nuker / observer lifted / `wasLap` that walk's gated lap), parent `c004ef4` (r29p19 takeTowerSwap.from the published take / unreachedClusters / unreachableExts / servedExts). 172/172 rooms, 0 errors. Fleet physicals re-summed off the structure lists: **10,320 extensions · 14,100 roads · 8,208 ramparts · 300 declarations · 236 notes**. Same numbers as r26–r41. Boards did not move. Baseline `checkRoom` (fleet-median stripped): **pass 172/172**.

**Verdict.** No board defect in the sample. No film-vs-board caption lie in the sample. 134(b) is still 34 adds / 27 rooms. r29p20 closes the *stated* leftover flattens: `baseOverGated := 0` **BITES** (fleet 57/57), `baseOverGated += 1` **BITES**, `wasLap := 0` **BITES** (fleet 4/4), `wasLap += 1` **BITES**. `seedScore` still **BITES**. The p19 four + p18 two + p17 five + p16 four + p15 ten + p14 nine + p12 five still **BITES**. The named 88 leak + `complete=false` still **BITES**. The named 98 one-list append still **BITES**. They do **not** close the classes. Twin of the named 98 append (also write `lane.reserved`, keep `wanted > tiles`) **ESCAPE**. `wanted += 1` **ESCAPE**. Sealing same-lap nudge `29,33→28,34` **ESCAPE**. `hubDistCap` 16→19 in-enum **ESCAPE** (fleet 170/170). Remaining META_DARK presence still flips 172/172 silent once `baseCut`+`shallowNow` are left alone. Exact pick still unread. `protectRadius` is still an enum. `hubDistCap` is still an enum. `baseOverGated` is now the freeze-cut over-target walk. `wasLap` is that walk's gated lap. `cutPasses.rampartsDeleted` split still a log.

Throwaway probes: `tools/plan-suite/v2/_r42-owner/` (not committed).

---

## Sample

`h(room) = fmix32(fnv1a32("round42-owner|" + room))` over the 172 names. Lowest five (not cherry-picked):
...
The 27 adopting rooms (same set as r28–r41):
...
Zero of this hash sample adopt.
...
## Closed since r41 — re-probed

**r29p20 / two leftover presence names.** Mutated on rooms that ship them:

| attack | room | result |
|--------|------|--------|
| `baseOverGated := 0` | E11S2 (4 vs freeze-cut walk 4) | **BITES** |
| `baseOverGated += 1` | E11S2 (5 vs 4) | **BITES** |
| `wasLap := 0` | E11S6 (1.5 vs freeze-cut gated lap 1.5) | **BITES** |
| `wasLap += 1` | E11S6 (2.5 vs 1.5) | **BITES** |

Fleet: `baseOverGated := 0` **fail 57**, pass 115 (the 115 rooms already at 0 were not mutated). `wasLap := 0` **fail 4**, pass 168 (the 168 rooms with no refused `wasLap` were not mutated). The named flatten is done.

`baseOverGated` is no longer a comment. It is the gated over-target pair count on the freeze cut with the extension mass, nuker and observer lifted. `+= 1` fails. `wasLap` is that walk's gated lap on each refused row. Flattening it fails. Those two are walks.
...
Fleet: `takeTowerSwap.from` other D8 **fail 3**, pass 169. `unreachedClusters += 1` / `unreachableExts += 1` / `servedExts += 1` still fail on the rooms that ship them.
...
| `servedExts` + `fillerTiles` + `extFace` += 1 | **BITES** (laid book) |
...
**r29p12 / five leftover presence names.** Still BITES. Hash #4 is the `roadsEaten` witness (E9S2, 3).
...
**Shallow extensions** re-derived against live depth < 4: **0 bare** in all 8. E9S2 ships **15** shallow extensions, all with personal ramparts, published 15, declared. E11S1's `shallowExt` note is a search note (0 live shallow).
...
**Lab diamond:** 10 labs, 4×4 bbox, 6 holes in all 8. Mandated stamp. Haul cheb 1–5. E3S8 haul 1, E14S3 / E2S4 / E18S2 haul 2, E2S7 / E1S4 haul 3, E9S2 haul 4 (variant `anti`), E11S1 haul 5.
...
**E14S3.** 70-cut, 80 ramparts, 70 roads. `baseCut=70` = freeze = cut (pricey). `protectRadius=9`. Mobility 0. Controller unenclosed. Mineral `27,35` **on** the network (seat `26,34`). Seed `18,31` ≠ hub `20,31`. `seedScore=-26.4`. 1 redundant cut tile, explained (load-bearing on interior floor). Labs haul 2. 0 2×2. 0 D4-blind. Film: 70 crossing / 1 seat.outside / 3 cover / 2 seat.inside / 4 ring. Hash #1 is a 70-tile pricey wall the room does not price, and a seed that is not the hub. The mineral being on-network is the one thing this sample does not have to excuse.

**E3S8.** 40-cut, 45 ramparts, 70 roads. `baseCut=40` = freeze = cut. `protectRadius=8`. Mobility 0. Controller enclosed. Mineral `44,13` off-network, declared (seat `45,14`). Seed=hub `30,27`. `seedScore=-17.2`. Labs haul 1. Tower clump 4. 1 2×2. 0 D4-blind. Film: 40 crossing / 2 cover / 1 seat.inside / 2 seat.outside. Quiet pocket, named off-network, no discarded fat.

**E2S4.** 37-cut, 42 ramparts, 81 roads. `baseCut=37` = freeze = cut. `protectRadius=9`. Mobility 0. Controller enclosed. Mineral `8,17` off-network, declared (seat `9,16`). Seed `22,17` ≠ hub `22,18`. `seedScore=-20.9`. 2 redundant cut tiles, both explained (depth promotion). Labs haul 2. 0 2×2. 0 D4-blind. Film: 37 crossing / 2 seat.outside / 2 cover / 1 seat.inside. The seed is not the hub. The pick equals the cut.

**E9S2.** 21-cut, 40 ramparts, 68 roads. `baseCut=21` = freeze = cut. `protectRadius=12`. Mobility 0. Controller enclosed. Mineral `44,31` off-network, declared (seat `43,30`). Seed=hub `35,13`. `seedScore=-40.3`. **15** shallow extensions, all ramped, published 15, declared. 7 redundant cut tiles, all explained (2 load-bearing, 5 depth promotion). Labs variant `anti`, haul 4, **lab-road-eat 3** (the p12 `roadsEaten` witness). 2 2×2. Tower clump 4. 0 D4-blind. Film: 21 crossing / 17 cover / 2 seat.outside. Hash #4 is a 15-tile shallow tax and a stamp that ate three roads because the enclosure offered no deep anchor.

**E18S2.** 44-cut, 47 ramparts, 65 roads. `baseCut=46 ≠ 44` (pricey). `protectRadius=12`. Mobility 0. Controller enclosed. Mineral `23,45` off-network, declared (seat `24,46`). Seed `10,17` ≠ hub `11,17`. `seedScore=-25.9`. Weak-battery declared. Labs haul 2. 0 2×2. 0 D4-blind. Film: 44 crossing / 2 seat.outside / 1 seat.inside. The pick is 46 against a freeze and a shipped cut of 44. Hash #5 is an unread pick and a named weak battery.

### Mandated churn

**E11S1.** 32-cut, 43 ramparts, 101 roads. Real shrink: `fullRun.reserved` === `lane.reserved` === 6 tiles / 2 rounds. `wanted=7`, `used=3`, `ext=60`, `shallow=3`. Suffix COORD list is gone. Seed `25,39` ≠ hub `24,41`. `seedScore=-92.9`. Controller unenclosed. Mineral `23,7` off-network, declared (seat `24,8`). Eco 27-tile controller walk, declared. 3 2×2 on a road spine. 0 D4-blind. Labs haul 5. Film: 32 crossing / 4 seat.outside / 3 cover / 4 ring. Reserved board: `18,27 19,28 19,29 | 24,28 25,27 25,29`. `19,27` is D8 of `18,27` and `19,28` and is not in the set. The 98 residue room, mandated. `maxHubDist=15`. `baseOverGated=0` on this room — the p20 witness that *has* a nonzero walk is E11S2 (4).
...
No sampled room is a checkerboard, a solid brick, or a maze. Extension mass is corridor-flanked (**0 D4-blind extensions in all 8**). 2×2 squares exist in E3S8 (1), E9S2 (2), and E11S1 (3); E11S1's sit on a road spine. E14S3 / E2S4 / E11S1 spend roads on eco, not on a city grid. Towers clump 4 on E3S8 and E9S2; the rest of the sample clumps 0–3.

Nothing in the sample looks accidental. The ugly rooms are ugly for a named, declared reason (E14S3 70-tile pricey wall, E9S2 15 shallow + lab-road-eat, E18S2 weak battery + 46-pick, E11S1 27-tile controller walk) — except the unread pick on E18S2 (`baseCut=46` against a 44-cut) / E1S4 (`baseCut=24` against a 26-cut), which the rooms do not price as compositions.
...
**Exploit.** E11S1 after r29p10/p11/p12/p13/p14/p15/p16/p17/p18/p19/p20: `fullRun.reserved` === `lane.reserved` === 6 tiles / 2 rounds. `wanted=7`. The refused tail is a count.
...
r29p20 did not touch this. A tile that *is* this room's floor and *touches* the walk can still be added, or swapped for a kept prefix tile, if both lists stay twins. The refused walk is now four unread integers (`wanted`, `used`, `ext`, `shallow`) plus `premium`.
...
`discardedCutSeals` no longer trusts `row.complete`. A leak bites either way. That is the named close. A 42-tile nudged cut that *does* seal is longer than the shipped 37, so the would-have-taken rule returns before it looks. Any other 42-tile COORD list that walks the claimed lap and seals passes. Flipping `complete` on a sealing rung is free. This sample has no extra last-fat of that class (none of the hash five ship discarded fat).

**Root cause.** Discarded `cutTiles` are a free list held to `enclosureMobility` of themselves, plus "not freeze/shipped", plus "not shorter-and-prettier", plus "seals". The composed enclosure is not on the board. r29p11 gated leaks on the flood, not the flag. It did not bind the list. r29p20 did not touch this.
...
**Exploit.** Zeroed every remaining `klass:"presence"` numeric/bool/array leaf across all 172 rooms. 34 presence names, 65 derived (r41 was 36 / 63; p20 moved `baseOverGated` / `wasLap`). 15 kinds, 1,189 events. `checkRoom`: **fail 172/172**, every first fail is `baseCut := 0`.

Exclude `baseCut` and `shallowNow`: **pass 172/172**. 13 kinds, 1,014 events, silent. Same 172/172 if those two plus `hubDistCap` are skipped. The r41 "minus those two is silent" door is still open — p20 reclassified two leaves that were already in the leftover flatten (`baseOverGated` / `wasLap`). They are no longer in the 1,014-event skip set. It did not derive the class.
...
r29p3 / r29p9 / r29p12 / r29p13 / r29p14 / r29p15 / r29p16 / r29p17 / r29p18 / r29p19 / r29p20 derived names still BITES. The class is still the class. `protectRadius` is *classified* `derived` and is only an enum. `hubDistCap` is *classified* `derived` and is only an enum. `baseOverGated` is *classified* `derived` and is now the walk. `wasLap` is *classified* `derived` and is now the walk.

**Root cause.** Criticism 141(c). They derived the two cheap leaves this review named. Both of them are real walks of the freeze-cut / lifted-mass board. The rest are still comments.

**Board moved?** No.

### M5 — MEDIUM, not board-affecting

**Exact `baseCut` / `protectRadius` / `hubDistCap` pick is unread. `seedScore` is not. `stubCap` is not. `maxHubDist` is not. `baseOverGated` is not. `wasLap` is not.**
...
Sample: E14S3 `baseCut=70` = cut (pricey). E3S8 `baseCut=40` = cut. E2S4 `baseCut=37` = cut. E9S2 `baseCut=21` = cut. E18S2 `baseCut=46` / cut=44 (pricey). E1S4 `baseCut=24` / cut=26. The pick is not freeze and not the shipped cut. `protectRadius` is an enum, not this room's pick. `hubDistCap` is an enum, not this room's cohesion rung. `seedScore` is the window max. `baseOverGated` is the walk. `wasLap` is the walk. 46 rooms still have seed ≠ hub. The coordinates exist. The score is the walk. The cap is four legal integers.
...
- **p20 wasLap** is that same walk's gated lap, not the refused row's own before-lap of a different board.
...
- Not an anti-pattern auto-fail. No maze, no brick, no road-on-every-rampart, no silent cap in the sample. E9S2's 15 shallow and 3 eaten roads are declared.
