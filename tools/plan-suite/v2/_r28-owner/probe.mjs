/**
 * Round-28 owner-voice probe. Independent of validate.mjs for board facts.
 * Terrain from ROOMS_FILE. Mutates clones in memory only. Never writes the artifact.
 */
import fs from "fs";
...
function roomHash(room) {
  return fmix32(fnv1a32("round28-owner|" + room));
}
...
/** Independent D8 exterior flood from the four edges. */
...
function ringCensus(plan, seat, net) {
  const holds = structMap(plan);
  const ring = [];
  const touching = [];
  for (const [dx, dy] of D8) {
    const x = seat.x + dx,
    ...
    const k = K(x, y);
    const h = holds.get(k);
    ring.push({ k, holds: h ? h.join("+") : "nothing of ours" });
  ...
  return { ring, touching, nearestRoad };
}
...
const SEAT_OUT =
  "seat outside the shell — a container beyond the wall, covered where it stands";
...
function fleetCutDrift() {
  ...
  for (const p of plans) {
    ...
    for (const mk of passes) for (const k of Object.keys(mk || {})) passLeafKeys.add(k);
  }
  return {
    adds,
    removes,
...
function rederiveAdds() {
  ...
  for (const p of plans) {
    ...
    if (!d) {
      bad.push({ room: p.room, why: "no terrain" });
      continue;
    }
    const ramp = new Set((p.structures?.rampart || []).map(KT));
    const sitter = p.sitter || p.hub;
    const adds = (p.meta?.shell?.cutDrift || []).filter((e) => e.op === "add");
    for (const e of adds) {
      const k = KT(e);
      const fire = sealTest(d.terrain, ramp, sitter, k);
      if (!fire) {
        roomsFailSeal.push({ room: p.room, tile: k, pass: e.pass });
      } else ok++;
    }
  }
  return { ok, fail: roomsFailSeal };
}
...
function mineralWhyRooms(names) {
  ...
  for (const name of names) {
    ...
    const pubSeat = p.meta?.mineralSeat;
    const why = p.meta?.misc?.mineralOffNetworkWhy || "";
    const off = p.meta?.misc?.mineralOffNetwork;
    const net = seat ? netTiles(p, seat) : new Set();
    const c = seat ? ringCensus(p, seat, net) : null;
    const roadOnSeat = seat && (p.structures?.road || []).some((r) => r.x === seat.x && r.y === seat.y);
    const roadOnRing = c
      ? c.ring.filter((r) => (p.structures?.road || []).some((rd) => KT(rd) === r.k)).map((r) => r.k)
      : [];
    // parse published ring
    ...
    const pubRing = ringHit
      ...
      : [];
    const pubTouch = +((ringHit && ringHit[2]) || -1);
    const seatHit = /mineral seat at (\d+),(\d+)/.exec(why);
    const pubSeatInWhy = seatHit ? { x: +seatHit[1], y: +seatHit[2] } : null;
    const derivedRing = c ? c.ring.map((r) => ({ k: r.k, holds: r.holds })) : [];
    const ringDisagree = [];
    if (c && pubRing.length) {
      const a = new Map(pubRing.filter((r) => r.k).map((r) => [r.k, r.holds]));
      const b = new Map(derivedRing.map((r) => [r.k, r.holds]));
    ...
    out.push({
      ...
      seat,
      pubSeat,
      seatMatchesPub: seat && pubSeat && seat.x === pubSeat.x && seat.y === pubSeat.y,
      off,
      why,
      pubSeatInWhy,
      pubTouch,
...
function labDiamond(plan) {
  ...
  // classic diamond: 4x4 with 4 corners empty, 2 inputs in the middle pair
  const box16 = w === 4 && h === 4;
  let holes = 0;
  if (box16) {
...
function hubTrio(plan) {
  ...
  return {
    ...
    hubLink,
    ok,
    spawnCheb: (plan.structures?.spawn || []).map((s) => cheb(s, sitter)).sort((a, b) => a - b),
    spawnPairs: pairwiseMin(plan.structures?.spawn || []),
  };
}

function pairwiseMin(pts) {
  let m = 99;
  for (let i = 0; i < pts.length; i++)
    for (let j = i + 1; j < pts.length; j++) m = Math.min(m, cheb(pts[i], pts[j]));
  return m === 99 ? null : m;
}

function visual(plan, terrain) {
  ...
  const noD4Tiles = [];
  for (const e of extn) {
    const faces = D4.some(([dx, dy]) => roads.has(K(e.x + dx, e.y + dy)));
    if (!faces) {
      noD4++;
      noD4Tiles.push(KT(e));
    }
  ...
  for (const e of extn) {
    ...
    for (let dy = 0; dy < 3 && ok; dy++)
      for (let dx = 0; dx < 3 && ok; dx++) {
        const h = occ.get(K(e.x + dx, e.y + dy)) || [];
        if (!h.includes("extension")) ok = false;
      }
    if (ok) bricks3++;
  }
  // road degree (D8, since creeps step D8)
  const roadDeg4 = new Map();
  const roadDeg8 = new Map();
  for (const r of plan.structures?.road || []) {
    let d4 = 0,
      d8 = 0;
    for (const [dx, dy] of D4) if (roads.has(K(r.x + dx, r.y + dy))) d4++;
    for (const [dx, dy] of D8) if (roads.has(K(r.x + dx, r.y + dy))) d8++;
    roadDeg4.set(KT(r), d4);
    roadDeg8.set(KT(r), d8);
  }
  const dead4 = [...roadDeg4].filter(([, d]) => d <= 1).length;
  const dead8 = [...roadDeg8].filter(([, d]) => d <= 1).length;
  // roads serving nothing: no structure (except road/rampart) in D8
  let orphanish = 0;
  for (const r of plan.structures?.road || []) {
    let serve = occ.get(KT(r))?.some((t) => t !== "road" && t !== "rampart");
    if (!serve) {
      for (const [dx, dy] of D8) {
        const h = occ.get(K(r.x + dx, r.y + dy)) || [];
        if (h.some((t) => t !== "road" && t !== "rampart")) {
          serve = true;
          break;
        }
      }
...
function enclosure(plan, terrain) {
  ...
  const pubShallow = plan.meta?.extensions?.shallow;
  return {
    cutN: cut.length,
...
function filmVsBoard(plan, terrain) {
  ...
  for (const st of film.steps || []) {
    ...
    for (let i = 0; i < cells.length; i += 3) {
      painted.push({ x: cells[i], y: cells[i + 1], caption: cap, raw: st.label });
    }
  }
  const disagrees = [];
  for (const t of painted) {
    ...
    if (t.caption !== want.caption) {
      disagrees.push({ k, film: t.caption, board: want.caption, facet: want.facet });
    }
  }
  const ramp = new Set((plan.structures?.rampart || []).map(KT));
...
function inspectRoom(name) {
  ...
  return {
    ...
    mineral: {
      ...
      why: (p.meta?.misc?.mineralOffNetworkWhy || "").slice(0, 500),
    ...
    page: {
      ...
      noteSnips: page.notes && !page.notes.parseFail
        ? Object.fromEntries(Object.entries(page.notes).map(([k, v]) => [k, String(v).slice(0, 220)]))
        : null,
      sf: page.sf,
      nt: page.nt,
    },
...
const mandated = ["E12S1", "E15S4", "E11S1", "E12S7", "E12S6", "E7S5", "E9S2"];
...
// fleet headline sanity (independent)
const fleet = {
  rooms: plans.length,
...
const out = {
  md5,
  expectMd5: "239f4e43331181cf4484d462003ff6b5",
...
console.log(
  JSON.stringify(
    {
      md5,
      md5Ok: md5 === "239f4e43331181cf4484d462003ff6b5",
      rooms: plans.length,
