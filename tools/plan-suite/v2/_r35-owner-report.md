...
Artifact md5 `8e5d6725885bd3f3731379bedf408326` — matches the brief. Same bytes as r32–r34. Tree HEAD is `954f00b` (r29p13), matching the brief. 172/172 rooms, 0 errors. Fleet physicals re-summed off the structure lists: **10,320 extensions · 14,100 roads · 8,208 ramparts · 300 declarations · 236 notes**. Same numbers as r26–r34. Boards did not move. Baseline `checkRoom` (fleet-median stripped): **pass 172/172**.

**Verdict.** No board defect in the sample. No film-vs-board caption lie in the sample. 134(b) is still 34 adds / 27 rooms. r29p13 closes the *stated* leftover presence walks: `mineralContainer` / `minDmgPicked` / `servedFree` zeroed now **BITES** (fleet 172/172). `stitched := 2` now **BITES**, including E8S4 where `laid.stitch=2` and the shipped flag is 1 — the flag is exact 0/1. The p12 five still **BITES**. The named 88 leak + `complete=false` still **BITES**. The named 98 one-list append still **BITES**. They do **not** close the classes. Twin of the named 98 append (also write `lane.reserved`, keep `wanted > tiles`) **ESCAPE**. `wanted += 1` **ESCAPE**. Sealing same-lap nudge `29,33→28,34` **ESCAPE**. Remaining META_DARK presence still flips 172/172 once `baseCut`+`shallowNow` are left alone. Exact pick and `seedScore` still unread. `cutPasses.rampartsDeleted` split still a log.

Throwaway probes: `tools/plan-suite/v2/_r35-owner/` (not committed).

---

## Sample

`h(room) = fmix32(fnv1a32("round35-owner|" + room))` over the 172 names. Lowest five (not cherry-picked):
...
The 27 adopting rooms (same set as r28–r34):
...
None of the eight sampled rooms adopt.
...
## Closed since r34 — re-probed

**r29p13 / mineralContainer · minDmgPicked · servedFree · stitched flag.** Zeroed / set-to-2 on rooms that ship them:
...
Fleet flatten of the three: **fail 172/172** (`mineralContainer` 172, `minDmgPicked` 172, `servedFree` 171). Fleet `stitched := 2`: **fail 172/172**. The named closes are done. `stitched` is the 0/1 flag they named, not `laid.stitch` (E8S4 ships 1 against laid 2 and that *is* the contract — setting it to the laid count now fails). `mineralContainer` is the cheb-1 seat count (172/172 ship 1). `minDmgPicked` is towers[0]+towers[1] on the freeze wall. `servedFree` is shipped-cut clusters that already D8-touch a pre-layer-7 road.
...
**93 recovers.** Taken-room `fixedHolders` ship `{type, x, y}` only. Rooms that publish holders, **0** publish `recovers` / `recoversDeep`. Invent holder `{lab,1,1,99/99}` **BITES**. Plant `recovers=2` / `recoversDeep=2` on E15S6's four labs + note twin **BITES**. E7S5 (hash #4) is taken: three lab holders, keys `{type,x,y}` only.
...
**Lab diamond:** 10 labs, 4×4 bbox, 6 holes in all 8. Mandated stamp. Haul cheb 2–5. E7S1 / E11S1 haul 5, E12S6 / E7S5 haul 4, E2S7 / E1S4 haul 3, E18S2 / E16S5 haul 2.
...
**E7S1.** 42-cut, 45 ramparts, 83 roads, 60/60. `baseCut=42` = freeze = cut. `protectRadius=9`. Seed=hub `14,39`. Mobility **3.5** declared (page and film agree). Controller enclosed. Mineral `17,26` off-network, declared (seat `18,27`). Labs variant `anti`, haul 5. 0 2×2. 0 D4-blind. 0 redundant. Film: 42 crossing / 2 seat.outside / 1 cover. Fat last rungs `3/49/47` against shipped `3.5/45/42`. Ugly for a named reason: the 3.5 lap. The last discarded enclosure is a free 47-tile neighbour of the 42.

**E12S6.** 33-cut, 48 ramparts, 124 roads. `baseCut=33` = freeze = cut. `protectRadius=12`. Mobility 0. Controller unenclosed. Mineral `19,9` **on** the network (seat `18,8`). Seed `28,10` ≠ hub `29,10`. **6 shallow** re-derived exact, all ramparted, declared. Weak-battery 9-step declared. Eco 36-tile controller walk, declared. **3 2×2** on a road spine. 0 D4-blind. 0 redundant. Labs haul 4. Film: 33 crossing / 8 cover / 2 seat.outside / 1 seat.inside / 4 ring. Roads and shallow are the basin tax, and the room says so.

**E18S2.** 44-cut, 47 ramparts, 65 roads. `baseCut=46 ≠ 44`. `protectRadius=12`. Mobility 0. Controller enclosed. Mineral `23,45` far south off-network, declared (seat `24,46`). Seed `10,17` ≠ hub `11,17`. 0 2×2. 0 D4-blind. 0 redundant. Labs haul 2. Unmanned cut tile `2,24` (link on the wall) + unreachable battlement, both declared. Weak-battery 1710 / 10-step refill, declared. Film: 44 crossing / 2 seat.outside / 1 seat.inside. The pick is 46 against a shipped 44. Quiet board, unread pick.

**E7S5.** 14-cut, 19 ramparts, 99 roads. Taken (3 lab holders, `{type,x,y}` only). `baseCut=14` = freeze = cut. `protectRadius=8`. Seed=hub `18,17`. Controller enclosed. Mineral `25,30` **on** the network (seat `24,31`). Covered-detour declared (35 inside vs 2 outside). Eco source-path tax 54 (gate 52), declared. 0 2×2. 0 D4-blind. 0 redundant. Labs haul 4. Film: 14 crossing / 3 seat.outside / 2 cover. Small shell, long eco arms, named.

**E16S5.** 52-cut, 60 ramparts, 87 roads. `baseCut=52` = freeze = cut. `protectRadius=8`. Mobility 0. Controller unenclosed. Mineral `43,20` off-network east, declared (seat `42,19`). Seed=hub `30,35`. **2 redundant** cut tiles re-derived: `38,44 39,44` — the note's 2 (depth promotion of `36,41`). Eco 21-tile controller walk, declared. **4 2×2** on a road spine. 0 D4-blind. Labs variant `anti`, haul 2. Film: 52 crossing / 3 cover / 3 seat.outside / 1 seat.inside / 1 ring. The 52-tile wall is the ugliness the room does not price beyond eco and the two redundant tiles.
...
No sampled room is a checkerboard, a solid brick, or a maze. Extension mass is corridor-flanked (**0 D4-blind extensions in all 8**). 2×2 squares exist in E16S5 (4), E12S6 (3) and E11S1 (3) and they sit on a road spine. E7S1 spends a named 3.5 lap. E12S6 / E7S5 / E11S1 / E16S5 spend roads on eco, not on a city grid. Towers are not a hub clump (sample clumps 0–3; E18S2's six sit off the sitter).

Nothing in the sample looks accidental. The ugly rooms are ugly for a named, declared reason (E7S1 lap 3.5, E12S6 6 shallow + 124-road eco, E7S5 covered-detour + 99-road eco, E18S2 unmanned link on the cut, E11S1 27-tile controller walk) — except the unread pick on E18S2 (`baseCut=46` against a 44-cut) and the unread last fats, which the rooms do not price as compositions.
...
**Exploit.** E11S1 after r29p10/p11/p12/p13: `fullRun.reserved` === `lane.reserved` === 6 tiles / 2 rounds. `wanted=7`. The refused tail is a count.
...
r29p13 did not touch this. A tile that *is* this room's floor and *touches* the walk can still be added, or swapped for a kept prefix tile, if both lists stay twins. The refused walk is now four unread integers (`wanted`, `used`, `ext`, `shallow`) plus `premium`.
...
`discardedCutSeals` no longer trusts `row.complete`. A leak bites either way. That is the named close. A 42-tile nudged cut that *does* seal is longer than the shipped 37, so the would-have-taken rule returns before it looks. Any other 42-tile COORD list that walks the claimed lap and seals passes. Flipping `complete` on a sealing rung is free. This sample has one more last fat of the same class (E7S1 `3/49/47`).

**Root cause.** Discarded `cutTiles` are a free list held to `enclosureMobility` of themselves, plus "not freeze/shipped", plus "not shorter-and-prettier", plus "seals". The composed enclosure is not on the board. r29p11 gated leaks on the flood, not the flag. It did not bind the list. r29p13 did not touch this.
...
**Exploit.** Zeroed every remaining `klass:"presence"` numeric/bool/array leaf across all 172 rooms. 68 presence names, 31 derived (r34 was 71 / 28). 37 kinds, 3,621 events. `checkRoom`: **fail 172/172**, every first fail is `baseCut := 0`. Exclude `baseCut` and `shallowNow`: **pass 172/172**. 35 kinds, 3,446 events, silent.

Single-room ESCAPE, flattering direction: `corridorPlaced`, `deepBudget`, `boundHeld` decrement-keep-nonzero / cleared. `spurred` decrement 5→4 still ESCAPE — and `spurred` is classified `derived`.

r29p3 / r29p9 / r29p12 / r29p13 derived names still BITES. The class is still the class. `protectRadius` is *classified* `derived` and is only an enum.
...
Sample: E7S1 `baseCut=42` = cut. E12S6 `baseCut=33` = cut. E18S2 `baseCut=46` / freeze=44 / cut=44. E7S5 `baseCut=14` = cut. E16S5 `baseCut=52` = cut. E1S4 `baseCut=24` / cut=26. The pick is not freeze and not the shipped cut. `protectRadius` is an enum, not this room's pick. `seedScore` is a finite number, not the confluence walk. 46 rooms still have seed ≠ hub. The coordinates exist. The score does not.
...
- Remaining META_DARK presence (68 names, 172/172 silent once `baseCut` is left alone)
