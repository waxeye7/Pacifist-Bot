# Round 33 mechanical review

Artifact `tools/plan-suite/out-v2/plans-hub.json` md5 `8e5d6725885bd3f3731379bedf408326` (27,780,512 bytes) — **matches expected**. Brief HEAD `1d7ca82` (r29p11). Tree is `6dd763c` (spawn interleave after r29p11); planner sources and this artifact are unchanged since `1d7ca82`. Rooms dump `tools/plan-suite/v2/_r28-mech/rooms.json` (172/172, 0 missing). Node `v22.13.0` (`fnm` install on PATH). Probes in `tools/plan-suite/v2/_r33-mech/`. No suite source committed. Artifact not written. **No board moved.** No `boards` field. Physicals still 10,320 / 14,100 / 8,208.
...
A gate that agrees with itself is not a gate. r29p11 closed the *named* 88 leak + `complete=false` skip. r29p10's one-copy `"19,27"` still BITES. The kept prefix is still a D8-neighbor bag. `wanted` is still a count. The last discarded rung is still a free enclosure that walks its own lap. The presence class still flips.

---

## BLOCKING

None on the board. r28 mineral-why / swap-offer doors stay closed. 93 plant-back stays closed. 88 leaky + `complete=false` is now closed.
...
**Exploit.** 76 presence names, 23 derived. Zeroed every remaining `klass:"presence"` numeric/bool/nonempty-array leaf across all 172 rooms. 44 kinds, 4,319 events. `checkRoom`: **fail 172/172**, every first fail is `baseCut := 0` (three rooms also `shallowNow` note-identity).

Exclude `baseCut` only: **pass 169/172**. The three fails are `shallowNow` note identity (E12S6, E2S3, E9S2). Exclude `baseCut`+`shallowNow`: **pass 172/172**.
...
Sample: E19S7 `baseCut=18` / cut=21. E15S4 `baseCut=30` / cut=29. E8S9 `baseCut=68` = cut (pricey). The pick is not freeze and not the shipped cut.
...
Independent single-removal over all **7,246** `cutAtFreeze` tiles: **0 loose**, 0 core-seal fails, 29 rooms differ from the shipped cut, 34 add + 46 remove, 27 adopt / 29 drift. Same census as r29–r32.
...
## Attacked and held (r32 closures + r29p11 named, re-probed)
...
`h(room) = fmix32(fnv1a32("round33-mech|" + room))` over the 172 names. Lowest five (not cherry-picked):
...
|---|---|---|---|---|
| E12S5 | 60/76/35/32/32 | 0 | 28,41 off | **dropped** (lane 0 / fullRun 7), seed `11,19` ≠ hub `9,21` |
| E19S7 | 60/74/24/21/21 | 0 | 27,32 **on** | `baseCut=18≠21`, seed `32,38` ≠ hub `33,38` |
| E8S9 | 60/71/74/68/68 | 0 | 7,20 off | **pricey** 68, seed=hub |
| E15S4 | 60/74/36/29/29 | 0 | 23,6 off | **adopter**, `baseCut=30≠29`, seed `25,23` ≠ hub `23,21` |
...
Nothing in the sample looks accidental. The expensive rooms say so (E11S2 lap 1.5, E11S7 lap 8.67 + take, E8S9 68-cut pricey, E12S5 drop).

---

## Clean re-derivations (terrain + shipped structure lists)

From `_r33-mech/rederive.mjs` (does not import `validate.mjs`):
...
Findings 1–3 are the round. r29p11 closed the *named* leak+`complete=false` skip. The prefix is still a neighbor list. The last discarded rung is still a free enclosure that seals. A closer who only adds another identity over producer leaves will ship this review again.
...
4. **Presence.** Zero `corridorPlaced` / `stubRoads` / `towerOnly` / `stitched` / `boundHeld` / `digRoads` / `roadsEaten` on a single room. Fleet-wide zero of the class minus `baseCut`+`shallowNow` is silent. Fleet-wide zero minus `baseCut` only fails 3 rooms on the `shallowNow` note. Lane-anchor / swap-offer "BITES" die if you zero both copies (and regen the basis).
