/**
 * Fleet inventory for r30 attacks. Terrain + shipped lists. checkRoom only on sample.
 */
import fs from "fs";
...
const cutAdopted = { nonempty: 0, empty: 0, l7adds: 0, l7badds: 0, roomsWithL7add: [] };
const cutPasses = { reconcile: 0, pruneDel: 0, twoPruneDel: 0, twoPruneRooms: [] };
const boardsField = [];
...
for (const p of plans) {
  ...
  if (p.meta?.sealedRecovery?.outcome === "taken") {
    taken.push({
      ...
      pocketTiles: (p.meta.sealedRecovery.pockets || []).reduce((n, pk) => n + (pk?.tiles || 0), 0),
      notes: (p.meta.noteRecords || []).filter((n) => n && n.cls === "sealedRecovery").length,
    });
  }
  ...
  if (p.seed && Number.isInteger(p.seed.x)) {
    ...
    if (hub && p.seed.x === hub.x && p.seed.y === hub.y) seed.eqHub++;
      ...
      if (seed.neHubRooms.length < 8) {
        seed.neHubRooms.push({ room: p.room, seed: p.seed, hub, score: p.meta?.seedScore, skip: p.meta?.seedSkip });
      }
    }
  ...
  const l7 = drift.filter((e) => e && e.op === "add" && e.pass === "layer7-reconcileSeal");
  const l7b = drift.filter((e) => e && e.op === "add" && e.pass === "layer7b-reconcileSeal");
  cutAdopted.l7adds += l7.length;
  cutAdopted.l7badds += l7b.length;
  if (l7.length && cutAdopted.roomsWithL7add.length < 6) {
    cutAdopted.roomsWithL7add.push({ room: p.room, add: l7[0], adopted: sh.cutAdopted });
  }
...
function inspectRoom(plan) {
  ...
  return {
    ...
    hub: plan.hub,
    sitter,
    mineral: seat ? { ...seat, off: !roads.has(K(seat)) && !(s.container || []).some((c) => K(c) !== K(seat) && cheb(c, seat) <= 1) } : null,
    leaks,
    throughCut,
    throughFreeze,
...
const fatDiff = ladders.filter((l) => l.fat && !l.cutEqFreeze);
...
const out = {
  ...
  ladders: {
    ...
    fatAndCutNeFreeze: fatDiff.length,
    fatDiff: fatDiff.slice(0, 8),
...
console.log(JSON.stringify({
  ...
  boardsField,
  fullRun,
  shrunkN: shrunk.length,
  droppedN: dropped.length,
  ladders: out.ladders.n,
  fat: out.ladders.fat,
  fatAndCutNeFreeze: out.ladders.fatAndCutNeFreeze,
  fatDiffRooms: fatDiff.map((l) => l.room),
