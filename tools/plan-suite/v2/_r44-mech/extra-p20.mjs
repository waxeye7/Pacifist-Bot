...
const rLabs = run("P21-labs-baseLap-zeroed-E11S2", room, (p) => {
  if (p.meta.labs?.lapVeto) p.meta.labs.lapVeto.baseLap = 0;
});
const rLabsPlus = run("P21-labs-baseLap-plus-1-E11S2", room, (p) => {
  if (p.meta.labs?.lapVeto && typeof p.meta.labs.lapVeto.baseLap === "number") {
    p.meta.labs.lapVeto.baseLap += 1;
  }
});
const rVetoAndLabs = run("P21-veto-and-labs-baseLap-both-zeroed", room, (p) => {
  p.meta.misc.mobilityVeto.baseLap = 0;
  if (p.meta.labs?.lapVeto) p.meta.labs.lapVeto.baseLap = 0;
});

let haveLabs = 0, matchLabs = 0;
let haveTowers = 0, matchTowersP21 = 0;
for (const p of plans) {
  const d = byRoom.get(p.room);
  if (!d) continue;
  const st = wantMob(d.terrain, p);
  const got = p.meta?.labs?.lapVeto?.baseLap;
  if (typeof got === "number" && st) {
    haveLabs++;
    if (Math.abs(st.maxGated - got) < 1e-9) matchLabs++;
  }
  const tw = p.meta?.towers?.mobilityVeto?.baseLap;
  if (typeof tw === "number" && st) {
    haveTowers++;
    if (Math.abs(st.maxGated - tw) < 1e-9) matchTowersP21++;
  }
}

const rTowers0 = run("P22-towers-baseLap-zeroed-E11S7", "E11S7", (p) => {
  p.meta.towers.mobilityVeto.baseLap = 0;
});
const rTowersPlus = run("P22-towers-baseLap-plus-1-E11S7", "E11S7", (p) => {
  p.meta.towers.mobilityVeto.baseLap += 1;
});
const rTowersToP21 = run("P22-towers-baseLap-set-to-p21-E11S7", "E11S7", (p) => {
  p.meta.towers.mobilityVeto.baseLap = p.meta.misc.mobilityVeto.baseLap;
});

console.log(JSON.stringify({
  haveBog, matchBog, haveW, matchW, haveB, matchB, haveLabs, matchLabs,
  haveTowers, matchTowersP21,
  miss, sample, wasLapVsBaseLap,
  rBog0, rWas0, rBaseLap, rBoth, rBogAndBase, rLabs, rLabsPlus, rVetoAndLabs,
  rTowers0, rTowersPlus, rTowersToP21,
}, null, 2));
