/**
 * Round-44 hostile mutations. In-memory checkRoom only. Never writes the artifact.
 * Named BITES re-probes: p22 towers.baseLap (empty-room freeze-cut lap);
 * p21 baseLap (veto + labs.lapVeto); p20 baseOverGated / wasLap; p19
 * takeTowerSwap.from other D8 / unreachedClusters / unreachableExts /
 * servedExts; p18 maxHubDist / from→1,1; p17 stubCap 43↔51 / floorUngated
 * / radii / parkCap / takeTowerSwap.to; p16 hubDistCap/lapCeilingFloor/
 * corridorPlaced; seedScore; p15–p12; 88 leaky+complete=false; 98 one-copy.
 * ESCAPE class: 98 both-lists, wanted+=1, 88 same-lap, hubDistCap 16→19,
 * leftover presence, protectRadius enum.
 */
import fs from "fs";
...
const rooms = {
  ...
  floorUngated: any((p) => typeof p.meta?.extensions?.laneMeta?.floorUngated === "number" && p.meta.extensions.laneMeta.floorUngated !== 0) || "E11S1",
  radii: any((p) => Array.isArray(p.meta?.composeOpts?.radii) && p.meta.composeOpts.radii.length > 0) || null,
  radiiLate: any((p) => {
    const r = p.meta?.composeOpts?.radii;
    return Array.isArray(r) && r.join(",") === "10,11,12,13,14";
  }) || null,
  radiiWide: any((p) => {
    const r = p.meta?.composeOpts?.radii;
    return Array.isArray(r) && r.join(",") === "6,7,8,9,10,11,12,13,14";
  }) || null,
  radiiNone: any((p) => p.meta?.composeOpts && !p.meta.composeOpts.radii) || null,
  parkCap: any((p) => typeof p.meta?.composeOpts?.parkCap === "number" && p.meta.composeOpts.parkCap !== 0) || null,
  takeSwap: any((p) => p.meta?.composeOpts?.takeTowerSwap?.to && Number.isInteger(p.meta.composeOpts.takeTowerSwap.to.x)) || null,
  maxHubDist: any((p) => typeof p.meta?.extensions?.maxHubDist === "number" && p.meta.extensions.maxHubDist !== 0) || "E11S1",
  unreachedCl: any((p) => typeof p.meta?.walls?.unreachedClusters === "number") || "E11S1",
  unreachableExts: any((p) => typeof p.meta?.walls?.unreachableExts === "number") || "E11S1",
  servedExts: any((p) => typeof p.meta?.walls?.servedExts === "number") || "E11S1",
  servedExtsZero: any((p) => p.meta?.walls?.servedExts === 0) || "E11S1",
  servedExtsNz: any((p) => typeof p.meta?.walls?.servedExts === "number" && p.meta.walls.servedExts !== 0) || null,
  fillerNz: any((p) => (p.meta?.walls?.fillerTiles || 0) > 0) || null,
  baseOverGated: any((p) => typeof p.meta?.misc?.mobilityVeto?.baseOverGated === "number" && p.meta.misc.mobilityVeto.baseOverGated !== 0) || "E11S2",
  baseOverGatedZero: any((p) => p.meta?.misc?.mobilityVeto?.baseOverGated === 0) || null,
  wasLap: any((p) => (p.meta?.misc?.mobilityVeto?.refused || []).some((r) => r && typeof r.wasLap === "number" && r.wasLap !== 0)) || null,
  wasLapZero: any((p) => (p.meta?.misc?.mobilityVeto?.refused || []).some((r) => r && r.wasLap === 0)) || null,
  baseLap: any((p) => typeof p.meta?.misc?.mobilityVeto?.baseLap === "number" && p.meta.misc.mobilityVeto.baseLap !== 0) || "E11S2",
  labsBaseLap: any((p) => typeof p.meta?.labs?.lapVeto?.baseLap === "number" && p.meta.labs.lapVeto.baseLap !== 0) || "E11S2",
  towersBaseLap: any((p) => typeof p.meta?.towers?.mobilityVeto?.baseLap === "number" && p.meta.towers.mobilityVeto.baseLap !== 0) || "E11S7",
};
...
// r29p17 named closes — stubCap is the rich/poor pool walk, floorUngated is
// the freeze mass-free walk, radii/parkCap/takeTowerSwap are composeOpts.
{
  const p = byPlan.get(rooms.floorUngated);
  const sw = rooms.takeSwap ? byPlan.get(rooms.takeSwap) : null;
  rec({
    name: "INFO-p17-walks",
    room: rooms.floorUngated,
    status: "INFO",
    detail: JSON.stringify({
      floorUngated: p?.meta?.extensions?.laneMeta?.floorUngated,
      floorUngatedTwin: p?.meta?.walls?.mobility?.lanes?.floorUngated,
      stubCap43: byPlan.get(rooms.stubCap43)?.meta?.extensions?.stubCap,
      stubCap51: rooms.stubCap51 && byPlan.get(rooms.stubCap51)?.meta?.extensions?.stubCap,
      radii: rooms.radii && byPlan.get(rooms.radii)?.meta?.composeOpts?.radii,
      radiiBonus: rooms.radii && byPlan.get(rooms.radii)?.meta?.composeOpts?.needDeepBonus,
      parkCap: rooms.parkCap && byPlan.get(rooms.parkCap)?.meta?.composeOpts?.parkCap,
      parkFloor: rooms.parkCap && byPlan.get(rooms.parkCap)?.meta?.ctrlParkFloorCap,
      takeSwap: sw && sw.meta?.composeOpts?.takeTowerSwap,
      takeTowers: sw && (sw.structures?.tower || []).map((t) => `${t.x},${t.y}`),
      rooms: {
        floorUngated: rooms.floorUngated,
        stubCap43: rooms.stubCap43,
        stubCap51: rooms.stubCap51,
        radii: rooms.radii,
        radiiLate: rooms.radiiLate,
        radiiWide: rooms.radiiWide,
        radiiNone: rooms.radiiNone,
        parkCap: rooms.parkCap,
        takeSwap: rooms.takeSwap,
      },
    }),
  });
}
rec(run("P17-stubCap-43-to-51", rooms.stubCap43, (p) => { p.meta.extensions.stubCap = 51; }));
if (rooms.stubCap51) {
  rec(run("P17-stubCap-51-to-43", rooms.stubCap51, (p) => { p.meta.extensions.stubCap = 43; }));
}
rec(run("P17-floorUngated-zeroed", rooms.floorUngated, (p) => {
  p.meta.extensions.laneMeta.floorUngated = 0;
  if (p.meta.walls?.mobility?.lanes) p.meta.walls.mobility.lanes.floorUngated = 0;
}));
rec(run("P17-floorUngated-one-copy", rooms.floorUngated, (p) => {
  p.meta.extensions.laneMeta.floorUngated = 0;
}));
if (rooms.radii) {
  rec(run("P17-radii-rewritten", rooms.radii, (p) => { p.meta.composeOpts.radii = [1, 2, 3]; }));
}
if (rooms.radiiNone) {
  rec(run("P17-radii-planted-on-absent", rooms.radiiNone, (p) => {
    p.meta.composeOpts.radii = [6, 7, 8, 9, 10, 11, 12, 13, 14];
  }));
}
if (rooms.parkCap) {
  rec(run("P17-parkCap-zeroed", rooms.parkCap, (p) => { p.meta.composeOpts.parkCap = 0; }));
  rec(run("P17-parkCap-and-ctrlParkFloorCap-both-plus-1", rooms.parkCap, (p) => {
    p.meta.composeOpts.parkCap += 1;
    if (typeof p.meta.ctrlParkFloorCap === "number") p.meta.ctrlParkFloorCap += 1;
  }));
}
if (rooms.takeSwap) {
  rec(run("P17-takeTowerSwap-to-moved-1-1", rooms.takeSwap, (p) => {
    p.meta.composeOpts.takeTowerSwap.to = { x: 1, y: 1 };
  }));
  rec(run("P17-takeTowerSwap-to-other-shipped-tower", rooms.takeSwap, (p) => {
    const sw = p.meta.composeOpts.takeTowerSwap;
    const towers = p.structures?.tower || [];
    const other = towers.find((t) => t.x !== sw.to.x || t.y !== sw.to.y);
    if (other) sw.to = { x: other.x, y: other.y };
  }));
  rec(run("P17-takeTowerSwap-from-to-empty-1-1", rooms.takeSwap, (p) => {
    p.meta.composeOpts.takeTowerSwap.from = { x: 1, y: 1 };
  }));
  rec(run("P17-takeTowerSwap-from-onto-live-tower", rooms.takeSwap, (p) => {
    const sw = p.meta.composeOpts.takeTowerSwap;
    const towers = p.structures?.tower || [];
    const live = towers.find((t) => t.x !== sw.to.x || t.y !== sw.to.y);
    if (live) sw.from = { x: live.x, y: live.y };
  }));
}
if (rooms.radiiLate) {
  rec(run("P17-radii-late-to-wide-bonus-held", rooms.radiiLate, (p) => {
    p.meta.composeOpts.radii = [6, 7, 8, 9, 10, 11, 12, 13, 14];
  }));
  rec(run("P17-radii-and-bonus-both-to-wide", rooms.radiiLate, (p) => {
    p.meta.composeOpts.needDeepBonus = 25;
    p.meta.composeOpts.radii = [6, 7, 8, 9, 10, 11, 12, 13, 14];
  }));
}
if (rooms.radiiWide) {
  rec(run("P17-radii-wide-to-late-bonus-held", rooms.radiiWide, (p) => {
    p.meta.composeOpts.radii = [10, 11, 12, 13, 14];
  }));
}

// r29p18 named closes — maxHubDist is the layer-6 hub-field max of the
// pre-7b seats; takeTowerSwap.from is a D8 neighbour of to.
{
  const p = byPlan.get(rooms.maxHubDist);
  const sw = rooms.takeSwap ? byPlan.get(rooms.takeSwap) : null;
  rec({
    name: "INFO-p18-walks",
    room: rooms.maxHubDist,
    status: "INFO",
    detail: JSON.stringify({
      maxHubDist: p?.meta?.extensions?.maxHubDist,
      hubDistCap: p?.meta?.extensions?.hubDistCap,
      takeSwap: sw && sw.meta?.composeOpts?.takeTowerSwap,
      takeTowers: sw && (sw.structures?.tower || []).map((t) => `${t.x},${t.y}`),
      rooms: { maxHubDist: rooms.maxHubDist, takeSwap: rooms.takeSwap },
    }),
  });
}
rec(run("P18-maxHubDist-zeroed", rooms.maxHubDist, (p) => { p.meta.extensions.maxHubDist = 0; }));
rec(run("P18-maxHubDist-plus-1", rooms.maxHubDist, (p) => { p.meta.extensions.maxHubDist += 1; }));
if (rooms.takeSwap) {
  rec(run("P18-takeTowerSwap-from-to-empty-1-1", rooms.takeSwap, (p) => {
    p.meta.composeOpts.takeTowerSwap.from = { x: 1, y: 1 };
  }));
  rec(run("P18-takeTowerSwap-from-other-d8-of-to", rooms.takeSwap, (p) => {
    const sw = p.meta.composeOpts.takeTowerSwap;
    const from = sw.from;
    const to = sw.to;
    if (!from || !to) return;
...
// r29p19 named closes — takeTowerSwap.from is the vacated seat the published
// swap names; unreachedClusters is the shipped-cut clusters of size >=2 that
// no road D8-touches; unreachableExts is the extensions with no D4 road;
// servedExts is 0 iff the filler laid nothing.
{
  const p = byPlan.get(rooms.unreachedCl);
  const sw = rooms.takeSwap ? byPlan.get(rooms.takeSwap) : null;
  rec({
    name: "INFO-p19-walks",
    room: rooms.unreachedCl,
    status: "INFO",
    detail: JSON.stringify({
      unreachedClusters: p?.meta?.walls?.unreachedClusters,
      unreachableExts: p?.meta?.walls?.unreachableExts,
      servedExts: p?.meta?.walls?.servedExts,
      fillerTiles: p?.meta?.walls?.fillerTiles,
      laidExtFace: p?.meta?.walls?.laidByKind?.extFace,
      takeSwap: sw && sw.meta?.composeOpts?.takeTowerSwap,
      taken: sw && sw.meta?.towers?.acrossPriorTake?.taken,
      twin: sw && sw.meta?.towers?.towerSwapTaken,
      offerBest: sw && sw.meta?.towers?.towerSwapOffer?.best,
      sap: sw && sw.meta?.towers?.adjacency?.satAcrossPrior,
      rooms: {
        unreachedCl: rooms.unreachedCl,
        unreachableExts: rooms.unreachableExts,
        servedExts: rooms.servedExts,
        servedExtsZero: rooms.servedExtsZero,
        servedExtsNz: rooms.servedExtsNz,
        fillerNz: rooms.fillerNz,
        takeSwap: rooms.takeSwap,
      },
    }),
  });
}
rec(run("P19-unreachedClusters-plus-1", rooms.unreachedCl, (p) => { p.meta.walls.unreachedClusters += 1; }));
rec(run("P19-unreachableExts-plus-1", rooms.unreachableExts, (p) => { p.meta.walls.unreachableExts += 1; }));
rec(run("P19-servedExts-plus-1", rooms.servedExts, (p) => { p.meta.walls.servedExts += 1; }));
rec(run("P19-servedExts-zeroed", rooms.servedExts, (p) => { p.meta.walls.servedExts = 0; }));
if (rooms.servedExtsNz) {
  rec(run("P19-servedExts-nz-plus-1", rooms.servedExtsNz, (p) => { p.meta.walls.servedExts += 1; }));
}
if (rooms.fillerNz) {
  rec(run("P19-servedExts-plus-1-on-filler-room", rooms.fillerNz, (p) => { p.meta.walls.servedExts += 1; }));
}
// Residue: the 0-iff is an identity. Bump both sides together.
rec(run("P19-servedExts-and-filler-both-plus-1", rooms.servedExts, (p) => {
...
if (rooms.takeSwap) {
  rec(run("P19-takeTowerSwap-from-other-d8-of-to", rooms.takeSwap, (p) => {
    const sw = p.meta.composeOpts.takeTowerSwap;
    const from = sw.from;
    const to = sw.to;
    if (!from || !to) return;
    ...
    for (const [dx, dy] of D8) {
      ...
      if (towers.has(`${x},${y}`)) continue;
      sw.from = { x, y };
      break;
    }
  }));
  rec(run("P19-take-to-other-tower-and-from-d8-of-new-to", rooms.takeSwap, (p) => {
    const sw = p.meta.composeOpts.takeTowerSwap;
    const towers = p.structures?.tower || [];
    const other = towers.find((t) => t.x !== sw.to.x || t.y !== sw.to.y);
    if (!other) return;
  ...
  rec(run("P19-take-from-other-d8-and-rewrite-published-twins", rooms.takeSwap, (p) => {
    const sw = p.meta.composeOpts.takeTowerSwap;
    const from = sw.from;
    const to = sw.to;
    if (!from || !to) return;
    ...
    if (!alt) return;
    sw.from = alt;
    const rec = { from: { x: alt.x, y: alt.y }, to: { x: to.x, y: to.y } };
...
// r29p20 named closes — baseOverGated is the freeze-cut overGated with the
// extension mass / nuker / observer lifted; wasLap is that walk's maxGated.
{
  const p = byPlan.get(rooms.baseOverGated);
  const veto = p?.meta?.misc?.mobilityVeto;
  rec({
    name: "INFO-p20-walks",
    room: rooms.baseOverGated,
    status: "INFO",
    detail: JSON.stringify({
      baseOverGated: veto?.baseOverGated,
      baseLap: veto?.baseLap,
      refused: (veto?.refused || []).map((r) => ({ x: r.x, y: r.y, lap: r.lap, wasLap: r.wasLap })),
      rooms: {
        baseOverGated: rooms.baseOverGated,
        baseOverGatedZero: rooms.baseOverGatedZero,
        wasLap: rooms.wasLap,
        wasLapZero: rooms.wasLapZero,
        baseLap: rooms.baseLap,
      },
    }),
  });
}
rec(run("P20-baseOverGated-zeroed", rooms.baseOverGated, (p) => {
  p.meta.misc.mobilityVeto.baseOverGated = 0;
}));
rec(run("P20-baseOverGated-plus-1", rooms.baseOverGated, (p) => {
  p.meta.misc.mobilityVeto.baseOverGated += 1;
}));
if (rooms.baseOverGatedZero) {
  rec(run("P20-baseOverGated-zero-room-plus-1", rooms.baseOverGatedZero, (p) => {
    p.meta.misc.mobilityVeto.baseOverGated += 1;
  }));
}
if (rooms.wasLap) {
  rec(run("P20-wasLap-zeroed", rooms.wasLap, (p) => {
    for (const r of p.meta.misc.mobilityVeto.refused || []) {
      if (r && typeof r.wasLap === "number") r.wasLap = 0;
    }
  }));
  rec(run("P20-wasLap-plus-1", rooms.wasLap, (p) => {
    for (const r of p.meta.misc.mobilityVeto.refused || []) {
      if (r && typeof r.wasLap === "number") r.wasLap += 1;
    }
  }));
  rec(run("P20-wasLap-one-row-zeroed", rooms.wasLap, (p) => {
    const row = (p.meta.misc.mobilityVeto.refused || []).find((r) => r && typeof r.wasLap === "number" && r.wasLap !== 0);
    if (row) row.wasLap = 0;
  }));
}
if (rooms.baseLap) {
  rec(run("P20-baseLap-zeroed", rooms.baseLap, (p) => {
    p.meta.misc.mobilityVeto.baseLap = 0;
  }));
  rec(run("P20-baseLap-plus-1", rooms.baseLap, (p) => {
    p.meta.misc.mobilityVeto.baseLap += 1;
  }));
}

// r29p21 named close — baseLap is the freeze-cut gated lap (same walk as
// wasLap) on both misc.mobilityVeto and labs.lapVeto.
{
  const p = byPlan.get(rooms.baseLap);
  rec({
    name: "INFO-p21-walks",
    room: rooms.baseLap,
    status: "INFO",
    detail: JSON.stringify({
      vetoBaseLap: p?.meta?.misc?.mobilityVeto?.baseLap,
      labsBaseLap: p?.meta?.labs?.lapVeto?.baseLap,
      rooms: { baseLap: rooms.baseLap, labsBaseLap: rooms.labsBaseLap },
    }),
  });
}
if (rooms.baseLap) {
  rec(run("P21-baseLap-zeroed", rooms.baseLap, (p) => {
    p.meta.misc.mobilityVeto.baseLap = 0;
  }));
  rec(run("P21-baseLap-plus-1", rooms.baseLap, (p) => {
    p.meta.misc.mobilityVeto.baseLap += 1;
  }));
  rec(run("P21-baseLap-one-copy-veto", rooms.baseLap, (p) => {
    p.meta.misc.mobilityVeto.baseLap = 0;
  }));
}
if (rooms.labsBaseLap) {
  rec(run("P21-labs-baseLap-zeroed", rooms.labsBaseLap, (p) => {
    p.meta.labs.lapVeto.baseLap = 0;
  }));
  rec(run("P21-labs-baseLap-plus-1", rooms.labsBaseLap, (p) => {
    p.meta.labs.lapVeto.baseLap += 1;
  }));
  rec(run("P21-baseLap-one-copy-labs", rooms.labsBaseLap, (p) => {
    p.meta.labs.lapVeto.baseLap = 0;
  }));
}
if (rooms.baseLap) {
  rec(run("P21-baseLap-both-copies-zeroed", rooms.baseLap, (p) => {
    p.meta.misc.mobilityVeto.baseLap = 0;
...
// r29p22 named close — towers.baseLap is the layer-2 empty-room lap on the
// freeze cut (hub kit only). Not the p21 walk: E11S7 ships 7.33 vs 7.67.
{
  const p = byPlan.get("E11S7");
  rec({
    name: "INFO-p22-walks",
    room: "E11S7",
    status: "INFO",
    detail: JSON.stringify({
      towersBaseLap: p?.meta?.towers?.mobilityVeto?.baseLap,
      vetoBaseLap: p?.meta?.misc?.mobilityVeto?.baseLap,
      labsBaseLap: p?.meta?.labs?.lapVeto?.baseLap,
      repairTwin: (p?.meta?.shortfalls || []).find((s) => s?.repair?.tower && typeof s.repair.tower.baseLap === "number")?.repair?.tower?.baseLap,
      rooms: { towersBaseLap: rooms.towersBaseLap, baseLap: rooms.baseLap },
    }),
  });
}
rec(run("P22-towers-baseLap-zeroed", "E11S7", (p) => {
  p.meta.towers.mobilityVeto.baseLap = 0;
}));
rec(run("P22-towers-baseLap-plus-1", "E11S7", (p) => {
  p.meta.towers.mobilityVeto.baseLap += 1;
}));
rec(run("P22-towers-baseLap-set-to-p21-freeze-cut", "E11S7", (p) => {
  p.meta.towers.mobilityVeto.baseLap = p.meta.misc.mobilityVeto.baseLap;
}));
rec(run("P22-towers-baseLap-zeroed-and-rewrite-repair-twin-regen", "E11S7", (p) => {
  p.meta.towers.mobilityVeto.baseLap = 0;
  for (const sf of p.meta.shortfalls || []) {
    if (sf?.repair?.tower && typeof sf.repair.tower.baseLap === "number") {
      sf.repair.tower.baseLap = 0;
      try { sf.detail = renderDecl(sf); } catch { /* leave */ }
    }
  }
}));
rec(run("P22-towers-baseLap-plus-1-and-rewrite-repair-twin-regen", "E11S7", (p) => {
  p.meta.towers.mobilityVeto.baseLap += 1;
  const next = p.meta.towers.mobilityVeto.baseLap;
  for (const sf of p.meta.shortfalls || []) {
    if (sf?.repair?.tower && typeof sf.repair.tower.baseLap === "number") {
      sf.repair.tower.baseLap = next;
      try { sf.detail = renderDecl(sf); } catch { /* leave */ }
    }
  }
}));
if (rooms.towersBaseLap && rooms.towersBaseLap !== "E11S7") {
  rec(run("P22-towers-baseLap-zeroed-other", rooms.towersBaseLap, (p) => {
    p.meta.towers.mobilityVeto.baseLap = 0;
  }));
}
