# Round 39 mechanical review

Artifact `tools/plan-suite/out-v2/plans-hub.json` md5 `8e5d6725885bd3f3731379bedf408326` (27,780,512 bytes) — **matches expected**. HEAD `80601a0` (r29p17: derive leftover presence — `stubCap` rich/poor walk, `floorUngated` freeze mass-free, `radii`/`parkCap`/`takeTowerSwap` composeOpts). Artifact bytes unchanged since r32. p17 is a gate + mutate-corpus pass (`r27-gates.mjs`, `mutate.mjs`). Rooms dump `tools/plan-suite/v2/_r28-mech/rooms.json` (172/172, 0 missing). Node `v22.13.0` (`fnm` install on PATH). Probes in `tools/plan-suite/v2/_r39-mech/`. No suite source committed. Artifact not written. **No board moved.** No `boards` field. Physicals still 10,320 / 14,100 / 8,208.
...
**Exploit.** E11S1 is a real shrink. After r29p10 the suffix is gone: `fullRun.reserved === lane.reserved` (6 tiles / 2 rounds), `shrunk.wanted=7`. The refused extra is a count. p17 did not touch this.
...
**Exploit.** E11S2 rungs: `1.56/46/37 ×3`, then `2.5/50/42` (last, eco-capped). Shipped wall 46/37 = freeze. Last fat is complete, 42 tiles, has `20,9` and `29,33`. p17 did not touch this.
...
**Exploit.** 40 presence names, 59 derived (p17 moved four: `floorUngated` / `radii` / `parkCap` / `takeTowerSwap`; `stubCap` was already derived in p15 and is now the pool walk). Zeroed every remaining `klass:"presence"` numeric/bool/nonempty-array leaf across all 172 rooms. 18 kinds, 1,422 events. `checkRoom`: **fail 172/172**, every first fail is `baseCut := 0`.

Exclude `baseCut` only: **fail 3/172** — first fail is the `shallowNow` note (E12S6, E2S3, E9S2). Exclude `baseCut`+`shallowNow`: **pass 172/172**. Same 172/172 if those two plus all newly derived names (p12–p17) are skipped. Same 172/172 if those two plus **every** `klass:"derived"` name are skipped (16 leftover kinds, 1,247 events). The r38 "minus derived+baseCut+shallowNow is silent" door is still open.

Single-room flatten, flattering direction — **13 ESCAPE** on the first copy:

`baseOverGated`, `deepExhausted`, `digRoads`, `freeLeft`, `haulCost`, `inertPromoted`, `maxHubDist`, `rescuedLap`, `rescuedTo`, `rolledBackFrom`, `shallowRamparts`, `stubExhausted`, `wasLap`
...
Closed this artifact (re-probed, BITES): `nukerInWindow`, `center`, `mineralSeatNetTiles`, `coveredDetourDeclared`, **p12 five**, **p13 three**, **p14 nine** + `boundLap`, **p15 ten**, **`seedScore`**, **p16 four**, **p17 five**.

`baseCut` is still *classified* `presence` and is only bounded (`>= 1` + `priceyWall` sync). The klass lies. `hubDistCap` is *classified* `derived` and is an enum whose only reader is the sibling formula.

**Root cause.** Criticism 141(c). They derived the four cheap leaves this review named. They bound `stubCap` to the pool (a real walk) and `floorUngated` to the freeze mass-free (a real walk). `radii` / `parkCap` / `takeTowerSwap.to` are identities. The class is still the class.
...
### 4. Exact `baseCut` / `protectRadius` / `hubDistCap` picks are unread. `stubCap` 43↔51 is closed.
...
`stubCap := 0` **BITES**. E11S1 `43 → 51` **BITES** (pool wants 43). E11S2 `51 → 43` **BITES** (pool wants 51). Fleet `{43:12, 51:160}`. The named enum swap is closed. It is 51 iff deep ext-capable tiles / 60 ≥ 1.5, not a comment.
...
Sample: E11S9 `baseCut=7` / cut=9. E17S3 `baseCut=58` = cut (pricey). E1S1 `baseCut=45` = cut = freeze.
...
### 7. p17 five — named flatten BITES; three of the five are identities, not the pick.

| attack | result |
|---|---|
| `stubCap` 43↔51 | **BITES** (pool walk) |
| `floorUngated := 0` both copies / one copy | **BITES** (freeze mass-free / twin) |
| `radii := [1,2,3]` | **BITES** |
| plant `radii` on a no-bonus room (E11S1) | **BITES** (table is absent) |
| `parkCap := 0` (E12S5, vs `ctrlParkFloorCap=2`) | **BITES** |
| `parkCap` and `ctrlParkFloorCap` both `+= 1` | **BITES** (`winningCap` 2 vs re-derived 3) |
| `takeTowerSwap.to → 1,1` | **BITES** (not a shipped tower) |
| `takeTowerSwap.from` onto a live tower | **BITES** |
| late radii → wide, bonus held at 85 | **BITES** |
| wide radii → late, bonus held at 25 | **BITES** |
| `takeTowerSwap.to` → another shipped tower | **ESCAPE** |
| `takeTowerSwap.from → 1,1` (empty) | **ESCAPE** |
| late radii **and** `needDeepBonus := 25` | **ESCAPE** |

`stubCap` is the pool. `floorUngated` is the freeze mass-free walk. `radii` is the bonus table (`25/55 → RADII_WIDE`, `85 → [10..14]`, else absent). `parkCap` is `ctrlParkFloorCap` (and on the two rooms that ship it, `winningCap` too). `takeTowerSwap.to` is some shipped tower, not the destination.

### 8. p16 four / p15 ten — named flatten BITES; identities remain.
...
Independent single-removal over all **7,246** `cutAtFreeze` tiles: **0 loose**, 0 core-seal fails, 29 rooms differ from the shipped cut, 34 add + 46 remove, 27 adopt / 29 drift. Same census as r29–r38.
...
## Attacked and held (r38 closures + r29p17 named, re-probed)
...
`h(room) = fmix32(fnv1a32("round39-mech|" + room))` over the 172 names. Lowest five (not cherry-picked):
...
|---|---|---|---|---|
| E1S1 | 60/83/54/45/45 | 0 | 15,17 off | hashed #1, seed=hub, `protectRadius=8` |
| E11S9 | 60/104/16/9/9 | 0 | 42,41 off | pick 7 ≠ cut 9, seed `18,37` ≠ hub `17,35` |
| E15S6 | 60/81/18/16/16 | 0 | 34,43 off | **taken** 4 fixedHolders, seed=hub |
| E13S5 | 60/83/22/21/21 | 0 | 29,7 off | seed=hub, `protectRadius=10` |
| E17S3 | 60/93/68/58/58 | 0 | 18,35 off | **pricey** 58, seed `12,23` ≠ hub `15,24` |
...
Nothing in the sample looks accidental. The expensive rooms say so (E11S2 lap 1.5, E11S7 lap 8.67 + take, E17S3 pricey 58 + seed≠hub). E11S9 publishes a `baseCut` that is not the shipped cut.

---

## Clean re-derivations (terrain + shipped structure lists)

From `_r39-mech/rederive.mjs` (does not import `validate.mjs`):
...
Findings 1–3 are the round. r29p17 closed the *named* leftover flattens (`stubCap` pool / `floorUngated` walk / `radii` table / `parkCap` twin / `takeTowerSwap.to` on-board). The prefix is still a neighbor list. The last discarded rung is still a free enclosure that seals. A closer who only adds another identity over producer leaves will ship this review again.
...
4. **Presence.** Zero `maxHubDist` / `digRoads` / `deepExhausted` on a single room. Fleet-wide zero of the class minus `baseCut`+`shallowNow` is silent 172/172. Fleet-wide zero minus those **and every `klass:"derived"`**: silent 172/172. Lane-anchor / swap-offer "BITES" die if you zero both copies (and regen the basis).
5. **`protectRadius` 10→12** (or any other `RADII_WIDE` swap). **`baseCut ± 1`** staying on the same side of 45. **`hubDistCap` 16→19/23/999**.
6. **p17 take / radii residues.** E14S1: `takeTowerSwap.to` onto another shipped tower. `takeTowerSwap.from → 1,1`. E18S2: set `needDeepBonus := 25` **and** `radii := RADII_WIDE`.
...
**No.** Named p17 five leftover flattens / named p16 four leftover flattens / named p15 ten leftover walks + `seedScore` / named p14 nine + `boundLap` / named p13 three + stitch 0/1 / named p12 five / named 88 leaky+`complete=false` / named 98 `19,27` on one copy / 88 leaky complete / named 98 COORD bag / 88 shipped-cut / 88 8-tile box / 93 plant-back closed. The kept prefix is a D8-neighbor bag. `wanted` is a count. The last discarded rung is a free sealing enclosure. The presence class still flips once `baseCut`+`shallowNow` are left alone. `hubDistCap` / `protectRadius` are enums. `takeTowerSwap.to` is a tower, not the destination. `radii` is a bonus table. The reserved mineral pair is a topology bound. `clean_rounds` is 0. The bar is 2 consecutive all-clean. Boards did not move.
