# Round 35 mechanical review

Artifact `tools/plan-suite/out-v2/plans-hub.json` md5 `8e5d6725885bd3f3731379bedf408326` (27,780,512 bytes) — **matches expected**. HEAD `954f00b` (r29p13). Artifact bytes unchanged since r32. p13 is a gate + mutate-corpus pass (`r27-gates.mjs`, `mutate.mjs`). Rooms dump `tools/plan-suite/v2/_r28-mech/rooms.json` (172/172, 0 missing). Node `v22.13.0` (`fnm` install on PATH). Probes in `tools/plan-suite/v2/_r35-mech/`. No suite source committed. Artifact not written. **No board moved.** No `boards` field. Physicals still 10,320 / 14,100 / 8,208.
...
A gate that agrees with itself is not a gate. r29p13 closed the *named* leftover walks (`mineralContainer` / `minDmgPicked` / `servedFree`) and made `stitched` the exact 0/1 flag. r29p12 closed the five leftover presence walks. r29p11 closed leaky+`complete=false`. r29p10 closed one-copy `"19,27"`. The kept prefix is still a D8-neighbor bag. `wanted` is still a count. The last discarded rung is still a free enclosure that walks its own lap. The presence class still flips.

---

## BLOCKING

None on the board. r28 mineral-why / swap-offer doors stay closed. 93 plant-back stays closed. 88 leaky + `complete=false` stays closed. Named 98 one-copy stays closed. The five p12 walks stay closed. p13 three walks stay closed. `stitched` 0/1 (including `:= 2` and `:= laid.stitch`) stays closed.
...
7 real shrinks. 9 dropped. 156 plain. 163 rooms have `reserved === lane.reserved`; the 9 `reservedGtLane` are the drops (refused walk still published). A neighbor of the walk is unread once the two copies agree. p13 did not touch this.
...
**Exploit.** 68 presence names, 31 derived (p13 moved three: `mineralContainer`, `minDmgPicked`, `servedFree`; p12 had already moved five). Zeroed every remaining `klass:"presence"` numeric/bool/nonempty-array leaf across all 172 rooms. 37 kinds, 3,621 events. `checkRoom`: **fail 172/172**, every first fail is `baseCut := 0` (three rooms also `shallowNow` note-identity).

Exclude `baseCut` only: **pass 169/172**. The three fails are `shallowNow` note identity (E12S6, E2S3, E9S2). Exclude `baseCut`+`shallowNow`: **pass 172/172**. Same 172/172 if those two plus all newly derived names (p12 five + p13 three) are skipped (the eight are already off the presence walk).

Single-room flatten, flattering direction — **30 ESCAPE** on the first copy:

`baseOverGated`, `boundHeld`, `boundLap`, `corridorFallback`, `corridorPlaced`, `deepExhausted`, `deepReach`, `digRoads`, `floorGated`, `floorOver`, `floorOverGated`, `freeDin`, `freeLeft`, `haulCost`, `hubDistCap`, `inertPromoted`, `lapCeilingFloor`, `massAdds`, `maxDist`, `maxHubDist`, `minDmgArray`, `parkCap`, `radii`, `rescuedLap`, `shallowCost`, `shallowRamparts`, `shallowRefused`, `stubCap`, `stubExhausted`, `wasLap`
...
Closed this artifact (re-probed, BITES): `nukerInWindow`, `center`, `mineralSeatNetTiles`, `coveredDetourDeclared`, **p12 five** (`stitched` / `stitchTiles` / `roadsEaten` / `towerOnly` / `stubRoads`), **p13 three** (`mineralContainer` / `minDmgPicked` / `servedFree`).

`stitched := 0` on E11S9 **BITES**. `stitched := 2` on E8S4 (laid.stitch=2, shipped flag=1) **BITES**. `stitched := laid.stitch` (same room, 2) **BITES**. `stitched := 1` on a room with laid.stitch=0 **BITES**. `stitched := 1` where the room already ships 1 is a no-op. The p12 polarity door is closed. The flag is exact 0/1, not the laid count (`stitchTiles` is the count).

`baseCut` is still *classified* `presence` and is only bounded (`>= 1` + `priceyWall` sync). The klass lies.

**Root cause.** Criticism 141(c). They derived the three cheap walks this review named, and bound the stitch flag. The class is still the class.
...
Sample: E15S9 `baseCut=61` = cut (pricey). E19S7 `baseCut=18` / freeze=21 / cut=21 (pick is neither). E17S6 `baseCut=41` = cut, lap 1.44. The pick is not freeze and not the shipped cut.
...
Independent single-removal over all **7,246** `cutAtFreeze` tiles: **0 loose**, 0 core-seal fails, 29 rooms differ from the shipped cut, 34 add + 46 remove, 27 adopt / 29 drift. Same census as r29–r34.
...
## Attacked and held (r34 closures + r29p13 named, re-probed)
...
`h(room) = fmix32(fnv1a32("round35-mech|" + room))` over the 172 names. Lowest five (not cherry-picked):
...
|---|---|---|---|---|
| E17S6 | 60/86/43/41/41 | **1.44** | 34,18 **on** | **98 shrink** 12 / wanted 17, seed `29,28` ≠ hub `29,27` |
| E4S2 | 60/98/40/31/31 | **1.5** | 31,39 off | seed=hub, `protectRadius=10` |
| E15S9 | 60/79/73/61/61 | 0 | 45,5 off | **pricey** 61, seed=hub, `protectRadius=8` |
| E19S7 | 60/74/24/21/21 | 0 | 27,32 **on** | **pick** `baseCut=18` ≠ freeze 21, seed `32,38` ≠ hub `33,38` |
| E4S9 | 60/71/23/17/17 | 0 | 28,23 off | empty reserved, seed=hub, `protectRadius=6` |
...
Nothing in the sample looks accidental. The expensive rooms say so (E17S6 lap 1.44 + shrink, E4S2 lap 1.5, E15S9 61-cut pricey, E11S2 lap 1.5, E11S7 lap 8.67 + take). E19S7's `baseCut` is not the freeze and not the shipped cut.

---

## Clean re-derivations (terrain + shipped structure lists)

From `_r35-mech/rederive.mjs` (does not import `validate.mjs`):
...
Findings 1–3 are the round. r29p13 closed the *named* three leftover walks and the stitch-flag polarity. The prefix is still a neighbor list. The last discarded rung is still a free enclosure that seals. A closer who only adds another identity over producer leaves will ship this review again.
...
4. **Presence.** Zero `corridorPlaced` / `stubCap` / `boundHeld` / `digRoads` on a single room. Fleet-wide zero of the class minus `baseCut`+`shallowNow` is silent. Fleet-wide zero minus `baseCut` only fails 3 rooms on the `shallowNow` note. Lane-anchor / swap-offer "BITES" die if you zero both copies (and regen the basis).
