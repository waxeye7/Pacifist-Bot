...
function filmVsBoard(plan, terrain) {
  ...
  const facetCounts = {};
  for (const t of painted) {
    const want = classifyRampart(plan, terrain, KT(t.x, t.y), depthFreeze, extFreeze);
...
for (const name of names) {
  ...
  out.push({
    ...
    shortfalls: (plan.meta?.shortfalls || []).map((s) => s.gate || s.kind).filter(Boolean),
