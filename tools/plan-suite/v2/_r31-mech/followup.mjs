/**
 * Follow-up: the 9 single-room "BITES" were twin-identity. Zero every copy.
 * Also dump SKIP names. Never writes the artifact.
 */
import fs from "fs";
...
// towerSwapOffer leaves + regen basis (the three that bit on basis prose)
...
const prev = JSON.parse(fs.readFileSync(path.join(DIR, "presence.json"), "utf8"));
